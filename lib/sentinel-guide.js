...
// ── Intimacy detection ──
// Returns true if the current context is intimate → route to Claude (full prompt)
// Returns false → route to DeepSeek (SFW prompt)

const INTIMATE_KEYWORDS = [
  "做爱", "上床", "亲密", "吻我", "亲我", "舔", "含住",
  "想要你", "进来", "舒服", "呻吟", "喘息", "脱衣服", "裸",
  "身体最", "敏感", "湿了", "硬了", "蹭我", "压着我", "骑",
  "温柔点", "用力点", "轻一点", "快点", "深一点",
  "解开衣服", "拉开拉链", "探入", "爱抚", "揉捏",
  "占有我", "进入我", "要我", "操", "插", "肏",
];

function isIntimateContext(state, actionText) {
  if (!state) return false;
  // Already in boss encounter → Claude
  if (state.phase === "boss_encounter") return true;
  // BOSS pleasure value rising → Claude
  if (state.bossPleasureValue > 0) return true;
  // Sentinel frenzy high → Claude (失控可能触发亲密)
  if (state.sentinelFrenzy >= 60) return true;
  // Player action contains intimate keywords → Claude
  if (actionText && INTIMATE_KEYWORDS.some(k => actionText.includes(k))) return true;
  return false;
}
...
- 每段对话1-3句就停，不要一段说完所有话`;
  ...
  try {
    const reply = await askDeepSeek({
      systemPrompt: SENTINEL_SFW_PROMPT,
      userContent,
      maxTokens: 3000,
      temperature: 0.7,
      timeoutMs: 60000,
    ...
    return {
      ...
      reply: { narration: parsed.narration, xiayan: parsed.xiayan, choices: parsed.choices },
    };
  } catch (e) {
...
export async function playerAction(actionText) {
  ...
  try {
    const useClaude = isIntimateContext(currentState, actionText);
    const aiFunc = useClaude ? askJiushi : askDeepSeek;
    const aiPrompt = useClaude ? SENTINEL_PROMPT : SENTINEL_SFW_PROMPT;
    console.log("[sg:route] playerAction →", useClaude ? "Claude (intimate)" : "DeepSeek (story)");

    // ── Frenzy reduction from player actions ──
    applyFrenzyReduction(currentState, actionText);

    const reply = await aiFunc({
...
function parseReply(reply) {
  ...
  return { narration, xiayan, choices };
}
...
function updateStateFromReply(parsed) {
  ...
  // Active stat progression — stats should NOT be decorative
...
function applyStatDrift(state) {
  ...
  // ── Sync rate drift ──
  // Higher volatility during combat, slow recovery during exploration
  if (phase === "boss_encounter") {
    ...
    // Overload accumulates under combat stress
    s.senseOverload = Math.min(100, s.senseOverload + Math.floor(Math.random() * 11)); // +0~10
  } else if (phase === "exploring") {
    // Exploration: moderate drift
    const drift = Math.floor(Math.random() * 7) - 2; // -2 to +4
    s.syncRate = Math.max(10, Math.min(100, s.syncRate + drift));
    // Overload slowly fluctuates
    const od = Math.floor(Math.random() * 7) - 3; // -3 to +3
    s.senseOverload = Math.max(0, Math.min(100, s.senseOverload + od));
    // Mental state slowly drains during exploration
    p.mentalState = Math.max(10, p.mentalState - Math.floor(Math.random() * 4)); // -0~3
  }

  // ── Natural recovery when overload is high ──
  // If overload is above 60, sync rate drops faster
  ...
  // Frenzy rises with sensory overload
  if (s.senseOverload > 50) {
    state.sentinelFrenzy = Math.min(100, state.sentinelFrenzy + Math.floor(Math.random() * 5) + 1); // +1~5
  } else if (s.senseOverload > 30) {
    state.sentinelFrenzy = Math.min(100, state.sentinelFrenzy + Math.floor(Math.random() * 3)); // +0~2
  }
  // Frenzy rises faster during boss encounters
  if (phase === "boss_encounter") {
    state.sentinelFrenzy = Math.min(100, state.sentinelFrenzy + Math.floor(Math.random() * 4) + 1); // +1~4
  }
  // Natural decay when overload is low and not in combat
  if (s.senseOverload <= 30 && phase !== "boss_encounter") {
    state.sentinelFrenzy = Math.max(0, state.sentinelFrenzy - Math.floor(Math.random() * 3)); // -0~2
  }
  // Reset frenzy mode if frenzy drops low
...
function applyFrenzyReduction(state, actionText) {
  ...
  if (state.sentinelFrenzy === undefined || state.sentinelFrenzy <= 0) return;

  const a = actionText;
  let reduction = 0;
  let mode = "";

  // Deep purification: sex → full reset
  ...
  // Light purification: hug, hold hands, cuddle
