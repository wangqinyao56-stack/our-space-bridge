...
function parseReply(reply) {
  ...
  if (working.includes("[旁白]")) {
    ...
    // No markers — parse line-by-line
    // Lines starting with （ or " → xiayan (actions/dialogue)
    ...
    for (const line of allLines) {
      const t = line.trim();
      if (!t) {
        // Empty line: add to whichever list was last (keeps paragraph cohesion)
        continue;
      }
