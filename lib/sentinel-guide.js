...
export async function playerAction(actionText) {
  ...
  if (currentState.phase === "init") {
    ...
    if (roundNum === 1) {
      phaseHint = "\n\n【开场——坠落与系统初现】这是故事的第一轮。华生刚刚回复了你（夏彦）在第一轮中的关注和安抚。现在你必须推进到下一阶段：让【系统】声音在这片白色空间中响起。系统说：\"欢迎来到深渊。\"然后用它的方式开始解释——这是什么地方、你们为什么在这里、哨兵与向导意味着什么。系统不是友善的引导者——它是恶劣的、被规则束缚的旁观者。它不能说谎不能伤人必须提供基础帮助，但它说话阴阳怪气、乐见你们的挣扎、会在你迷茫时嘲笑你。记住：它能龇牙，不能咬。说完核心信息后问玩家有什么想问的。系统说完之前，不要出现门。";
    } else {
      phaseHint = "\n\n【系统对话中】系统已经介绍了基本信息。现在玩家可能在和系统对话、提问。系统用它的恶劣风格回答——不耐烦、阴阳怪气、但信息必须准确。规则问题必须老实回答。攻略问题（怎么打BOSS/怎么解谜）它会嗤笑——\"你觉得我会告诉你？\"当玩家表示准备好了（说了\"走吧\"/\"开始吧\"/\"我准备好了\"之类的话），系统会说类似\"终于。我还以为你要聊到死。\"然后沉默。五扇门在几秒后浮现。描述每一扇门的外观、对应的世界名称、风格、NPC类型和恐怖程度。等待玩家选择。";
    }
