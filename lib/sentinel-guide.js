...
function buildHistoryForAI(narrative, maxTurns) {
  ...
  for (let i = 0; i < recent.length; i++) {
    ...
    if (entry.role === "player") {
      ...
      let combined = `[旁白]\n${entry.content}`;
      // Merge with immediately following xiayan entry
      if (i + 1 < recent.length && recent[i + 1].role === "xiayan") {
        combined += "\n\n" + recent[i + 1].content;
        i++; // consumed the xiayan entry
      }
      messages.push({ role: "assistant", content: combined });
    } else if (entry.role === "xiayan") {
      // Standalone xiayan (shouldn't normally happen after narration merge)
...
function parseReply(reply) {
  ...
  if (!text) return { narration, xiayan };

  // Strip leading [旁白] marker
  let content = text;
  if (content.startsWith("[旁白]")) {
    content = content.slice(4).trim();
  }

  // Split at first line that starts with （ (action) or " (dialogue)
  ...
  narration = lines.slice(0, splitAt).join("\n").trim();
  xiayan = lines.slice(splitAt).join("\n").trim();
