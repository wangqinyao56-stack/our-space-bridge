...
export async function clearWorld(pointsEarned) {
  ...
  const shopCatalog = SHOP_ITEMS.map(item => ({
    ...item,
    affordable: currentState.points >= item.cost,
  }));
...
function applyStatDrift(state) {
  ...
  if (phase === "boss_encounter") {
    ...
    const od = Math.floor(Math.random() * 5) - 2; // -2 to +2 (was -3 to +3)
    s.senseOverload = Math.max(0, Math.min(100, s.senseOverload + od));
    // Mental state gentle drain — reduced from -0~3 to -0~1
    p.mentalState = Math.max(10, p.mentalState - Math.floor(Math.random() * 2));
  }

  // ── Mental state recovery in safe phases ──
  if (phase === "init" || phase === "door_select" || phase === "cleared") {
    p.mentalState = Math.min(100, p.mentalState + Math.floor(Math.random() * 4) + 1); // +1~4
  }
