...
function parseReply(reply) {
  ...
  // Split by [旁白] markers. AI generates alternating blocks:
  // （action）"dialogue" [旁白] narration （action）"dialogue" [旁白] narration ...
  const segments = text.split(/\n?\[旁白\]\s*\n?/);
  // segments[0] = everything before first [旁白] (xiayan: actions + dialogue)
  // segments[1] = narration after first [旁白], until next [旁白] or end
  // segments[2] = xiayan after second [旁白] marker... etc.
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (!seg) continue;
    if (i % 2 === 0) {
      // Even index (0, 2, 4...): xiayan (was outside [旁白] blocks)
      xiayanParts.push(seg);
    } else {
      // Odd index (1, 3, 5...): narration (was inside [旁白] blocks)
      narrationParts.push(seg);
    }
  }

  narration = narrationParts.join("\n\n").trim();
  xiayan = xiayanParts.join("\n\n").trim();

  // Fallback: if no [旁白] markers found at all
  if (narrationParts.length === 0 && xiayanParts.length === 1) {
    // Single block with no markers — treat first paragraph as narration if it looks like prose
    const parts = xiayan.split(/\n\n+/);
  ...
  // Extract 【数值变化】 block from xiayan
  ...
  // Safety: strip any remaining 【标记】 blocks from narration and xiayan
  // (prevents marker text from leaking into displayed content)
