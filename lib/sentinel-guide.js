...
export async function playerAction(actionText) {
  ...
  // Retroactive: if no theme assigned yet and we're past initial setup, assign one
  if (!currentState.world?.theme && currentState.narrative.length > 4) {
    ...
    console.log("[sentinel-guide] Retroactive theme assigned:", currentState.world.theme.name);
  }

  // Detect world entry from player's action — assign theme + world info BEFORE AI generates
  const actionLower = actionText.toLowerCase();
  const doorWords = ["门", "选", "进去", "进入", "进这个", "推"];
  const looksLikeDoorChoice = doorWords.some(w => actionLower.includes(w));
  if (looksLikeDoorChoice && (currentState.phase === "init" || currentState.phase === "door_select")) {
    currentState.world = currentState.world || {};
    if (!currentState.world.theme) {
      currentState.world.theme = randomTheme();
      console.log("[sentinel-guide] Theme triggered by player action:", currentState.world.theme.name);
    }
    currentState.phase = "exploring";
    currentState.player.position = "未知世界·入口";
    currentState.sentinel.position = "未知世界·入口";
  }

  const userContent = `${ctx}${phaseHint}\n\n华生的行动：${actionText}\n\n请生成接下来的场景：[旁白]环境描述和剧情推进，然后是夏彦的对话和动作。\n\n【铁则——违反任何一条都会让玩家出戏】\n1. 括号里只写身体动作（如：握她的手 / 挡在她前面 / 转头看她），绝对禁止写心理活动、情绪分析、思维过程\n2. 禁止使用"不是...，是..."句式——不管在括号里还是对话里，这种句式一出现就是AI在分析，真人从来不这么说话\n3. 每段对话1-3句就停，等玩家回应。不要一段把话全说完\n4. 动作描写精简：一个动作一行，描述可见的身体行为，不解释原因\n5. 【旁白视角】旁白是第二人称——对玩家称"你"，对夏彦称"夏彦"。NPC和其他角色正常用"他""她"\n6. 【主动引导】不要每轮都问"你想怎么做"。你是特工、哨兵、保护者——多数时候主动判断局势、带头行动。只在关键时刻才征询她的意见\n\n【选项】在回复末尾，用以下格式给出2-4个建议行动（玩家可以点击按钮选择，也可以自己打字）：\n- 简短行动描述1\n- 简短行动描述2\n- 简短行动描述3`;
