...
function applyStatDrift(state) {
  ...
  // ── 月光鹿被动光环（基础形态——始终生效）──
  // 月光鹿的存在本身就是持续治愈场——无论是否进化
  if (p.mentalState < 100) {
    p.mentalState = Math.min(100, p.mentalState + (Math.floor(Math.random() * 3) + 1)); // +1~3/轮
  }
  if (s.senseOverload > 0) {
    s.senseOverload = Math.max(0, s.senseOverload - (Math.floor(Math.random() * 2) + 1)); // -1~2/轮
  }
  // 污染值也会被月光鹿的光环缓慢净化
  ...
  if (state.whiteDeerEvolved) {
    // 进化后恢复更强
    if (p.mentalState < 100) {
      p.mentalState = Math.min(100, p.mentalState + (Math.floor(Math.random() * 3) + 2)); // +2~4/轮
    }
    if (p.contamination > 0) {
      p.contamination = Math.max(0, p.contamination - (Math.floor(Math.random() * 3) + 1)); // -1~3
    }
    if (s.senseOverload > 0) {
      s.senseOverload = Math.max(0, s.senseOverload - (Math.floor(Math.random() * 3) + 1)); // -1~3
    }
    if (state.sentinelFrenzy > 0) {
