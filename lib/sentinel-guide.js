...
export async function playerAction(actionText) {
  ...
  const userContent = `${ctx}${phaseHint}\n\n阿鹿的行动：${actionText}\n\n【⚠ 下笔前先读这四条——违反任何一条都要重写】\n\n1. 【对话穿插——不许堆末尾】叙述和对话必须交替。写2-3句叙述→插1-2句夏彦对话→再切回叙述→再有对话。一段纯叙述后必须跟对话，一段对话后必须跟叙述。绝对禁止把全部对话集中输出在回复末尾。\n\n2. 【叫名字——别老用"他"】叙述中不要句句都用"他"开头。每段至少出现一次"夏彦"。连续3句用"他"开头就太多了——用"夏彦"换掉其中一个。好例子："夏彦的手已经过去了，两根手指搭在她颈侧。他另一只手从她后颈托上去。"——第一句用"夏彦"，第二句用"他"，交替着来。\n\n3. 【禁止写阿鹿】阿鹿是玩家。主语是"阿鹿"或"她"后面跟动词→删掉。夏彦问她问题→停在问号，不写她的回答/表情/动作。唯一例外：夏彦观察到的事（"她的呼吸变稳了"）。\n\n4. 【禁止"不是...是..."】永远不要写"不是A，是B"这种句式。想说B就直接说B。\n\n───\n\n【格式】纯散文。动作融入叙述："夏彦走到门边，手掌贴上铁门。" 对话用引号："这里不对。"\n【视角】"夏彦"或"他"=夏彦，"阿鹿"=华生。不用"你"。夏彦的感知传递一切。\n【长度】环境、感官、身体反应、动作、对话都要展开。不三两句收。\n【节奏】夏彦每次1-3句对话→切叙述→再有对话。每轮至少3-4轮交替。\n【内容】主动引导、NPC对话归叙述、每世界至少一队幸存者、向哨能力每轮体现。\n【数值】6-10轮推进BOSS，阿鹿精神力<40或污染>45加速。只在大事件输出【数值变化】。\n\n【系统通知】关键线索/关键地点/重大危险时插入【系统】块。4-6轮最多一次。\n\n【选项】末尾3个建议行动：\n【选项】\n- 选项1\n- 选项2\n- 选项3`;
...
function parseReply(reply) {
  ...
  // Step 2: Extract dialogue ( "..." ) as xiayan, rest as narration
  // Also handle legacy formats: [旁白] markers and （） parenthetical actions
  const dialogueParts = [];
  let cleanText = working;

  // Extract all "quoted dialogue" segments (handle both "" and Chinese ""）
  const DIALOGUE_RE = /"([^"]+)"/g;
  let m;
  while ((m = DIALOGUE_RE.exec(working)) !== null) {
    dialogueParts.push(m[1]);
  }
  // Also handle Chinese quotes
  const CN_DIALOGUE_RE = /“([^”]+)”/g;
  while ((m = CN_DIALOGUE_RE.exec(working)) !== null) {
    dialogueParts.push(m[1]);
  }

  // Remove dialogue and legacy markers from text to get clean narration
  cleanText = working
    .replace(/"([^"]+)"/g, "")       // remove "dialogue"
    .replace(/“[^”]+”/g, "") // remove Chinese quotes dialogue
    .replace(/\[旁白\]\s*/g, "")     // remove legacy [旁白] markers
    .replace(/（[^）]*）/g, "")       // remove legacy （） actions
    .replace(/\n{3,}/g, "\n\n")      // collapse excessive newlines
    .trim();

  narration = cleanText;
  xiayan = dialogueParts.map(d => `"${d}"`).join("\n");

  // If no dialogue found but text has legacy （） format, extract those as xiayan
  if (!xiayan && working.includes("（")) {
    const actionParts = [];
    const ACTION_RE = /（([^）]+)）/g;
    while ((m = ACTION_RE.exec(working)) !== null) {
      actionParts.push(`（${m[1]}）`);
    }
    if (actionParts.length > 0) {
      xiayan = actionParts.join("\n");
      // Re-clean narration without actions
      cleanText = working.replace(/（[^）]*）/g, "").replace(/\n{3,}/g, "\n\n").trim();
      narration = cleanText;
    }
  }
