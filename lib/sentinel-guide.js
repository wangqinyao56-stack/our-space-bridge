...
function updateStateFromReply(parsed) {
  ...
  let syncMatch = statText.match(/同步率[：:].*?→\s*(\d+)/);
  if (!syncMatch) syncMatch = statText.match(/同步率[：:]\s*(\d+)/);
  if (syncMatch) {
    currentState.sentinel.syncRate = parseInt(syncMatch[1]);
  }

  let overloadMatch = statText.match(/(?:感官)?过载[：:].*?→\s*(\d+)/);
  if (!overloadMatch) overloadMatch = statText.match(/(?:感官)?过载[：:]\s*(\d+)/);
  if (overloadMatch) {
    currentState.sentinel.senseOverload = parseInt(overloadMatch[1]);
  }

  let contamMatch = statText.match(/(?:精神)?污染[：:].*?→\s*(\d+)/);
  if (!contamMatch) contamMatch = statText.match(/(?:精神)?污染[：:]\s*(\d+)/);
  if (contamMatch) {
    currentState.player.contamination = parseInt(contamMatch[1]);
  }

  let mentalMatch = statText.match(/精神力[：:].*?→\s*(\d+)/);
  if (!mentalMatch) mentalMatch = statText.match(/精神力[：:]\s*(\d+)/);
  if (mentalMatch) {
    currentState.player.mentalState = parseInt(mentalMatch[1]);
  }

  // Detect 失控值 changes — ONLY from 【数值变化】 block
  let frenzyMatch = statText.match(/(?:失控值|哨兵失控)[：:].*?→\s*(\d+)/);
  if (!frenzyMatch) frenzyMatch = statText.match(/(?:失控值|哨兵失控)[：:]\s*(\d+)/);
  if (frenzyMatch) {
  ...
  // ── 自动结算检测 ──
  return detectAndApplySettlement(combined);
}

// 自动检测通关结算——AI在叙述中写了通关但服务端状态未更新
function detectAndApplySettlement(combined) {
  if (!currentState) return null;
  ...
  const settlePatterns = [
    "通关", "结算", "世界通关", "副本通关", "恭喜通关", "副本完成",
    "传送回安全屋", "回到安全屋", "安全屋的门", "回到那间安全屋",
    "世界已通关", "已通关", "通关奖励", "积分结算",
    "安全屋的灯光", "安全屋里", "推开安全屋",
  ];
  const hasSettlement = settlePatterns.some(p => combined.includes(p));

  // Also detect system notification with clearance
  const hasClearSysNotification = /恭喜通关|通关结算|副本.*完成|世界.*通关/.test(combined);

  if (!hasSettlement && !hasClearSysNotification) return null;

  // ── 二次确认：必须同时有"积分"或"安全屋"才触发，减少误判 ──
  const hasPointsRef = /积分|★|\+.*分|获得.*分/.test(combined);
  const hasSafehouse = /安全屋|传送回|回到了|门.*关/.test(combined);
  if (!hasPointsRef && !hasSafehouse) return null;
