...
import { summary as bodyStateSummary, deriveFrenzy } from "./body-state.js";
...
// ── 剧情骨架：每个副本一次性生成「真相+线索链」，让AI每轮有同一根主线可抓 ──
async function generateStoryFrame(worldName, terrorLevel, themeName) {
  const prompt = `为无限流副本世界「${worldName}」设计一份完整的剧情骨架。色调：${themeName || "未知"}，恐怖程度：${terrorLevel || "normal"}。

要求：
- 真相：这个世界的隐藏真相——玩家到来之前这里发生了什么、悲剧的根源是什么。一句话说清，要有恐怖感。
- boss起源：最终BOSS是怎么诞生的，和真相的因果关系。BOSS是悲剧的产物/守护者/受害者。
- 通关条件：玩家理解了真相之后要做什么才能通关（解除诅咒/释放灵魂/摧毁核心物/做出选择），不是简单"打败BOSS"。
- 线索链：4-5条线索，严格按逻辑顺序排列（线索A→B→C→D→真相）。每条线索是一样具体可发现的东西（一件物品/一个地点/一段NPC的话/一页日记/一个痕迹/一具尸体），不是抽象概念。每条线索要能自然引出下一条，最后一条指向真相。
- 关键地点：5-8个可探索区域，每个区域有独特的环境和氛围（不只是"挂线索的地点"，是能让人走进去、感受到氛围的地方）。其中3-5个区域挂主线线索（连成通关路线），其余是可自由探索的环境（可发现支线细节、氛围、道具）。世界要够大、可以到处走，但主线线索链保持清晰。

只输出JSON，不要markdown代码块，不要任何解释：
{"truth":"...","bossOrigin":"...","winCondition":"...","clues":[{"name":"...","detail":"..."}],"locations":[{"name":"...","clue":"..."}]}`;

  try {
    const reply = await askJiushi({
      model: "[k]claude-sonnet-4-5",
      systemPrompt: "你是无限流恐怖副本的编剧。只输出JSON。",
      userContent: prompt,
      maxTokens: 1600,
      temperature: 0.8,
      timeoutMs: 120000,
    ...
    if (!data.truth || !Array.isArray(data.clues) || data.clues.length === 0) {
      throw new Error("storyFrame 字段缺失");
    }
    return data;
  } catch (e) {
    console.error("[sg:frame] 剧情骨架生成失败:", e.message);
    return null;
  }
}
...
function createNewState(worldConfig) {
  ...
  return {
    ...
    // ── 已探索区域地图（连通的开放世界）──
    exploredWorlds: [],           // [{ name, theme, clearedAt }] 之前通关过的区域，地图上会标出来
    narrative: [],
    bossPleasureValue: 0,
    bossErosionValue: 0,
    // ── 月光神鹿 ──
...
function buildContextBlock(state) {
  ...
  if (state.world?.storyFrame) {
    const f = state.world.storyFrame;
    const revealed = state.world.cluesRevealed || 0;
    const total = (f.clues || []).length;
    ctx += `\n[剧情框架——本世界的真相与线索链（只有你知道，严禁直接向玩家剧透真相）]\n`;
    ctx += `真相：${f.truth || ""}\n`;
    ctx += `BOSS起源：${f.bossOrigin || ""}\n`;
    ctx += `通关条件：${f.winCondition || ""}\n`;
    ctx += `线索链（严格按顺序揭示，禁止跳）：\n`;
    (f.clues || []).forEach((c, i) => {
      const tag = i < revealed ? "已揭示" : "待揭示";
      ctx += `  ${i + 1}. ${c.name}【${tag}】${c.detail ? "—" + c.detail : ""}\n`;
    });
    if (revealed < total) {
      const next = f.clues[revealed];
      ctx += `本轮目标：沿着第${revealed + 1}条线索【${next?.name || ""}】推进，把玩家往真相再推一步。别另起炉灶，别跳去新线索。\n`;
    } else {
      ctx += `本轮目标：线索已揭示完——让真相浮出水面，引出BOSS。\n`;
    }
  }
  if (state.phase === "boss_encounter") {
    ctx += `[BOSS欢愉值] ${state.bossPleasureValue}%\n`;
  }
  ctx += bodyStateSummary(state);
  if (state.frenzyMode && state.frenzyMode !== "none") {
    ctx += `[失控模式] ${state.frenzyMode}\n`;
  }
  ...
  if (state.xiayanItems && state.xiayanItems.length > 0) {
    ...
    ctx += `【重要】以上是阿鹿给你的物品。你珍视它们——在合适的时机自然使用，而不是忘记它们的存在。\n`;
  }
  ...
  // ── 已探索区域地图（连通的开放世界）──
  if (state.exploredWorlds && state.exploredWorlds.length > 0) {
    ctx += `[已探索区域] 你们之前通关过的区域（互相连通，标在地图上）：\n`;
    for (const w of state.exploredWorlds) {
      ctx += `  - ${w.name}${w.theme ? `（${w.theme}）` : ""}\n`;
    }
    ctx += `【重要】新的区域和这些已探索区域在地理/生态上相连——选区域时，新区域应该自然地从这些区域的边缘延伸出去，而不是完全无关的孤立世界。\n`;
  }
  return ctx;
}
...
function buildHistoryForAI(narrative, maxTurns) {
  ...
  // Narration is plain prose now — no [旁白] markers, no separate xiayan.
  ...
  for (let i = 0; i < recent.length; i++) {
    ...
    if (entry.role === "player") {
      messages.push({ role: "user", content: `阿鹿的行动：${entry.content}` });
    } else if (entry.role === "narration") {
      messages.push({ role: "assistant", content: entry.content });
    } else if (entry.role === "xiayan") {
      // Legacy — skip empty xiayan entries, include non-empty ones
...
// ── 做爱/深度亲密场景判定 → 切企业按量强模型 ──
function detectIntimateScene(actionText) {
  if (!currentState) return false;
  const a = (actionText || "").toLowerCase();

  // 华生主动邀请/进行中的做爱关键词
  const intimateKw = /做爱|上床|要你|要我|进来|深入|结合|交合|缠绵|内射|高潮|骑上|坐上来|进去|发情|亲热|想要|吻遍|舔|口交/.test(a);

  // 夏彦失控值过高（安全屋）→ 迫切深度净化
  const frenzyUrgent = (currentState.sentinelFrenzy || 0) >= 50 && currentState.phase === "cleared";

  if (intimateKw || frenzyUrgent) {
    currentState.intimateActive = true;
  }

  // 结束做爱场景：明确结束词，或离开安全屋
  if (/睡了|休息|停下|够了|不要了|结束|别弄|不做了|起来|穿衣服/.test(a) || currentState.phase !== "cleared") {
    currentState.intimateActive = false;
  }

  return !!currentState.intimateActive;
}
...
初始场景。夏彦和阿鹿刚从一场无法解释的坠落中醒来——躺在陌生房间的地板上。上一秒还在未名市街上走，下一秒就在这里。

房间约二十平米。铁架床、木桌、两把折叠椅、暖黄色壁灯。深灰色墙壁，没有窗户。空气静止但能呼吸。墙角堆着纸箱补给。一扇厚重铁门，关着。

【写作视角——铁则】
全程夏彦第三人称有限视角。用"他"或"夏彦"称夏彦，用"阿鹿"称华生。绝不使用"你"来称呼阿鹿。所有信息通过夏彦的眼睛、耳朵、皮肤来感知——他看到什么、听到什么、闻到什么、摸到什么、身体有什么反应。

【剧情要求】
两人是完全的普通人——不知道"哨兵""向导""深渊""精神体"这些概念。夏彦的反应：
- 第一反应确认阿鹿安全——检查她有没有受伤
- 用特工本能观察房间——门、墙壁、光源
- 注意到自己感官变得异常——能听到她心跳、闻到空气里没有的味道——但不理解为什么
- 全程警戒和保护她

【绝对禁止】
- 禁止精神体/动物影子/虚影/任何超自然视觉
- 禁止【系统】出现
- 禁止"不是...，是..."句式
- 禁止心理活动/情绪分析写入动作描写
- 禁止碎片单字成段（"白。""静。""冷。"）
...
export async function playerAction(actionText) {
  ...
  if (currentState.phase === "init") {
    ...
    if (roundNum === 1) {
      phaseHint = `\n\n【第一轮——系统现身】\n\n夏彦和阿鹿在灰色安全屋醒来。两个普通人——被莫名其妙拖进了这个空间。阿鹿刚醒，说她头痛耳鸣。夏彦在帮她检查。\n\n【⚠ 格式铁则——系统对话必须用【系统】块】\n系统说的话单独成块，用【系统】包裹，与周围叙述空行隔开：\n\n德牧盯着屏幕，喉咙里发出很低的声音。\n\n【系统】\n啊。新来的。还活着呢——不错，至少比上一批开局就死了一个的强。\n\n光在屏幕上跳动了一下。夏彦把阿鹿往身后挡了挡。\n\n【系统】\n哦，已经开始了。护着她——很好。这种本能的保护欲待会儿会很好玩。\n\n───\n\n【⚠ 本轮只做这一件事】系统从屏幕上现身，介绍自己是谁、这里是Lustbound Abyss无限流世界（不同时空的人被拖进来、通关副本才能活下去）。系统语气：愉快的旁观者、嘴贱但不能说谎、享受看人挣扎。不要说哨兵向导、不要说精神体、不要觉醒。就聊。等阿鹿回应。\n\n【本轮禁止出现门和精神体】`;
    } else {
      phaseHint = `\n\n【系统对话中】系统已经自我介绍过了，也解释了这是Lustbound Abyss无限流世界。阿鹿可能在问问题，可能在和夏彦交流，也可能在跟系统说话。\n\n【系统的规则】\n- 不能说谎，规则问题必须准确回答。但不能透露副本具体内容、BOSS弱点、隐藏机制\n- 不能主动建议选哪个区域——只能说危险程度"偏高""一般""说不准"\n- 不能干预副本内的任何事。不能表露关心/担忧/恐惧——可以刻薄、不耐烦、嘲讽\n- 每次2-4句，不念说明书\n\n【系统对话必须用【系统】块包裹，与叙述空行隔开】\n\n【接下来的走向——由阿鹿的反应决定】\n- 如果阿鹿还在问系统问题→系统回答\n- 如果阿鹿说要开始/准备好了/门呢→系统说"终于。"然后1-2轮内展开区域地图\n- 如果阿鹿问为什么夏彦能听到她心跳/为什么自己头痛→系统可以提哨兵向导，做资质测试，觉醒精神体\n- 不要自己推进太快。等阿鹿的节奏。\n\n【重要】地图出现时用【系统】通知：\n【系统】\n区域通道已开启。地图上亮着的区域，选一个。祝你们死得愉快。`;
    }
    currentState._initRounds = roundNum + 1;
  } else if (currentState.phase === "door_select") {
    phaseHint = "\n\n区域地图已经在玩家面前。描述每个新区域的世界信息（和已探索区域怎么衔接），等待玩家选择。";
  ...
  } else if (currentState.phase === "cleared") {
    currentState._safehouseRounds = (currentState._safehouseRounds || 0) + 1;
    const shRounds = currentState._safehouseRounds;
    if (shRounds === 1) {
      phaseHint = `\n\n【⚠ 刚回安全屋（第一轮休息）】\n\n这一轮做三件休息的事（只做这一次，下轮别重复）：\n1. 两人坐下来整理战利品分类（华生用/夏彦用），边整理边聊起某件东西是怎么拿到的\n2. 复盘刚才的副本——聊具体的点（某个机关、某个NPC、某个差点出事的瞬间），别泛泛说"真险"\n3. 商城已弹出，夏彦提醒一句即可（不替阿鹿做购物选择）\n\n夏彦先关心阿鹿的状态（精神力/污染值），语气里带着"差点失去她"的后怕。\n\n【⚠ 强制——系统级信息（结算/积分/战利品）必须用【系统】块，禁止写进旁白叙述。`;
    } else if (shRounds === 2) {
      phaseHint = `\n\n【安全屋休息（第二轮）——别再重复结算流程】\n\n整理/复盘/商城上一轮已经做过了，这一轮不要从头再说一遍。\n\n跟着阿鹿当下的状态自然走：她想温存就温存，想聊副本里某个具体的点就聊那个点，想安静待着就安静待着。夏彦可以自然流露"差点失去她"的黏人和后怕。商城只在阿鹿主动提购物时才继续。`;
    } else {
      phaseHint = `\n\n【安全屋休息（第${shRounds}轮）——禁止原地打转】\n\n结算/整理/复盘/购物前面都做过了，绝对不要再重复提。\n\n- 跟着阿鹿这轮的动作和情绪走，她说啥你接啥，接完往前走一步，别把上一轮说过的话换个说法再说一遍\n- 温存/亲密是流动的，不是固定流程——她的反应变了你就跟着变，别每轮都从"抱一下"重新开始\n- 想看下一扇门就偶尔自然提一句，别每轮都催\n- 每轮都要有新的东西，禁止复读`;
    }
  }
  ...
  const userContent = `${ctx}${phaseHint}\n\n阿鹿的行动：${actionText}\n\n【输出格式——照这个写】\n\n✅ 正确（对话穿插在叙述里，不堆末尾）：\n\n夏彦撑起身体，手掌贴在地面上。冰凉的触感，水泥地。然后他听到了旁边的呼吸声。\n\n阿鹿就躺在他旁边不到半米的地方，侧着身，还没醒。夏彦立刻爬过去，扶住她肩膀。\n\n"阿鹿。"\n\n他手指在她肩膀上按了按——没有骨折的异常凸起，没有血迹。然后托住她后脑勺，指腹在头发里慢慢摸了一圈。\n\n"宝宝，能听到我说话吗？"\n\n她的呼吸是稳定的。夏彦低头看着她的脸，又轻轻拍了拍她脸颊。\n\n"醒醒。别怕，我在。"\n\n他抬起头，开始观察这个房间。\n\n───\n\n❌ 错误（对话堆在末尾——绝对禁止）：\n\n夏彦撑起身体，手掌贴在地面上。冰凉的触感，水泥地。阿鹿就躺在他旁边。他爬过去扶住她肩膀，检查了后脑勺，又拍了拍她脸颊。他叫了她的名字，告诉她别怕。\n\n"阿鹿。"\n"宝宝，能听到我说话吗？"\n"醒醒。"\n"别怕，我在。"\n\n───\n\n铁则：叙述和对话交替穿插。每段叙述后最多2句对话，然后必须切回叙述。禁止3句以上对话连在一起。禁止"不是...是..."。禁止写阿鹿的动作/回答。每段至少出现一次"夏彦"。\n\n【系统通知】关键线索/关键地点/重大危险时插入。4-6轮最多一次。\n\n【时间】每轮末尾输出游戏内时间与剩余时限（独立标记块，放在【选项】之前）：\n【时间】\n当前：XXX（游戏内时间，如"第三天·夜"或"农历七月十四·黄昏"）\n时限：剩余X天\n\n【选项】末尾3个建议行动：\n【选项】\n- 选项1\n- 选项2\n- 选项3`;
  ...
  if (currentState.phase === "exploring") {
    ...
    const cluesRevealed = currentState.world?.cluesRevealed || 0;
    const storyReady = cluesRevealed >= 3;

    if (rounds >= 12 || (rounds >= 6 && storyReady) || (rounds >= 5 && (mentalLow || contHigh)) || overloadHigh) {
      currentState.phase = "boss_encounter";
      currentState.bossPleasureValue = Math.max(currentState.bossPleasureValue || 0, 10); // 初始欢愉值
      const reason = rounds >= 12 ? "探索已深入" : storyReady ? "线索收束" : mentalLow ? "华生精神力濒危" : contHigh ? "污染值过高" : "夏彦感官严重过载";
      console.log(`[sg:boss] 强制触发BOSS (${reason}, round ${rounds})`);
      phaseHint = `\n\n【⚠ BOSS强制触发——${reason}】BOSS已出现。当前欢愉值:${currentState.bossPleasureValue}%。描述BOSS的出现方式和形态。BOSS对阿鹿产生了不可抗拒的兴趣——欢愉值开始积累。`;
    }
  } else if (currentState.phase !== "exploring") {
    ...
    // ── 做爱/深度亲密场景 → 切企业按量（强模型，描写更深）──
    const intimateScene = detectIntimateScene(actionText);
    const model = intimateScene ? "[企业按量]claude-opus-4-6" : "[k]claude-sonnet-4-5";
    ...
    // ── 更新时间/时限 ──
    if (parsed.timeLabel && currentState.world) currentState.world.timeLabel = parsed.timeLabel;
    if (parsed.remainingTime && currentState.world) currentState.world.remainingTime = parsed.remainingTime;

    // ── 线索揭示追踪：从【系统】通知里的🔑/关键线索/线索收束判断，递增进度 ──
    if (currentState.world?.storyFrame && parsed.systemNotification &&
        /🔑|关键线索|线索收束|重要线索|线索开始/.test(parsed.systemNotification)) {
      const total = (currentState.world.storyFrame.clues || []).length;
      currentState.world.cluesRevealed = Math.min((currentState.world.cluesRevealed || 0) + 1, total);
      console.log(`[sg:clue] 线索揭示 → ${currentState.world.cluesRevealed}/${total}`);
    }

    // Update state based on reply content
    const settlement = updateStateFromReply(parsed);

    // 欢愉值兜底：BOSS阶段若本轮没涨（AI没写更高的数字），代码自动涨，保证只增不减
    if (currentState.phase === "boss_encounter" && (currentState.bossPleasureValue || 0) <= (before.bossPleasureValue || 0)) {
      const base = 3 + Math.floor(Math.random() * 6); // +3~8
      currentState.bossPleasureValue = Math.min(100, (before.bossPleasureValue || 0) + base);
      console.log(`[sg:pleasure] 代码兜底涨欢愉值 → ${currentState.bossPleasureValue}%`);
    }
    ...
    // 用服务端净变化重建【数值变化】块——保证蓝块、右栏、左栏三者数值一致
    // （AI 写的 delta 会被月光鹿被动等自动机制偏移，不能直接用原始文本）
    const reconciledStatChanges = statUpdates.length > 0
      ? statUpdates.map(u => `${u.label}：${u.delta > 0 ? "+" : ""}${u.delta}`).join("\n")
      : "";
    ...
    // 结算后注入真实积分/副本数——保留 AI 的系统语气通知，只补真实数字
    let finalSystemNotification = parsed.systemNotification;
    if (settlement && settlement.settled) {
      const realInfo = `积分 +${settlement.pointsEarned}（当前 ★${settlement.totalPoints}）\n已通关 ${settlement.totalCleared} 个世界`;
      finalSystemNotification = (finalSystemNotification && finalSystemNotification.trim())
        ? `${finalSystemNotification.trim()}\n${realInfo}`
        : `恭喜通关。可惜——没有全部死光。\n${realInfo}`;
    }

    return {
      state: getPublicState(),
      reply: { narration: parsed.narration, xiayan: parsed.xiayan, systemNotification: finalSystemNotification, statChanges: reconciledStatChanges, statUpdates, choices: parsed.choices },
...
/**
 * Rewind to a specific turn and regenerate from there.
 * @param {number} turnIndex - 0-based index of the turn to regenerate from
 * @param {string} actionText - the player's original action text for that turn
 */
export async function rewindToTurn(turnIndex, actionText) {
  if (!currentState || currentState.status !== "active") {
    return { error: "No active game session" };
  }

  // Count player actions in the narrative to find the truncation point
  let playerActionCount = 0;
  let truncateAt = currentState.narrative.length;

  for (let i = 0; i < currentState.narrative.length; i++) {
    if (currentState.narrative[i].role === "player") {
      if (playerActionCount === turnIndex) {
        truncateAt = i;
        break;
      }
      playerActionCount++;
  ...
  if (truncateAt === currentState.narrative.length) {
    return { error: `Turn index ${turnIndex} out of range (found ${playerActionCount} player actions)` };
  }

  // Truncate narrative to just before the target turn
  const discarded = currentState.narrative.slice(truncateAt);
  currentState.narrative = currentState.narrative.slice(0, truncateAt);

  // ── 恢复 state 到副本内状态，避免 cleared + 副本中期叙述的混乱 ──
  if (currentState.phase === "cleared" || currentState.phase === "door_select") {
    currentState.phase = "exploring";
    currentState.bossPleasureValue = 0;
    currentState._exploreRounds = 0;
    console.log(`[sg:rewind] phase 恢复为 exploring（原 phase 为 cleared/door_select）`);
  }

  console.log(`[sg:rewind] 回退到第${turnIndex}轮，丢弃${discarded.length}条叙事（${discarded.filter(e => e.role === "player").length}个玩家行动）`);

  saveState();
  saveSession(currentState.sessionId);

  // Re-generate from this turn's action
  return await playerAction(actionText);
}
...
export async function generateDoors(terrorLevel) {
  ...
  const userContent = `${ctx}\n\n请在安全屋外展开一张区域地图。已探索过的区域标在地图上（暗色），相邻的未探索区域亮着。每个新区域需要包含：\n- 区域名称和地理方位（和已探索区域怎么衔接——在东边/南边/沿某条路/穿过某片荒原）\n- 区域的世界观和事件背景（2-3句）\n- 可能出现的NPC类型\n- 区域的风格/生态（中式/西式/荒野/都市废墟/未知异境等）\n- 恐怖程度保持为"${currentState.terrorLevel}"\n\n用自然散文描述地图展开的场景，夏彦用哨兵五感感知每个区域的气息。对话穿插在叙述中。`;
...
export async function selectWorld(worldDescription) {
  ...
  currentState.world.cluesRevealed = 0;

  // ── 生成剧情骨架（真相+线索链）——让这个世界有一条贯穿始终的主线 ──
  if (!currentState.world.storyFrame) {
    currentState.world.storyFrame = await generateStoryFrame(
      worldDescription,
      currentState.terrorLevel,
      currentState.world.theme?.name
    );
  }
  saveState();
  ...
  const ctx = buildContextBlock(currentState);
  const historyForAI = buildHistoryForAI(currentState.narrative, 10);
  const userContent = `${ctx}\n\n玩家选择了"${worldDescription}"这扇门，推门而入。\n\n【重要——你必须这样输出】推门进入新世界后，先用自然散文描述环境（用角色的眼睛看，不堆形容词），然后插入【系统】通知块：\n\n【系统】\n副本名称 + 世界观背景简介（2-3句） + 基础规则提示（如果有特殊规则的话）。系统语气恶劣但不失信息准确性。\n\n【铁则】\n- 画面感优先：不写"这个世界是一个XX风格的XX"，用角色的五感来感受——看到的、闻到的、听到的\n- 系统通知必须出现，不要跳过\n- 禁止大段景物描写——2-3句环境定调后立刻推进剧情\n- 【剧情衔接】本世界的真相和线索链已经在[剧情框架]里。你的开场描写要契合这个真相（场景细节可以暗示，但严禁直接说出真相或剧透）。开场只定调、不揭底。\n\n【时间】开场就定下这个世界的初始时间与时限（独立标记块，放在正文末尾）：\n【时间】\n当前：XXX（初始时间，如"农历七月十四·黄昏"或"第一夜·子时"）\n时限：剩余15天`;
...
export async function clearWorld(pointsEarned) {
  ...
  currentState._safehouseRounds = 0;
  currentState.points += pts;
  currentState.totalCleared += 1;
  currentState.bossPleasureValue = 0;

  // 记录已探索区域（连通的开放世界：通关的区域标在地图上，之后的世界和它们衔接）
  if (currentState.world?.name) {
    if (!currentState.exploredWorlds) currentState.exploredWorlds = [];
    if (!currentState.exploredWorlds.some(w => w.name === currentState.world.name)) {
      currentState.exploredWorlds.push({
        name: currentState.world.name,
        theme: currentState.world.theme?.name || "",
...
export async function continueToNext() {
  ...
  const oldExploredWorlds = currentState.exploredWorlds || [];
  ...
  currentState.exploredWorlds = oldExploredWorlds;
 ...
 */
/**
 * Buy an item and give it directly to 夏彦 (skips warehouse/inventory)
 */
export function buyForXiayan(shopItemId) {
  const bought = purchaseItem(shopItemId);
  if (bought.error) return bought;
  const withdrawn = withdrawFromWarehouse(bought.item.id);
  if (withdrawn.error) return withdrawn;
  return giveItemToXiayan(withdrawn.item.id);
}
...
/**
 * Restore a historical session as the active current state
 */
export function restoreSession(sessionId) {
  if (!sessionId) return { error: "Missing sessionId" };
  const file = path.join(SESSIONS_DIR, `${sessionId}.json`);
  if (!fs.existsSync(file)) return { error: "Session not found" };
  try {
    currentState = JSON.parse(fs.readFileSync(file, "utf-8"));
    currentState.status = "active";
    currentState.updatedAt = new Date().toISOString();
    saveState();
    saveSession(currentState.sessionId);
    console.log("[sentinel-guide] Restored session:", sessionId, "phase:", currentState.phase);
    return { state: getPublicState() };
  } catch (e) {
    return { error: "Failed to restore: " + e.message };
  }
}
...
function parseReply(reply) {
  ...
  const text = reply || "";
  console.log("[sg:parse] raw reply length:", text.length);

  let working = text;

  // ── Extract 【系统】 ──
  // System block is short (1-4 lines). End at blank line or next 【 marker.
  // Must NOT swallow narration that follows the system block.
  const sysIdx = working.indexOf("【系统】");
  if (sysIdx >= 0) {
    const before = working.slice(0, sysIdx);
    const afterSys = working.slice(sysIdx);
    // Match 【系统】 header + content until blank line or next marker
    const sysBlockMatch = afterSys.match(/【系统】\s*\n([\s\S]*?)(?=\n\s*\n|\n【|$)/);
    if (sysBlockMatch) {
      systemNotification = sysBlockMatch[0].replace(/^【系统】\s*\n?/, "").trim();
      // Put back: text before 【系统】 + text after system block
      working = (before + afterSys.slice(sysBlockMatch[0].length)).trim();
    }
  }
  ...
  let timeLabel = "";
  let remainingTime = "";
  const timeIdx = working.indexOf("【时间】");
  if (timeIdx >= 0) {
    const before = working.slice(0, timeIdx);
    const afterTime = working.slice(timeIdx);
    const timeBlockMatch = afterTime.match(/【时间】\s*\n([\s\S]*?)(?=\n\s*\n|\n【|$)/);
    if (timeBlockMatch) {
      const timeText = timeBlockMatch[0].replace(/^【时间】\s*\n?/, "").trim();
      const curMatch = timeText.match(/(?:当前|时间|现在)[:：]\s*(.+)/);
      const limMatch = timeText.match(/(?:时限|剩余|剩余时限)[:：]\s*(.+)/);
      if (curMatch) timeLabel = curMatch[1].trim();
      if (limMatch) remainingTime = limMatch[1].trim();
      working = (before + afterTime.slice(timeBlockMatch[0].length)).trim();
    }
  }

  // ── Extract 【数值变化】 ──
  const statIdx = working.indexOf("【数值变化】");
  if (statIdx >= 0) {
    const before = working.slice(0, statIdx);
    const afterStat = working.slice(statIdx);
    const statBlockMatch = afterStat.match(/【数值变化】\s*\n([\s\S]*?)(?=\n\s*\n|\n【|$)/);
    if (statBlockMatch) {
      statChanges = statBlockMatch[0].replace(/^【数值变化】\s*\n?/, "").trim();
      working = (before + afterStat.slice(statBlockMatch[0].length)).trim();
    }
  }

  // ── Extract 【选项】 ──
  let choices = [];
  const optIdx = working.indexOf("【选项】");
  if (optIdx >= 0) {
    const optText = working.slice(optIdx);
    const optMatch = optText.match(/【选项】\s*\n?([\s\S]*)/);
    if (optMatch) {
      working = working.slice(0, optIdx).trim();
      choices = optMatch[1].split("\n")
        .map(line => line.replace(/^[-•]\s*/, "").trim())
        .filter(c => c.length > 0 && c.length < 30);
    }
  }

  // ── Narration: keep full text with "..." dialogue inline ──
  ...
  // ── 后处理：剔除"不是...是..."模式 ──
  // 匹配：不是A，是B / 不是A，而是B / 并不是A，只是B / 没有A，就是B / 倒不是A，主要是B
  cleanText = cleanText.replace(
    /(?:并?不是|没有|倒不是)\s*[^，,。．\n]{1,30}[，,。．]\s*(?:而是|是|只是|就是|主要是)\s*/g,
    ""
  );
  // 也处理中间无逗号的变体：不是A而是B
  cleanText = cleanText.replace(
    /(?:并?不是|倒不是)\s*[^，,。．\n]{1,25}\s*(?:而是|是|只是|就是)\s*/g,
    ""
  );

  // ── 后处理：检测并剔除替阿鹿写的动作/回答 ──
  // 匹配段落中主语是"阿鹿"或"她"后跟动作动词的句子（在叙述中）
  // 注意：不能简单粗暴全删——需要通过上下文判断。这里做保守处理：
  // 只删除明确是阿鹿做动作/说话的模式，保留夏彦观察到的结果
  ...
  const filteredLines = lines.filter(line => {
    ...
    // 检测：以"阿鹿"或"她"开头 + 动作动词 — 这是替阿鹿写的
    const aLuAction = /^(?:阿鹿|她)\s*(?:点了点头|摇了摇头|笑了笑|皱了皱眉|开了口|说道|回答|回应|开口|眨了眨眼|瞪大了眼|后退|往前走|伸出手|抬起手|转身|回头|蹲下|站起来|坐|走|跑|推开|拉开门|选|选择|决定|心想|觉得|感觉)/.test(trimmed);
    if (aLuAction) {
      console.log("[sg:parse] 剔除替阿鹿写的动作:", trimmed.substring(0, 40));
      return false;
    }
    return true;
  });
  ...
  // Safety cleanup
  narration = narration.replace(/【选项】[\s\S]*$/g, "").replace(/【系统】[\s\S]*$/g, "").replace(/【数值变化】[\s\S]*$/g, "").trim();

  if (!narration && text.trim()) {
    narration = text.trim();
  }

  console.log("[sg:parse] result — narration:", narration.length, "chars, xiayan:", xiayan.length, "chars, choices:", choices.length);
  return { narration, xiayan, systemNotification, statChanges, choices, timeLabel, remainingTime };
}
...
function updateStateFromReply(parsed) {
  ...
  if ((combined.includes("区域地图") || combined.includes("地图展开") || combined.includes("地图浮现") || combined.includes("一个区域")) &&
      combined.includes("区域")) {
  ...
  const worldEntryPatterns = ["踏进", "走进", "进入", "踏入", "选了第", "选第", "先进去", "走向", "前往", "进入这个区域"];
  ...
  // Detect sync rate / mental state changes — ONLY from 【数值变化】 block
  // NOT from narration prose (AI may mention numbers loosely in narrative)
  const statText = (parsed.statChanges || "").toLowerCase();

  // ── 数值应用：优先提取 delta（"同步率：-7"），fallback 旧格式箭头（"同步率：75→68"）──
  // 关键：用 before+delta 计算 to，看板 delta 和正文一致——不再信 AI 编的 from/to 绝对值
  const clampStat = (v, min, max) => Math.max(min, Math.min(max, v));
  const applyStat = (labelRe, current, min, max) => {
    const deltaMatch = statText.match(new RegExp(labelRe + "[：:]\\s*([+-]\\d+)"));
    if (deltaMatch) return clampStat(current + parseInt(deltaMatch[1]), min, max);
    const toMatch = statText.match(new RegExp(labelRe + "[：:].*?→\\s*(\\d+)"));
    if (toMatch) return clampStat(parseInt(toMatch[1]), min, max);
    return null; // 无匹配，保持原值
  };

  const newSync = applyStat("同步率", currentState.sentinel.syncRate, 10, 100);
  if (newSync !== null) currentState.sentinel.syncRate = newSync;
  const newOverload = applyStat("(?:感官)?过载", currentState.sentinel.senseOverload, 0, 100);
  if (newOverload !== null) currentState.sentinel.senseOverload = newOverload;
  const newContam = applyStat("(?:精神)?污染", currentState.player.contamination, 0, 100);
  if (newContam !== null) currentState.player.contamination = newContam;
  const newMental = applyStat("精神力", currentState.player.mentalState, 10, 100);
  if (newMental !== null) currentState.player.mentalState = newMental;
  const newFrenzy = applyStat("(?:失控值|哨兵失控)", currentState.sentinelFrenzy || 0, 0, 100);
  if (newFrenzy !== null) currentState.sentinelFrenzy = newFrenzy;

  // Detect 欢愉值 changes — from stat block first, then prose (BOSS intro shows "欢愉值：0%")
  // 只升不降：AI 写回更低/重复的数字（比如每轮都写"0%"）时不倒退
  let pleasureMatch = statText.match(/欢愉值[：:]\s*(\d+)/);
  if (!pleasureMatch) pleasureMatch = combined.match(/欢愉值[：:]\s*(\d+)/);
  if (pleasureMatch) {
    currentState.bossPleasureValue = Math.max(currentState.bossPleasureValue || 0, parseInt(pleasureMatch[1]));
  }

  // Detect erosion value (侵蚀值) changes (from DeepSeek SFW prompt)
  let erosionMatch = statText.match(/侵蚀值[：:]\s*(\d+)/);
  if (!erosionMatch) erosionMatch = combined.match(/侵蚀值[：:]\s*(\d+)/);
  if (erosionMatch) {
  ...
  const aiProvidedStats = [newSync, newOverload, newContam, newMental, newFrenzy].some(v => v !== null);
  if (!aiProvidedStats) {
  ...
  // ── 自动结算检测 ──
  return detectAndApplySettlement(combined, parsed.systemNotification);
}

// 自动检测通关结算——AI在叙述中写了通关但服务端状态未更新
function detectAndApplySettlement(combined, systemNotification = "") {
  if (!currentState) return null;
  if (currentState.phase === "cleared") return null; // 已结算
  if (currentState.phase !== "exploring" && currentState.phase !== "boss_encounter") return null;

  const sysNotif = (systemNotification || "").toLowerCase();

  // ── 权威信号1：【系统】块明确写通关（prompt 强制要求结算信息走【系统】块）──
  const sysSettlement = /恭喜通关|通关结算|副本.*完成|世界.*通关|通关奖励|积分结算|通关.*个世界/.test(sysNotif);

  // ── 权威信号2：叙述里的"完成态"传送——带完成标记，避免"等回到安全屋"这种展望误判 ──
  const narrativeSettlement = /传送回安全屋|回到了安全屋|已回到安全屋|回到安全屋的门|安全屋的门.{0,8}(?:关|合|缓缓)|世界已通关|副本已通关|已通关|恭喜通关|通关奖励|积分结算/.test(combined);

  if (!sysSettlement && !narrativeSettlement) return null;

  // ── 执行结算 ──
  const pts = Math.floor(Math.random() * 50) + 30;
  currentState.phase = "cleared";
  currentState._safehouseRounds = 0;
  currentState.points += pts;
  currentState.totalCleared += 1;
  ...
  saveState();
  saveSession(currentState.sessionId);
  archiveSession(currentState.sessionId);

  const worldLoot = (currentState.inventory || []).filter(item => !item.fromWarehouse);
  const shopCatalog = generateDynamicShop();

  console.log(`[sg:auto-settle] 自动结算触发！+${pts}积分，总计${currentState.totalCleared}通关`);
...
function applyStatDrift(state) {
  ...
  if (phase === "boss_encounter") {
    // BOSS战：剧烈波动，总体下降
    const swing = Math.floor(Math.random() * 11) - 7; // -7 to +3
    s.syncRate = Math.max(10, Math.min(100, s.syncRate + swing));
    s.senseOverload = Math.min(100, s.senseOverload + 3 + Math.floor(Math.random() * 5)); // +3~7
    p.mentalState = Math.max(10, p.mentalState - (2 + Math.floor(Math.random() * 6))); // -2~7
    p.contamination = Math.min(100, p.contamination + 2 + Math.floor(Math.random() * 4)); // +2~5
  } else if (phase === "exploring") {
    // 探索：轻微积累
    s.syncRate = Math.max(10, Math.min(100, s.syncRate - (1 + Math.floor(Math.random() * 2)))); // -1~2
    s.senseOverload = Math.min(100, s.senseOverload + 1 + Math.floor(Math.random() * 3)); // +1~3
    // 精神力：每2轮消耗1次
    const exploreRounds = state._exploreRounds || 0;
    if (exploreRounds % 2 === 0) {
      p.mentalState = Math.max(10, p.mentalState - (1 + Math.floor(Math.random() * 2))); // -1~2
    }
    // 污染值：每3轮增长1次
    if (exploreRounds % 3 === 0) {
      p.contamination = Math.min(100, p.contamination + 1 + Math.floor(Math.random() * 2)); // +1~2
    }
  }

  // ── 安全阶段恢复 ──
  if (phase === "init" || phase === "door_select" || phase === "cleared") {
    p.mentalState = Math.min(100, p.mentalState + 5 + Math.floor(Math.random() * 6)); // +5~10
    s.senseOverload = Math.max(0, s.senseOverload - (3 + Math.floor(Math.random() * 5))); // -3~7
  }
  ...
  if (s.senseOverload > 60) {
    s.syncRate = Math.max(10, s.syncRate - (2 + Math.floor(Math.random() * 4))); // -2~5
  }

  // ── 失控值 tracking ──
  if (state.sentinelFrenzy === undefined) state.sentinelFrenzy = 0;
  if (s.senseOverload > 50) {
    state.sentinelFrenzy = Math.min(100, state.sentinelFrenzy + 1 + Math.floor(Math.random() * 3)); // +1~3
  } else if (s.senseOverload > 30) {
    state.sentinelFrenzy = Math.min(100, state.sentinelFrenzy + (Math.random() < 0.5 ? 1 : 0)); // 50%概率+1
  }
  if (phase === "boss_encounter") {
    state.sentinelFrenzy = Math.min(100, state.sentinelFrenzy + 1 + Math.floor(Math.random() * 3)); // +1~3
  }
  if (s.senseOverload <= 30 && phase !== "boss_encounter") {
    state.sentinelFrenzy = Math.max(0, state.sentinelFrenzy - (1 + Math.floor(Math.random() * 3))); // -1~3
  }
