...
function updateStateFromReply(parsed) {
  ...
  // Detect world entry — when player describes entering/pushing a door
  const worldEntryPatterns = ["推开门", "推门进入", "踏进", "走进", "进入", "跨过门槛", "穿过门", "选了第", "选第"];
  ...
  if (justEnteredWorld && (currentState.phase === "init" || currentState.phase === "door_select")) {
    ...
    // Try to extract world name from AI response (e.g., "中式老宅", "哥特城堡")
    const nameMatch = combined.match(/(?:进入|来到|踏入)(?:了)?(?:一座|一个|一栋|一处)?(.{2,6}?(?:古宅|城堡|医院|学校|小镇|村庄|森林|山洞|旅馆|公寓|工厂|教堂|墓地|废墟|雪山|荒原|矿井|灯塔|洋馆|庭院))/);
