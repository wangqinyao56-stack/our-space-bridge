...
/** 导入 txt：切段后从第 1 章开始读，批注由夏彦写 */
async function importBook(title, text) {
  const chunks = splitText(text);
  if (chunks.length === 0) return { state, error: "文件内容为空" };

  const book = { id: uuid().slice(0, 8), title: (title || "").trim() || "未命名", author: "", genre: "导入的书" };
  const chapter = 1;
  const annotation = await generateAnnotation(book, chunks[0]);
  ...
  state = {
    ...
    source: "txt",
    textChunks: chunks,
    passages: [{ id: uuid().slice(0, 8), chapter, text: chunks[0], annotation, timestamp: new Date().toISOString() }],
    discussions: [],
    updatedAt: new Date().toISOString(),
  };
  save();
...
export { getState, startReading, continueReading, discuss, pickBook, importBook };
