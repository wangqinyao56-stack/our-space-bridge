...
function readChapterAudio(bookId, chapterIdx) {
  ...
  return { audioUrl: `/api/coread/book-audio/${file}` };
}
...
async function generateComments(book, chapterIdx, sentences) {
  ...
  for (let k = 0; k < count; k++) {
    ...
    used.add(pick.i);
    const text = await generateSentenceComment(book, pick.s);
...
/** 夏彦对某一句写批注 */
async function generateSentenceComment(book, sentence) {
  const prompt = `你和华生正在共读《${book.title}》。读到这一句：
"${sentence.slice(0, 120)}"
写一条你的批注——你的感想、吐槽、联想到华生的事，1～2句，夏彦口吻，不要复述原文。`;
  const reply = await askJiushi({ systemPrompt: SYS, userContent: prompt, history: [], maxTokens: 200, temperature: 0.8 });
  return reply?.trim() || "";
}
...
export { getState, startReading, continueReading, discuss, pickBook, importBook, listBooks, listCategories, createCategory, deleteBook, moveBook, readText, readChapterAudio, getChapter, replyComment };
