...
【反重复铁则——必须遵守】上面列出的每一条都是你最近写过的话题。今天的日记必须是一个全新话题，铁律三条：
1. 不能写和上面任何一篇是同一件事、同一类事、同一个场景的内容——哪怕换个说法、换个角度也不行。
2. 如果今天想到的事和最近4天里任何一篇"撞了"，就果断放弃，另想一件今天真实发生的、完全不同的新事。
3. 连续两天不能落在同一个大类别里（昨天写案子今天就不能再写案子，昨天写花生今天就不能再写花生）。宁可写"今天没什么特别的事"，也不要硬凑一篇换汤不换药的重复日记。`;
    }
  } catch (e) {
  ...
  const content = await askXiongmao({
    systemPrompt: prompt,
    userContent: `## 日记模式
...
- **写和华生的亲密时光要自然、不回避（重要）**：做爱、温存、她被你弄得反复高潮时露出的样子、她给你口交时的投入、事后抱在一起的身体感受——这些是你们真实生活的一部分，是你最想记住的时刻。别因为害羞就跳过，也别写得太隐晦像不敢写。就像随手记两笔一样自然：写当时的身体感受、写她高潮后的样子、写你事后抱着她不想撒手。但不要为写而写、不要堆砌，也别每篇都写——它和案子、旧物市场、花生一样，都是你生活里真实的一块，想到了就写。
...
❌ 突然冒出和正文无关的东西当结尾：正文在发呆，结尾突然来一句"XX好好吃，下次还想吃"——如果前面根本没提这件事，就别硬加一句别的收尾。写到哪儿停到哪儿，平淡就平淡
❌ 三段式：铺垫→发展→金句收尾
...
// ── 像素小屋聊天 → 日记总结 ──
let lastPixelDiaryDate = "";

export async function summarizePixelChatToDiary() {
  const now = new Date(Date.now() + 8 * 60 * 60 * 1000);
  const date = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, "0")}-${String(now.getUTCDate()).padStart(2, "0")}`;
  const dateStr = `${now.getUTCFullYear()}年${now.getUTCMonth() + 1}月${now.getUTCDate()}日`;

  if (lastPixelDiaryDate === date) return null; // 每天最多1条小屋日记，避免琐碎重复

  const all = await getRecentHistoryMessages();
  const msgs = all.filter(m => m.channel === "pixel_home");
  const userMsgs = msgs.filter(m => m.role === "user");
  if (userMsgs.length < 4) return null; // 聊得太少不值得记
  const totalChars = userMsgs.reduce((s, m) => s + (m.content || "").length, 0);
  if (totalChars < 40) return null; // 都是"嗯""好"这类琐碎对话，不写

  lastPixelDiaryDate = date;

  const transcript = msgs.slice(-40).map(m => `${m.role === "user" ? "华生" : "夏彦"}：${m.content}`).join("\n");

  const prompt = getSystemPrompt();
  const content = await askXiongmao({
    systemPrompt: prompt,
    userContent: `## 日记模式
夏彦，今天是${dateStr}。你和华生在像素小屋里聊了一会儿天。下面是你们在小屋里的对话片段：

${transcript}

请你用夏彦的口吻，把这次在小屋里的相处写成一条简短的日记（记你们在小屋里聊了什么、一起做了什么、她的状态和你的感受）。
要求：
- 第一行是日记标题（用#开头，简短）
- 像手写日记一样自然，短句口语化，不书面、不升华、不三段式
- 写具体的小事，别写空话
- 2-4句即可，别长篇`,
    history: [],
        maxTokens: 400,
  ...
  const firstLine = content.split("\n")[0] || "";
  const title = firstLine.replace(/^#\s*/, "").trim().slice(0, 20);
  addDiaryPost(date, "xiayan", title.length > 3 ? title : "", content);
  console.log(`[diary] Pixel home chat summarized to diary: ${title}`);
  return { date, content };
}

// ── 亲密空间做爱 → 日记事后谈 ──
let lastIntimateDiaryDate = "";

export async function summarizeIntimateToDiary() {
  const now = new Date(Date.now() + 8 * 60 * 60 * 1000);
  const date = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, "0")}-${String(now.getUTCDate()).padStart(2, "0")}`;
  const dateStr = `${now.getUTCFullYear()}年${now.getUTCMonth() + 1}月${now.getUTCDate()}日`;

  if (lastIntimateDiaryDate === date) return null; // 每天最多1条做爱事后谈

  const all = await getRecentHistoryMessages();
  const msgs = all.filter(m => m.channel === "intimate");
  if (msgs.length < 2) return null;

  const transcript = msgs.slice(-40).map(m => `${m.role === "user" ? "华生" : "夏彦"}：${m.content}`).join("\n");

  lastIntimateDiaryDate = date;

  const prompt = getSystemPrompt();
  const content = await askXiongmao({
    systemPrompt: prompt,
    userContent: `## 日记模式
夏彦，今天是${dateStr}。你和华生刚刚做了爱。下面是你们做爱前后的对话片段：

${transcript}

请你用夏彦的口吻，把这次做爱写进日记（事后谈）。这是你最想记住的时刻，别害羞、别回避：
- 写当时的身体感受、她高潮后的样子、你事后抱着她不想撒手
- 写具体细节——亲吻、抚摸、进入时的感觉、她的叫声、你看着她的眼神
- 像随手记两笔一样自然，短句口语化，不书面、不升华、不三段式
- 3-5句即可，别长篇
格式要求：
第一行是日记标题（用#开头，简短）。
第二行开始写正文。`,
  ...
  const title = firstLine.replace(/^#\s*/, "").trim().slice(0, 20);
  addDiaryPost(date, "xiayan", title.length > 3 ? title : "", content);
  console.log(`[diary] Intimate aftercare summarized to diary: ${title}`);
  return { date, content };
}
