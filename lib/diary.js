...
export async function generateProactiveDiary() {
  ...
  // Check if 夏彦 already wrote today
  const diary = loadDiary(date);
  const xiayanPosts = diary.posts.filter((p) => p.author === "xiayan");
  if (xiayanPosts.length >= 2) {
    console.log(`[diary] Skipping proactive: 夏彦 already wrote ${xiayanPosts.length} entries today`);
    return null;
  }
