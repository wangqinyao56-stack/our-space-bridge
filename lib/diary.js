...
export function startProactiveDiary(callback, intervalHours = 6) {
  ...
  const MAX_PER_DAY = 2; // Max 2 diary entries per day
  const FORCE_AFTER_MS = 40 * 60 * 60 * 1000; // Guarantee: never silent more than ~40h
  ...
  const tryWrite = (forced) => {
    ...
    saveState();
    console.log(`[diary] Writing proactive diary${forced ? " (forced — 40h+ silent)" : ""}`);
  ...
  const check = () => {
    ...
    if (state.dailyCount < MAX_PER_DAY && (silentTooLong || Math.random() < 0.25)) {
      tryWrite(silentTooLong);
    } else {
      const reason = state.dailyCount >= MAX_PER_DAY ? "daily limit reached" : "skipped (25% chance)";
      console.log(`[diary] Skipping proactive diary this cycle (${reason})`);
    }
    const delay = (3 + Math.random() * 2) * 60 * 60 * 1000; // next check in 3-5h
    proactiveTimer = setTimeout(check, delay);
  };

  // First check 30-60 min after boot (was 2-4h — frequent redeploys kept resetting it
  // and the diary never got a chance to fire)
  const firstDelay = (30 + Math.random() * 30) * 60 * 1000;
  proactiveTimer = setTimeout(check, firstDelay);

  console.log(`[diary] Proactive diary enabled (max ${MAX_PER_DAY}/day, 25% chance, 40h force-write, first check in ${Math.round(firstDelay / 60000)} min)`);
}
