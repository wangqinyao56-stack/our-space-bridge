...
export function startProactiveDiary(callback, intervalHours = 6) {
  ...
  // Check every 3-5 hours, but with 25% chance + daily limit
  const check = () => {
    const now = new Date();
    ...
    const delay = (3 + Math.random() * 2) * 60 * 60 * 1000; // 3-5h
    proactiveTimer = setTimeout(() => {
      // Only write if under daily limit, with 25% chance each check
      if (dailyCount < MAX_PER_DAY && Math.random() < 0.25) {
        dailyCount++;
        generateProactiveDiary().catch((err) => {
          console.error("[diary] Proactive generation error:", err.message);
        });
      } else {
        const reason = dailyCount >= MAX_PER_DAY ? "daily limit reached" : "skipped (25% chance)";
  ...
  // First check after 2-4 hours
  const firstDelay = (2 + Math.random() * 2) * 60 * 60 * 1000;
  proactiveTimer = setTimeout(() => {
    if (dailyCount < MAX_PER_DAY && Math.random() < 0.25) {
      dailyCount++;
      generateProactiveDiary().catch((err) => {
  ...
  console.log(`[diary] Proactive diary enabled (max ${MAX_PER_DAY}/day, 25% chance, first check in ${Math.round(firstDelay / 3600000)}h)`);
}
