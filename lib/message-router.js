...
export async function handleTextMessage(text, isVoice = false, imageContext = null) {
  ...
  const timeTaggedChat = chatHistory.map(m => {
    if (!m.time) return m;
    const mTime = new Date(m.time + (m.time.includes("Z") || m.time.includes("+") ? "" : "+08:00"));
    const mBj = new Date(mTime.getTime()); // already adjusted
  ...
  history.push({
    role: "system",
    content: `【时间上下文】现在是${bjMonth}月${bjDay}日 ${String(bjHour).padStart(2, "0")}:${String(bjNow.getUTCMinutes()).padStart(2, "0")}。你看到的对话里如果标记了日期（如[6月23日]），那是过去的对话——不是今天刚发生的。不要直接接续昨天的话题说"刚才""等下""今天"——除非华生自己先提起。今天就是全新的一天。`,
  });
...
export async function handleIntimateMessage(text) {
  ...
  if (intimateMsgs.length > 0) {
    ...
    if (lastTs) {
      ...
      if (lastDay !== bjDay) {
        timeNote += `【强制】上一次亲密已经过去至少一天了（今天是${bjMonth}月${bjDay}日，上次是${new Date(lastTs).getMonth()+1}月${lastDay}日）。绝对禁止说"刚才""刚刚""刚才做完"之类的话。这是昨晚/之前的事，不是刚发生的。`;
      } else {
...
等晚上回家后才可以写肢体接触。`;
  ...
  console.log("[intimate] Calling askJiushi (宅恋 [0.04]报用鹿/claude-opus-4.5)...", { promptLen: prompt.length, historyLen: history.length, userContentLen: userContent.length });
  intimateLog({ stage: "calling_ai", model: "[0.04]报用鹿/claude-opus-4.5", provider: "zhailian", promptLen: prompt.length, historyLen: history.length });
  const t0 = Date.now();
  let reply;
  try {
