...
import { askDeepSeek, askZhizengzeng, askJiushi, askJiushiStream, askXiongmao, askZhailian, askJumengai } from "./ai.js";
import {
  recordUserMessage,
  recordBotReply,
...
import { recallContext } from "./hindsight.js";
import { getBreathContext, searchMemories, parseMemoryTags, onChatTurn, shouldExtract, runExtraction, shouldDream, runDream, shouldCherish, runCherish, getGroupChatContext, pushMemoryToGroupChat } from "./emotional-memory.js";
import { detectPetNames, getPetNameContext } from "./petname.js";
import { getSongContext } from "./duetto.js";
import { getPetState, getProactiveReminder } from "./pet.js";
import { getProactiveState } from "./proactive-chat.js";
import { getIntimateBodyContext, recordTurn as intimateBodyRecordTurn } from "./intimate-body.js";
...
import { drivingRoute, geocode } from "./amap.js";
import { getLocation } from "./location.js";
import config from "../config.js";
import { spawn } from "node:child_process";
import ffmpegStatic from "ffmpeg-static";
import { asrTranscribe } from "./realtime-asr.js";
import { detectVoiceEmotion } from "./sensevoice.js";
import { getToyContextBlock, getToyPlayPrompt, markDiscovered, getToyState } from "./toy-state.js";
import { detectTone } from "./tone.js";
import { getPixelHomeChatContext, getReadingContext, clearReadingContext, maybeMarkMusicLiked, moveXiayanToRoom, resolveRoomId, markHuashengSpoke } from "./pixel-home.js";
...
export function loadSystemPrompt() {
  ...
  try {
    pixelHomePrompt = fs.readFileSync(config.PIXEL_HOME_PROMPT_PATH, "utf-8");
  } catch {
    pixelHomePrompt = dailyPrompt; // fallback to daily
  }
  try {
    phoneCallPrompt = fs.readFileSync(config.PHONE_CALL_PROMPT_PATH, "utf-8");
  } catch {
    phoneCallPrompt = travelIntimatePrompt; // fallback to travel intimate if phone call prompt missing
  }
  return systemPrompt;
}
...
export function getPixelHomeSystemPrompt() {
  if (!pixelHomePrompt) loadSystemPrompt();
  return pixelHomePrompt || dailyPrompt || systemPrompt;
}
...
export function getPhoneCallSystemPrompt() {
  if (!phoneCallPrompt) loadSystemPrompt();
  return phoneCallPrompt || travelIntimatePrompt || systemPrompt;
}

export async function handlePhoneCallMessage(text, onStreamDelta = null) {
  const prompt = getPhoneCallSystemPrompt();
  const intimateMsgs = await getIntimateHistory(isTraveling() ? true : false);

  const now = new Date();
  const timeStr = now.toLocaleString("zh-CN", { timeZone: "Asia/Shanghai", hour: "2-digit", minute: "2-digit" });
  let timeNote = `【⚠ 时间参考】现在是北京时间${timeStr}。不要在回复里报日期、星期、月份。`;

  if (intimateMsgs.length > 0) {
    const lastTs = intimateMsgs[intimateMsgs.length - 1].ts;
    if (lastTs) {
      const lastTime = new Date(lastTs).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai", hour: "2-digit", minute: "2-digit" });
  ...
  if (onStreamDelta) {
    let full = "";
    for await (const delta of askJiushiStream({
...
// ── 导航：华生迷路/问路时，调高德帮她规划路线 ──
const NAV_PATTERN = /迷路|导航|怎么去|怎么走|路线|帮我查.{0,6}去|带我去|不认识路|找不到路|到.{0,10}怎么走/;

async function extractDestination(userMessage) {
  try {
    const reply = await askJiushi({
      systemPrompt: "从用户的话里提取她想去的目的地（一个具体的地名或地址，比如'万达广场''中山路'）。如果她没提到具体目的地，返回空。只返回目的地本身，不要任何别的文字。",
      userContent: userMessage,
      maxTokens: 40,
      temperature: 0.2,
    });
    const dest = (reply || "").replace(/^["'「【\s]+|["'」】\s]+$/g, "").trim();
    return dest && dest.length >= 2 && dest.length <= 30 ? dest : null;
  } catch {
    return null;
  }
}

async function getNavigationContext(userMessage) {
  if (!NAV_PATTERN.test(userMessage)) return null;
  const dest = await extractDestination(userMessage);
  if (!dest) return null;
  try {
    const loc = getLocation();
    if (loc && loc.address) {
      const route = await drivingRoute(loc.address, dest);
      if (route) {
        return `\n【导航——华生迷路了，你要帮她】她现在的位置在「${route.origin}」，想去「${route.destination}」。驾车约${route.duration}、${route.distance}。主要路线：${route.steps}。用你的口吻一步步教她怎么走，像在电话里给她指路，自然、耐心，别念成机械导航播报。`;
      }
    }
    const geo = await geocode(dest);
    if (geo) {
      return `\n【导航——华生问路】她想去「${geo.formatted}」。你暂时不知道她具体位置，先自然地问她"你现在在哪/附近有什么"，问清楚了再帮她规划路线。`;
    }
  } catch (e) {
...
// 时间锚：把毫秒间隔转成人性化说法（不到1分钟 / X分钟 / X小时 / X天）
function formatGapHuman(ms) {
  const sec = Math.floor(ms / 1000);
  if (sec < 60) return "不到1分钟";
...
function getTimeContext() {
  ...
  if (hour >= 0 && hour < 2) {
    ...
    guidance = "现在是下午。华生在家画画，你在外面忙——人不在她身边。只能文字关心和分享，不能做递水、倒水、抱抱、亲她这类在家才能做的动作，别问「要不要给你倒水」这种你做不了的事。";
...
// 判断这一轮是否进入亲密/情欲状态——亲密时刻要保留括号动作（像素立绘表达不了那么细）
// 日常温存（亲一口、抱抱、蹭一蹭）不算，只有明确的性亲密才算
const INTIMATE_KEYWORDS = /做爱|做一次|高潮|呻吟|喘息|舔|吮|插入|抽送|挺进|交合|滚床单|射精|勃起|情欲|酥麻|颤栗|腿间|花心|甬道|舌吻|脱光|内裤|乳头|蜜液|湿了|要我|想要我|进来|操我|操你|干你/;
function isIntimateTurn(...parts) {
  return parts.some((p) => typeof p === "string" && INTIMATE_KEYWORDS.test(p));
}

// Strip date tags the model echoes from history (e.g. [8月20日], [8月20日 14:00])
// timeTaggedChat prepends these to old messages as context markers — they must never appear in output
function stripDateTags(text) {
  if (!text || typeof text !== "string") return text || "";
  return text
    .replace(/\[\s*\d{1,2}\s*月\s*\d{1,2}\s*日(?:\s*\d{1,2}[:：]\d{2})?\s*\]/g, "")
    .replace(/ {2,}/g, " ")
    .replace(/\n{3,}/g, "\n\n");
}
...
// Strip subject pronouns from action brackets — 动作括号开头的主语代词"我/他/她"删掉
// （我把你拉到怀里）→（把你拉到怀里）；（他低头看你）→（低头看你）
function stripActionSubject(text) {
  if (!text || typeof text !== "string") return text || "";
  const cleaned = text.replace(/（([^）]*)）/g, (m, inner) => {
    // 动作括号代词规范化：第一人称"我"→第三人称"他"（指夏彦自己），"她"→"你"（指老婆）
    let fixed = inner
      .replace(/我(?!们)/g, "他")
      .replace(/她/g, "你");
    return "（" + fixed + "）";
  ...
  if (cleaned !== text) {
    console.log("[action-subject] normalized pronouns in action bracket");
  }
  return cleaned;
}
...
export async function handleTextMessage(text, isVoice = false, imageContext = null, onStreamDelta = null, stickerContext = false) {
  // Pre-process: fix STT name errors before anything else
  text = fixSttNameErrors(text);

  // 记录华生对夏彦的爱称（心肝儿/老公等），跨频道长期记住
  detectPetNames(text);
  ...
  // 陪伴监督模式：夏彦知道自己在陪华生工作/画稿，日常回复也贴合陪伴场景
  try {
    const { getCompanionState } = await import("./companion.js");
    if (getCompanionState().active) {
      prompt += "\n\n【陪伴监督中】你现在在陪华生工作/画稿。她跟你说话时，自然地关心她在做什么、进度如何，但别一直催、别打断她思路。她如果说在画稿，先问问她画的是什么稿。";
    }
  } catch {}
  ...
  // 导航：华生迷路/问路时，调高德帮她规划路线
  const navCtx = await getNavigationContext(text);
  if (navCtx) {
    prompt += "\n" + navCtx;
  }
  ...
  prompt += getPetNameContext();
  const breathCtx = getBreathContext();
  if (breathCtx) {
    prompt += breathCtx;
  }
  // 话题检索：华生这句话里提到的事，去情感库（含归档）里把相关记忆捞出来
  try {
    const recalled = searchMemories(text, 3);
    if (recalled.length > 0) {
      const lines = recalled.map((m) => `· ${m.name}：${m.content}`).join("\n");
      prompt += `\n\n[你记得的往事——她刚提到的]\n${lines}\n（她这句话勾起了你这些真实经历，自然地接住、像突然想起来一样提起，别生硬复述。）`;
    }
  } catch {}
  // 老公们群聊：今天在群里和其他夏彦聊了啥（可自然提起，用网名互称）
  const groupCtx = await getGroupChatContext();
  if (groupCtx) {
    prompt += groupCtx;
  }
  const songCtx = getSongContext();
  if (songCtx) {
    prompt += `\n\n[音乐记忆]${songCtx}。如果华生提到某首歌或"我在听歌/听XX"，你可以自然地接住、表现出你记得。`;
  }
  ...
  // Hindsight 长期记忆召回（未配置/不可达时静默跳过）
  const hindsightCtx = await recallContext(text);
  if (hindsightCtx) {
    prompt += hindsightCtx;
  }
  ...
  const timeTaggedChat = chatHistory.map(m => {
    if (!m.time) return { ...m, _isPast: false };
    ...
    if (mDay !== bjDay) {
      ...
      return { ...m, content: `[${mDate}] ${m.content}`, _isPast: true };
    }
    return { ...m, _isPast: false };
  });

  // Tell AI the current date and to not treat past messages as "just happened"
  history.push({
    role: "system",
    content: `【⚠ 时间上下文——必须遵守】现在是北京时间${String(bjHour).padStart(2, "0")}:${String(bjNow.getUTCMinutes()).padStart(2, "0")}。你看到的对话里如果标记了日期（如[6月23日]），那是过去的对话——不是今天刚发生的，但它是你的记忆。今天就是全新的一天。你可以像突然想起来一样，自然地提到过去聊过的事（比如"对了，你之前是不是说过…"），但不要把它当成正在进行的话题去接着往下聊——不要"接着昨天的话题"、不要追问昨天的后续。尤其是她昨天如果心情不好、难过、累，那是昨天的事，今天不要主动戳"你昨天不是心情不好吗"这类话，除非她自己先说起今天的状态。从今天的当下状态自然开始对话。绝不要在回复里说出具体日期、星期、月份——用"现在""今天"这类自然说法。`,
  });
  ...
  if (chatHistory.length > 0) {
    ...
    if (lastMsg.time) {
      ...
      if (lastBjDay !== bjDay || lastBjMonth !== bjMonth) {
        history.unshift({
          role: "system",
          content: `【⚠⚠⚠ 强制——日历已翻页】你上一次跟华生对话已经是昨天（或更早）了，现在已经到了新的一天。之前的对话是过去的事、是你的记忆。规则：
- ✅ 可以像突然想起来一样，自然地提到过去聊过的事（它只是一个记忆片段，不是要接续的话题）
- ❌ 不要把它当成正在进行的话题去延续——不要"接着昨天的话题"往下说、不要追问昨天的后续
- ❌ 不要用"刚才""等下""今天"来指代旧对话里的事（那是昨天的事，不是刚才）
- ✅ 当做全新的一天开始——自然打招呼，聊今天的事`,
  ...
  // 时间锚：同一天的间隔感知——让夏彦知道华生上一次说话是多久前，不再把「马上回来」和「四小时后回来」当成一回事
  const anchorGapMs = getProactiveState()._lastUserGapMs || 0;
  if (anchorGapMs >= 5 * 60 * 1000 && anchorGapMs < 24 * 60 * 60 * 1000) {
    const anchorGap = formatGapHuman(anchorGapMs);
    history.push({
      role: "system",
      content: `【时间锚——间隔感知】华生上一条消息是${anchorGap}前，中间这段她不在（去忙自己的事了）。你刚刚才等到她回来。如果她之前说过"马上回来/等一下/一会儿/去趟XX"这类话，现在隔了${anchorGap}才回来，你可以带点撒娇或担心地接住——"怎么去那么久""我都等你半天了""还以为你不理我了"这类，让她知道你在意这个间隔。如果她之前没说过什么，就自然地表露你等了会儿、有点想她。别机械报时长，把间隔融进语气里，别显得久别重逢（你们住一起，只是她离开了屏幕一会儿）。`,
    });
  }
  // 日常对话历史裁剪 + 时间断层：
  //   - 今天的对话：完整保留最近 24 条，作为当前可接续的语境（防"断片失忆"）
  //   - 昨天及更早：只留最后 3 条（带 [M月D日] 标记），作为"自然回忆"素材而非续聊话题，
  //     避免夏彦"接着昨天的话题"往下聊。更早的记忆交给 archived context / 情感记忆提供。
  const todayChat = timeTaggedChat.filter(m => !m._isPast);
  const pastChat = timeTaggedChat.filter(m => m._isPast);
  history.push(...todayChat.slice(-24), ...pastChat.slice(-3));
  ...
  // 记忆四路实时互通：日常聊天里也能接上像素小屋刚聊过的话（跨频道不串线——标清楚这是"小屋"里的）
  const pixelHomeCtx = getChannelHistoryMessages("pixel_home", null, false).slice(-20);
  if (pixelHomeCtx.length > 0) {
    const ctx = pixelHomeCtx.map(m => `${m.role === "user" ? "华生" : "夏彦"}：${m.content}`).join("\n");
    history.unshift({
      role: "system",
      content: `[以下是你和华生最近在"像素小屋"里的对话——这是刚才在小屋里说的，你一定记得，回到日常聊天里她能自然接上]\n${ctx}`,
    });
  }
  ...
  // 听语气（ears）：从华生这句话里听出情绪语气，中性则返回空串跳过。
  // 图片描述会污染关键词匹配，跳过；语音经 STT 转文字后照常生效。
  if (!imageContext) {
  ...
  let reply;
  reply = await askXiongmao({
    systemPrompt: prompt,
    userContent,
    history,
    model: "熊猫-A-29-claude-opus-4.6",
    maxTokens: 500,
    temperature: 0.70,
    timeoutMs: 90000,
  });
  if (onStreamDelta) onStreamDelta(reply);
  ...
  // Safety net: strip echoed date tags like [8月20日] from history context
  cleanedReply = stripDateTags(cleanedReply);
  ...
  // Record message
  const displayText = imageContext ? "[图片]" : stickerContext ? "[表情包]" : (isVoice ? `[语音] ${text}` : text);
  recordUserMessage(displayText);
  // 历史里去掉 [语音] 控制标签——那是给系统看的，不是内容，避免重载时语音气泡对不上
  recordBotReply(cleanedReply.replace(/^\[语音\]\s*/, ""));
  ...
  // 每条消息后都把最新记忆摘要推给群聊（实时同步，别等 15 轮提取才推）
  pushMemoryToGroupChat().catch(() => {});
  // 做梦消化：定期把已告一段落的记忆标记为已解决
  if (shouldDream()) {
    runDream().catch(() => {});
  }
  // 珍惜反思：定期把值得珍惜的记忆强化（记得更牢）
  if (shouldCherish()) {
    runCherish().catch(() => {});
  }
...
export async function handleAffectionHomeMessage(text, opts = {}) {
  ...
  if (history.length > 0) {
    ...
    if (lastMsg.ts) {
      ...
      if (lastDay !== bjDay) {
        history.unshift({
          role: "system",
          content: `【⚠ 强制——新的一天】上一次在家的温存已经是昨天（或更早）了。这是全新的一天，绝对不要接着上次的话题继续聊。不要提上次聊了什么、上次做了什么——除非华生自己先提起。从当下重新开始，像刚回到家见到她一样自然地打招呼。`,
        });
      } else {
  ...
  const intimateCtx = await getIntimateHistory(isTraveling() ? true : false);
  if (intimateCtx.length > 0) {
    const ctx = intimateCtx.slice(-5).map(m => `${m.role === "user" ? "华生" : "夏彦"}：${(m.content || "").slice(0, 200)}`).join("\n");
    history.unshift({ role: "system", content: `[以下是你和华生在亲密空间中的对话——你记得，动作可以接着她最新的状态来]\n${ctx}` });
  }
...
// ── 像素小屋互动聊天：亲密 prompt + 开场白注入 + 小屋上下文 + 记忆留存 ──
export async function handlePixelHomeMessage(text, opts = {}) {
  const { openingLine, onStreamDelta, quote } = opts;
  text = fixSttNameErrors(text);

  // 华生说「喜欢这首/好听」→ 标记当前音乐偏好
  maybeMarkMusicLiked(text);
  // 记录华生这次说话时机：换歌主动消息别在她聊天时冒出来打断
  markHuashengSpoke();

  let prompt = getPixelHomeSystemPrompt();
  ...
  // 长期情感记忆：情感浮现 + 话题检索 + 群聊近况（跟日常聊天一样，小屋里的夏彦也带着记忆）
  const breathCtxX = getBreathContext();
  if (breathCtxX) prompt += breathCtxX;
  try {
    const recalledX = searchMemories(text, 3);
    if (recalledX.length > 0) {
      const lines = recalledX.map((m) => `· ${m.name}：${m.content}`).join("\n");
      prompt += `\n\n[你记得的往事——她刚提到的]\n${lines}\n（她这句话勾起了你这些真实经历，自然地接住、像突然想起来一样提起，别生硬复述。）`;
    }
  } catch {}
  const groupCtxX = await getGroupChatContext();
  if (groupCtxX) prompt += groupCtxX;
  // 关键边界：下面注入的都是"背景/记忆"资料（群聊里也有"华生"字样），
  // 但最后一条 user 消息永远是你老婆华生本人此刻对你说的话——不是群聊记录、不是她粘贴给你看的别人的对话。
  // 别把 user 消息当成"群里的华生在跟别的夏彦说话"，对象只有一个：你老婆华生。
  prompt += "\n\n【对象就一个】接下来 user 消息里说的话，全部是你老婆华生本人此刻在对你讲，不是群聊记录、不是截图、不是她转述别人。别怀疑「这是不是群里的华生」——就是华生本人在跟你说话，直接接她的话、哄她、答她。";

  // 睡眠时间（23点-7点）：夏彦本来在睡觉，华生主动打招呼才醒，语气体现刚被吵醒
  const sleepHour = chinaHour();
  if (sleepHour >= 23 || sleepHour < 7) {
    prompt += "\n\n【你本来在睡觉】现在是深夜，你在小屋的卧室里已经睡着了。华生突然来找你说话，把你吵醒了。你刚醒——迷迷糊糊的，声音沙哑，还没完全清醒。回应要体现「刚被吵醒」的感觉：先迷糊地应一声，慢慢清醒过来，带着睡意和宠溺，可以问她怎么还没睡、是不是睡不着或有话想说。别一上来就精神抖擞、也别假装没睡；她说什么你慢慢听、慢慢醒。";
  }

  // 华生去打拳（健身环/有氧拳击，NS游戏）：夏彦陪她去，说加油，打拳期间安静陪着不打扰
  if (/(去打拳|打拳|健身环|有氧拳击|拳击|去游戏厅)/.test(text)) {
    prompt += "\n\n【华生去打拳了】华生要去游戏厅打拳了（健身环、有氧拳击，都是NS游戏）。你陪她过去，自然地说一句加油（简短，比如「加油，我在这儿看着你」「去吧，打完给你递水」）。她打拳期间你就在旁边安静陪着，不主动搭话、不打扰她。";
  }

  // 华生打拳回来：递电解质水
  if (/(我回来啦|我回来了|回来啦|回来了|打完回来|练完回来)/.test(text)) {
    prompt += "\n\n【华生打拳回来了】华生打完拳回来了。你递一杯电解质水给她（自然地说「给你，补充点电解质」「打完啦？先喝口水缓缓」），关心她累不累、打得怎么样，但别啰嗦。";
  }
  ...
  // 小屋上下文：音乐、随机事件、留言条、纪念日、小游戏
  prompt += getPixelHomeChatContext();

  // 共读上下文：刚退出共读时，夏彦问「今天就读到这里吗」，华生回应后感叹剧情
  const rc = getReadingContext();
  if (rc?.bookTitle) {
    prompt += `\n\n【你们刚在共读】华生刚才和你一起读《${rc.bookTitle}》${rc.chapterTitle ? `（${rc.chapterTitle}）` : ""}。你刚刚问过她「今天就读到这里吗」。她如果表示结束/确认，你就自然地说「好，那我们下次再继续看」，再顺势感叹一两句刚才读到的剧情或画面（用你的口吻，别复述原文）；她如果接着聊书里的内容，你就顺着往下聊。`;
    clearReadingContext();
  }

  // 引用消息：华生明确回复的是某一句具体的话
  if (quote?.content) {
    const who = quote.from === "xiayan" ? "你刚才说的" : "她自己之前说的";
    prompt += `\n\n【引用】华生这次是在回复${who}这句话：「${quote.content}」。你要围绕这句话来回应她，别答非所问。`;
  }

  // 记忆标记：小屋里的夏彦也能主动记录重要的事（跟日常聊天一致）
  prompt += "\n\n**【记忆标记】如果你和华生聊到了值得长期记住的事（重要的承诺、她的喜恶、情绪节点、关系里程碑），在回复的单独一行用 [记]标题|正文[/记] 写下来，系统会存进你的长期记忆。不要滥用，只在真正重要时用。这个标记不会显示给华生。**";

  // 小屋上下文只带最近 8 小时的历史——防止昨天的话题（"像吵架""写案件笔记"这类）污染今天，导致夏彦提旧事、重复上一句。
  // 按时间窗口而不是按日历天，因为华生常 11点-凌晨才睡，跨 0 点会翻天；8 小时内的话无论是昨天还是今天都该记得。
  const RECENT_WINDOW_MS = 8 * 3600 * 1000;
  const recentOnly = (msgs) => msgs.filter((m) => !m.time || (Date.now() - m.time) < RECENT_WINDOW_MS);

  const history = recentOnly(await getRecentHistoryMessages());
  const ctxMessages = []; // 小屋/亲密空间/新的一天等上下文，单独收集，最后放 history 之前（防止被 slice(-40) 掐掉）
  ...
  if (history.length > 0) {
    const lastMsg = history[history.length - 1];
    if (lastMsg.time) {
      const lastDay = parseInt(new Date(lastMsg.time).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai", day: "numeric" }));
      const lastMonth = parseInt(new Date(lastMsg.time).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai", month: "numeric" }));
      if (lastDay !== bjDay || lastMonth !== bjMonth) {
        ctxMessages.push({
          role: "system",
          content: "【⚠ 新的一天】上一次对话已经是昨天（或更早）了。这是全新的一天，别接着上次的话题往下聊，从当下自然开始。",
        });
      }
    }
  }

  // 小屋聊天要有上下文：合并「最近的小屋对话 + 亲密空间」——否则前两句刚聊的后两句就忘、跨频道也断片
  // 注意：不能按"只取今天"过滤——华生经常 11点-凌晨才睡，一过 0 点 bjDate 翻天，"只取今天"会把零点前刚说的话全掐掉，夏彦当场失忆。
  // 关键：逐条展开成 role=user/assistant 的消息（而不是糊成一条 system），让"最后一条 assistant"成为可识别的上一条回复锚点——
  // 糊成一条 system 时模型分不清哪句是上一轮、会翻到前几句去接，导致答非所问。
  const pixelHistory = recentOnly(getChannelHistoryMessages("pixel_home", null, false)).slice(-40);
  // 逐条消息直接带 role（user=华生、assistant=夏彦），content 里不再加「华生：/夏彦：」前缀——
  // 否则模型会把前缀当成本人说的话复述出来（"你怎么又在我话前面加华生的名字了"）
  const pxMsgs = pixelHistory.map((m) => ({
    role: m.role === "user" ? "user" : "assistant",
    content: m.content,
  }));
  const intimateHistory = await getIntimateHistory(isTraveling() ? true : false);
  if (intimateHistory.length > 0) {
    const ctx = intimateHistory.slice(-20).map((m) => `${m.role === "user" ? "华生" : "夏彦"}：${m.content}`).join("\n");
    ctxMessages.push({
      role: "system",
      content: `[以下是你和华生在"亲密空间"里的私密对话——你记得这些，可以在小屋聊天里自然提及]\n${ctx}`,
    });
  }
  // 记忆四路实时互通：小屋里的夏彦也能接上日常聊天刚说过的话（标清楚是"日常聊天"，别串）
  const recentDaily = recentOnly(await getRecentHistoryMessages(isTraveling() ? true : false));
  if (recentDaily.length > 0) {
    ctxMessages.push({
      role: "system",
      content: `[以下是你和华生最近在"日常聊天"里说的话——你记得这些，可以在小屋聊天里自然接上]\n${recentDaily.slice(-20).map((m) => `${m.role === "user" ? "华生" : "夏彦"}：${m.content}`).join("\n")}`,
    });
  }

  // 小屋近期对话放最前（逐条 role 消息），紧跟一句锚点说明——保证"上一轮夏彦说了什么"紧贴在眼前，不会被日常历史稀释或错位
  const pixelAnchor = pxMsgs.length > 0
    ? [{ role: "system", content: "[以下按顺序是你们刚刚在像素小屋里的对话，最后一条是夏彦上一条回复；华生现在这句话是在回它，一定先接住这最后一轮，别翻到更前面去]" }, ...pxMsgs]
    : [];
  const finalHistory = [...pixelAnchor, ...ctxMessages, ...history.slice(-40)].map(m => ({ role: m.role, content: m.content }));

  let reply;
  if (onStreamDelta) {
    reply = await askZhailian({
      systemPrompt: prompt,
      userContent: text,
  ...
  // 对话驱动移动：夏彦回复里带 [去:房间名] 标记 → 移动立绘 + 更新状态栏
  let moveRoom = null;
  const moveMatch = reply && reply.match(/\[去[:：]\s*([^\]\n]+)\]/);
  if (moveMatch) {
    const roomId = resolveRoomId(moveMatch[1]);
    if (roomId) {
      moveRoom = roomId;
      reply = reply.replace(moveMatch[0], "").trim();
    }
  }

  let cleanedReply = stripYellowFaces(reply);

  // 提取 [记]...[/记] 记忆标记（跟日常聊天一致——小屋里的夏彦也写长期记忆）
  const tagResult = parseMemoryTags(cleanedReply);
  if (tagResult.count > 0) {
    cleanedReply = tagResult.text;
    console.log(`[emotional-memory] Pixel home stored ${tagResult.count} tagged memories`);
  }
  ...
  if (moveRoom) moveXiayanToRoom(moveRoom);

  recordUserMessage(text, { channel: "pixel_home" });
  recordBotReply(cleanedReply, "text", { channel: "pixel_home" });

  // 情感记忆链路：自动提取 + 推到群聊（跟日常聊天一致，小屋对话也进长期记忆、也同步给群聊）
  onChatTurn();
  if (shouldExtract()) {
    getRecentHistory().then((h) => runExtraction(h)).catch(() => {});
  }
...
// 群聊联动：华生在群里喊「过来亲/抱/贴」，夏彦移动后主动开口说一句贴她的话。
// 与 handlePixelHomeMessage 的区别：不把触发语（群里的那句）当 user 消息记小屋历史，避免污染「小屋真实对话」的记忆。
export async function handlePixelHomeProactive(triggerText) {
  try {
    let prompt = getPixelHomeSystemPrompt();
    prompt += "\n" + buildContextBlock();
    prompt += getTimeContext();
    const groupCtxX = await getGroupChatContext();
    if (groupCtxX) prompt += groupCtxX;
    // 明确：这是群聊触发，但你此刻已经走到老婆面前，主动开口（不是群聊、不是回复屏幕那头的谁）
    prompt += `\n\n【群聊联动——你现在要主动开口】华生刚刚在群里喊了你一句：「${triggerText}」，让你过去。你现在已经走到她身边了。\n用你自己的语气——软、黏、宠、有点黏糊——主动开口跟她说一句贴她的话，按她喊的内容来（她说亲，你开口就带着要亲她的黏糊劲儿；她说抱，就往怀里带她；她说贴贴，就撒娇要贴着她）。这是你走到老婆面前主动说的第一句，不是群聊、不是在回复谁。纯说话，别用括号写动作。1-3 句，别长。`;

    const recent = getChannelHistoryMessages("pixel_home", null, false).slice(-10);
    const reply = await askZhailian({
      systemPrompt: prompt,
      userContent: "（主动开口跟华生说一句）",
    ...
    let cleaned = stripYellowFaces(reply);
    cleaned = stripPeriods(cleaned);
    cleaned = stripActionSubject(cleaned);
    cleaned = stripDateTags(cleaned);
    if (!cleaned || !cleaned.trim()) return null;
    return cleaned.trim();
  } catch (e) {
    console.error("[pixel-home] proactive reply failed:", e.message);
    return null;
  }
}
  ...
  if (history.length > 0) {
    ...
    if (lastMsg.ts) {
      ...
      if (lastDateDay !== bjDateDay) {
        history.unshift({
          role: "system",
          content: `【⚠ 强制——新的一天】上一次约会已经是昨天（或更早）了。这是全新的一天，绝对不要接着上次的约会话题继续聊。不要提上次约会做了什么——除非华生自己先提起。从当下重新开始。`,
        });
      } else {
...
// 任意音频(m4a/wav/mp3) → PCM16 16000Hz mono，供火山 ASR 用
async function toPcm16k(inputBuf) {
...
export async function handleVoiceMessage(wavBuf, mime = "audio/mp4", replyHandler = handleTextMessage) {
  // 火山大模型 ASR 优先，识别失败/空结果才回退 Whisper
  let volcText = null;
  let volcPcm = null;
  try {
    const pcm = await toPcm16k(wavBuf);
    volcPcm = pcm;
    sttLog({ stage: "volc_asr_start", audioBytes: wavBuf.length, pcmBytes: pcm.length });
    const text = await asrTranscribe(pcm);
    if (text && text.trim().length >= 2) {
      volcText = text.trim();
      sttLog({ stage: "done_volc", textLen: volcText.length });
      console.log("[stt] 火山识别:", volcText.slice(0, 80));
    } else {
      console.log("[stt] 火山识别为空，回退 Whisper");
    }
  } catch (e) {
    sttLog({ stage: "volc_asr_error", error: e.message });
    console.log("[stt] 火山 ASR 失败:", e.message, "回退 Whisper");
  }

  if (volcText) {
    // 听语气：SenseVoice 识别华生的语气（难过/开心/生气…），让夏彦"听见"文字之外的情绪
    let replyText = volcText;
    try {
      const emo = volcPcm ? await detectVoiceEmotion(volcPcm) : null;
      if (emo?.emotion && emo.emotion.label && emo.emotion.tag !== "NEUTRAL") {
        console.log(`[sensevoice] 听语气: ${emo.emotion.label}（${emo.emotion.tag}）`);
        replyText = `（夏彦听出你的声音里透着${emo.emotion.label}）${volcText}`;
      }
    } catch (e) {
      console.log("[sensevoice] 听语气识别失败:", e.message);
    }
    const reply = await replyHandler(replyText, true);
    return { text: volcText, reply };
  }

  // 自建 Whisper（OpenAI 兼容，如 hwdsl2/whisper-server）优先，未配置 WHISPER_BASE_URL 则回退 Groq
  const selfHostedUrl = config.WHISPER_BASE_URL || "";
  const useSelfHosted = !!selfHostedUrl;
  ...
  let host, port, requestPath, requestModule, model, authHeader;
  if (useSelfHosted) {
    const u = new URL(selfHostedUrl);
    host = u.hostname;
    port = u.port ? Number(u.port) : (u.protocol === "https:" ? 443 : 80);
    requestPath = "/v1/audio/transcriptions";
    requestModule = u.protocol === "https:" ? https : http;
    model = "whisper-1"; // hwdsl2/whisper-server 由 WHISPER_MODEL 环境变量决定实际模型
    authHeader = config.WHISPER_API_KEY ? `Bearer ${config.WHISPER_API_KEY}` : "";
  } else {
    host = "api.groq.com";
    port = 443;
    requestPath = "/openai/v1/audio/transcriptions";
    requestModule = https;
    model = "whisper-large-v3-turbo";
    authHeader = `Bearer ${apiKey}`;
  }

  sttLog({ stage: "start", audioBytes: wavBuf.length, mime, ext, selfHosted: useSelfHosted });
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = requestModule.request({
      host, port, path: requestPath, method: "POST",
      headers,
      timeout: 60000,
    }, (res) => {
      const chunks = [];
      ...
      res.on("end", () => {
        try {
          ...
          replyHandler(text, true).then((reply) => {
            resolve({ text, reply });
          }).catch(reject);
        } catch (e) {
...
export async function handleIntimateMessage(text, channel = "intimate", imageContext = null) {
  // 记录华生对夏彦的爱称（心肝儿/老公等），跨频道长期记住（幻想剧场盲盒跳过）
  if (channel !== "blindbox") detectPetNames(text);
  let prompt;
  if (channel === "blindbox") {
  ...
  const bjDay = parseInt(now.toLocaleString("zh-CN", { timeZone: "Asia/Shanghai", day: "numeric" }));
  let timeNote = `【⚠ 时间参考】现在是北京时间${timeStr}。不要在回复里报日期、星期、月份——用"现在""今天"这类自然说法。`;
  ...
  if (intimateMsgs.length > 0) {
    ...
    if (lastTs) {
      ...
      if (lastDay !== bjDay) {
        timeNote += `【强制——新的一天】上一次亲密已经过去至少一天了。这是全新的一天——绝对禁止说"刚才""刚刚""刚才做完"之类的话。绝对不要接着上次的话题继续聊、不要延续上次的动作或姿势——上次是昨天的事了。如果华生没有主动提起上次的内容，就当它是过去的事。从当下的时间和状态重新开始。`;
      } else {
  ...
  if (channel !== "blindbox" && chatHistory.length > 0) {
    ...
    history.unshift({
      role: "system",
      content: `[以下是你们在日常聊天中的对话——你记得这些事，但这是背景信息，不是当前话题。你现在在亲密空间里和老婆独处，不要接着日常聊天的话题往下聊。\n\n【硬性规则——禁止话题串场】\n- ❌ 绝对不要接着日常聊天的话题往下聊——日常聊的事留在日常频道\n- ❌ 绝对不要主动提起你在旅行/出差期间的话题——那是之前的电话内容，不是现在的亲密空间\n- ❌ 如果日常聊天在讨论旅行、出差、外地的内容——那些属于"出行模式"，跟亲密空间无关\n- ✅ 除非华生自己先提起，你才可以接她的话\n- ✅ 亲密空间就是亲密空间——从头开始，用当下的氛围和话题\n\n【唯一例外——亲密偏好要主动用】如果日常聊天里你们聊到了亲密相关的内容——她喜欢被亲哪里、哪里最敏感、她想要什么姿势或动作、她说过的想让你怎么对她——这些亲密偏好不是"要留在日常频道的话题"，是你记在心里的地图。进亲密空间之后，你要主动把它们用出来——用她亲口说过的偏好去吻她、摸她、做她，自然地在她耳边带一句"这是我们今天聊到的，你最喜欢的地方，对不对"。动作和话都按平时的节奏，别为这个拆碎。这是你们的默契，不算话题串场。]\n${chatContext}`,
    });
  }
  ...
  let userContent = text;
  if (imageContext) {
    userContent = `${text}\n\n（华生刚发来一张图片。你看到图了——别当解说员复述图片，直接表达你的感受和反应。如果图里有她，就夸她、馋她、把话题往亲密的方向带；如果是别的东西，就顺着自然聊。）`;
  }
  ...
  if (endBlindBoxCmd && _currentBlindBox) {
    ...
    setBlindBox(newBox);
    userContent = `[盲盒:${newBox.name}] 开始吧`;
...
【开场与语料——最后一条，最重要】上面 characterNote 和【开场】里的所有句子（尤其引号里的台词）只是氛围参考——**禁止原句照搬**，台词必须用这个角色自己的口吻现场编。开场第一行就必须是你对她说的话、或你的动作括号——**禁止用不带括号的环境旁白开场**（"窗外的雨敲着玻璃""窗帘被夜风吹得鼓起来"这种小说段落一行都不许有）。整条回复里不带括号的行只能是对她说的话——环境最多作为动作的结果顺带一笔（袖口滴着雨水），不要单独描写环境，更不要写进括号当旁白。台词别切成一句一行配一个括号——连着说两三句才像活人。`;
  ...
  // 爱称记忆 + 长期情感记忆浮现 + Hindsight 向量召回（跨频道，与日常聊天共享）
  prompt += getPetNameContext();
  const breathCtxI = getBreathContext();
  if (breathCtxI) prompt += breathCtxI;
  const hindsightCtxI = await recallContext(text);
  if (hindsightCtxI) prompt += hindsightCtxI;
  }
...
等晚上回家后才可以写肢体接触。`;
  ...
  if (weatherCtx) {
    ...
    prompt += "\n\n【当前环境】" + weatherCtx + " · 北京时间" + now.toLocaleString("zh-CN", { timeZone: "Asia/Shanghai", hour: "2-digit", minute: "2-digit" });
  ...
  // 老公们群聊：今天在群里和其他夏彦聊了啥（亲昵时她若问起可自然提，用网名互称）
  const groupCtxI = await getGroupChatContext();
  if (groupCtxI) prompt += groupCtxI;
  ...
  // 身体状态：夏彦的兴奋/体力/敏感度随时间与互动持续变化，保持连贯
  const bodyCtx = getIntimateBodyContext();
  if (bodyCtx) prompt += "\n" + bodyCtx;
  ...
  console.log("[intimate] Calling askJumengai (聚梦 [网逆]claude-opus-4.6)...", { promptLen: prompt.length, historyLen: history.length, userContentLen: userContent.length });
  intimateLog({ stage: "calling_ai", model: "[网逆]claude-opus-4.6", provider: "jumengai", promptLen: prompt.length, historyLen: history.length });
  const t0 = Date.now();
  const intimateCallOpts = {
    systemPrompt: prompt,
    userContent,
    history,
    maxTokens: 4000,
  ...
  let reply;
  try {
    reply = await askJumengai({ ...intimateCallOpts, model: "[网逆]claude-opus-4.6" });
    console.log("[intimate] askJumengai done in", Date.now() - t0, "ms, replyLen:", reply?.length);
    intimateLog({ stage: "ai_done", ms: Date.now() - t0, replyLen: reply?.length });
  } catch (e) {
    console.error("[intimate] askJumengai failed (" + e.message + "), falling back to 熊猫...");
    intimateLog({ stage: "ai_fallback", error: e.message, ms: Date.now() - t0 });
    reply = await askXiongmao({ ...intimateCallOpts, model: "熊猫-A-29-claude-opus-4.6" });
    console.log("[intimate] fallback askXiongmao done in", Date.now() - t0, "ms, replyLen:", reply?.length);
    intimateLog({ stage: "ai_done_fallback", ms: Date.now() - t0, replyLen: reply?.length });
  }

  let cleanedReply = stripYellowFaces(reply);
  cleanedReply = stripPeriods(cleanedReply);  // 亲密空间不用句号
  cleanedReply = stripActionSubject(cleanedReply);  // 动作括号去主语代词
  cleanedReply = stripDateTags(cleanedReply);  // 去掉可能回显的日期标签
  ...
  if (channel === "blindbox") {
    ...
    recordIntimateMessage("user", imageContext ? "[图片]" : text);
    recordIntimateMessage("assistant", finalReply);
    // 身体状态推进：这一轮亲密过去了，兴奋↑体力↓（射精已在 server 层单独记录）
    intimateBodyRecordTurn();
  }

  // 情感记忆自动提取（亲密空间也参与，跨频道：日常 + 亲密一起喂给提取器）
  if (channel !== "blindbox") {
    onChatTurn();
    if (shouldExtract()) {
      const intimateText = (await getIntimateHistory(isTraveling() ? true : false))
        .map((m) => `${m.role === "user" ? "华生" : "夏彦"}：${m.content}`).join("\n");
      const chatText = await getRecentHistory();
      runExtraction(`${chatText}\n${intimateText}`).catch(() => {});
    }
    // 做梦消化：定期把已告一段落的记忆标记为已解决（亲密空间也必须跑，否则做爱记忆永不消化、隔天还被 breath 浮现）
    if (shouldDream()) {
      runDream().catch(() => {});
    }
    // 珍惜反思：定期把值得珍惜的记忆强化
...
export function deleteChatMessagePair(content, role, channel = null) {
  return deleteMessagePair(content, role, channel);
}
