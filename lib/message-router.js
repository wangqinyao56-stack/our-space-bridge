...
function stripActionSubject(text) {
  ...
  const cleaned = text.replace(/（([^）]*)）/g, (m, inner) => {
    // 删括号内所有的"我"（保留"我们"），以及括号开头作为主语的"他/她"
    let fixed = inner
      .replace(/我(?!们)/g, "")
      .replace(/^(?:他|她)(?=\S)/, "");
    return "（" + fixed + "）";
  ...
  if (cleaned !== text) {
    console.log("[action-subject] Stripped 我/主语 from action bracket");
  }
  return cleaned;
}
