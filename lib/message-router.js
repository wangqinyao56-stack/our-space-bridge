...
export async function handlePixelHomeMessage(text, opts = {}) {
  ...
  if (pixelHistory.length > 0) {
    ctxMessages.push({
      role: "system",
      content: `[以下是你和华生最近在"像素小屋"里的对话——这是刚刚发生的，一定要接着往下聊，她刚才说了什么你都要记得]\n${pixelHistory.map((m) => `${m.role === "user" ? "华生" : "夏彦"}：${m.content}`).join("\n")}`,
    });
  }
  const intimateHistory = await getIntimateHistory(isTraveling() ? true : false);
  if (intimateHistory.length > 0) {
  ...
  const finalHistory = [...ctxMessages, ...history.slice(-40)].map(m => ({ role: m.role, content: m.content }));
