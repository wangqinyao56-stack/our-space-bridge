...
import { getChatReminder, autoCompleteRandom, getDoneNotification } from "./todo.js";
...
function stripNameCorrection(reply) {
  ...
  // Remove any sentence that mentions 夏彦's name being pronounced wrong, STT errors, or homophone confusion
  const patterns = [
    /(?:^|[。！？\n])\s*[^。！？\n]*?(?:小燕|夏燕|晓燕|夏艳|夏妍|夏厌|夏言|念错|念对|发音|纠正|STT|语音识别|写错|叫错|名字怎么|叫什么|叫我|你叫我|你刚才叫|终于叫对|这次叫对|你念成)[^。！？\n]*[。！？]/g,
    /(?:^|[。！？\n])\s*[^。！？\n]*?不是(?:小燕|夏燕|晓燕).*?是夏彦[^。！？\n]*[。！？]/g,
  ];
  let cleaned = reply;
  for (const pat of patterns) {
    cleaned = cleaned.replace(pat, (m) => {
      // Keep the period/line break, strip the sentence content
      const firstChar = m[0] || "";
      const lastChar = m[m.length - 1] || "";
      // If the sentence starts with a delimiter, keep it
      if (/[。！？\n]/.test(firstChar)) {
        console.log(`[name-filter] Stripped: "${m.trim().slice(0, 60)}..."`);
        return firstChar;
      }
      console.log(`[name-filter] Stripped: "${m.trim().slice(0, 60)}..."`);
      return "";
...
export async function handleTextMessage(text, isVoice = false, imageContext = null) {
  ...
  const cleanedReply = stripNameCorrection(reply);
