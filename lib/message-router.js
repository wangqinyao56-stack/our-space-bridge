...
export async function handleTextMessage(text, isVoice = false, imageContext = null) {
  ...
  let prompt;
  if (isTraveling()) {
    // Use standalone travel prompt — completely separate from daily rules
  ...
  if (weatherCtx) {
    prompt += "\n" + weatherCtx;
    // Skip "提议窝在家里/带她去走走" when traveling
    if (isTraveling()) {
...
export async function handleIntimateMessage(text) {
  let prompt;
  if (isTraveling()) {
    // Use standalone travel intimate prompt (phone sex mode)
  ...
  if (chatMsgs.length > 0) {
    ...
    history.unshift({
      role: "system",
      content: `[以下是你和华生在普通聊天中的对话——你记得这些内容]\n${chatContext}`,
    });
  }
