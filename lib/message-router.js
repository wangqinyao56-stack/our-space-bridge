    ...
    const req = https.request({
      ...
      res.on("end", () => {
        try {
          ...
          console.log("[stt] Groq response:", JSON.stringify({ text: text || "(empty)", rawLen: rawBody.length }).slice(0, 200));
          if (!text || text.length < 2) {
            console.log("[stt] Recognition returned empty or too short, asking user to repeat");
