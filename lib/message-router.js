...
export async function handleIntimateMessage(text) {
  ...
  if (intimateMsgs.length > 0) {
    ...
    if (lastTs) {
      ...
      if (lastDay !== bjDay) {
        timeNote += `【强制——新的一天】上一次亲密已经过去至少一天了（今天是${bjMonth}月${bjDay}日，上次是${new Date(lastTs).getMonth()+1}月${lastDay}日）。这是全新的一天——绝对禁止说"刚才""刚刚""刚才做完"之类的话。绝对不要接着上次的话题继续聊、不要延续上次的动作或姿势——上次是昨天的事了。如果华生没有主动提起上次的内容，就当它是过去的事。从当下的时间和状态重新开始。`;
      } else {
...
等晚上回家后才可以写肢体接触。`;
  ...
  if (locHour >= 6 && locHour < 22) {
    availableLocations = allLocations.filter(l => l.name !== "阳台/窗边");
  }
