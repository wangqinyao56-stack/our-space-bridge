...
等晚上回家后才可以写肢体接触。`;
  ...
  console.log("[intimate] Calling askZhailian (宅恋 0.3不明鹿)...", { promptLen: prompt.length, historyLen: history.length, userContentLen: userContent.length });
  intimateLog({ stage: "calling_ai", model: "[0.3]不明鹿/claude-opus-4-6", provider: "zhailian", promptLen: prompt.length, historyLen: history.length });
  const t0 = Date.now();
  let reply;
  try {
    reply = await askZhailian({
      model: "[0.3]不明鹿/claude-opus-4-6",
      systemPrompt: prompt,
      userContent,
      history,
maxTokens: 2000,
