...
maxTokens: 500,
  ...
  if (dateProposal) {
    const plan = proposeDate(dateProposal);
    console.log(`[date-plans] Auto-created from 夏彦 reply: ${plan.id} (${plan.status})`);

    // Send dating_invite to app if a scene was detected
    if (dateProposal.sceneId && _onDatingInvite) {
...
export async function handleAffectionDateMessage(text, sceneId = null) {
  ...
  if (sceneId) {
    const sceneLabels = {
      cafe: "咖啡馆", park: "公园", cinema: "电影院", beach: "海边",
      garden: "花园", snow_mountain: "雪山", aquarium: "水族馆",
      cemetery: "墓园", night_festival: "夜晚庙会",
      shopping_day: "商业街", shopping: "商业街", shopping_evening: "商业街", shopping_night: "商业街",
      subway: "地铁", hospital: "医院", antique_shop: "古物店",
      amusement_night: "游乐场", amusement_day: "游乐园",
      haunted_house: "鬼屋", craft_shop: "手工馆", bookstore: "书店",
      lecture: "讲座", car_ride: "车上", museum: "博物馆",
      camping: "野营地", ancient_town: "古风小镇", hotel: "旅馆",
      skate_climb: "滑板攀岩场",
    };
    const label = sceneLabels[sceneId] || sceneId;
    prompt += `\n\n**【当前约会场景：${label}】**你们现在正在${label}约会。你的对话应该自然融入这个场景的氛围和细节。`;
  }
  ...
  // Cross-channel: inject daily chat + home context
  const chatCtx2 = await getRecentHistoryMessages();
  if (chatCtx2.length > 0) {
    const ctx = chatCtx2.slice(-10).map(m => `${m.role === "user" ? "华生" : "夏彦"}：${m.content}`).join("\n");
    history.unshift({ role: "system", content: `[以下是你和华生在日常聊天中的对话——你记得]\n${ctx}` });
  }
  const homeCtx = getAffectionHistory("affection_home");
  if (homeCtx.length > 0) {
    const ctx = homeCtx.slice(-5).map(m => `${m.role === "user" ? "华生" : "夏彦"}：${m.content}`).join("\n");
    history.unshift({ role: "system", content: `[你们之前有过居家温存——你记得]\n${ctx}` });
  }
...
export async function handleIntimateMessage(text) {
  ...
  if (chatHistory.length > 0) {
    ...
    history.unshift({
      role: "system",
      content: `[以下是你和华生在日常聊天中的对话——你记得这些内容，可以在亲密时自然提及]\n${chatContext}`,
    });
  }
  ...
  if (affHomeCtx.length > 0) {
    ...
    history.unshift({ role: "system", content: `[你们之前在居家温存中的互动——你记得]\n${ctx}` });
  }
