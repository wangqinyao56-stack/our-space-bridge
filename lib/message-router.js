...
import { askDeepSeek, askZhizengzeng, askApi2d } from "./ai.js";
import {
  recordUserMessage,
  recordBotReply,
...
等晚上回家后才可以写肢体接触。`;
  ...
  console.log("[intimate] Calling askApi2d (claude-opus-4-5)...", { promptLen: prompt.length, historyLen: history.length, userContentLen: userContent.length });
  intimateLog({ stage: "calling_ai", model: "claude-opus-4-5", provider: "api2d", promptLen: prompt.length, historyLen: history.length });
  const t0 = Date.now();
  let reply;
  try {
    reply = await askApi2d({
      systemPrompt: prompt,
      userContent,
      history,
      model: "claude-opus-4-5",
    ...
    console.log("[intimate] askApi2d done in", Date.now() - t0, "ms, replyLen:", reply?.length);
    intimateLog({ stage: "ai_done", ms: Date.now() - t0, replyLen: reply?.length });
  } catch (e) {
    console.error("[intimate] askApi2d failed:", e.message);
