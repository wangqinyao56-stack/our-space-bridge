...
import { askDeepSeek, askZhizengzeng, askZhailian } from "./ai.js";
import {
  recordUserMessage,
  recordBotReply,
...
export async function handleIntimateMessage(text) {
  ...
  if (intimateMsgs.length > 0) {
    ...
    if (lastTs) {
      ...
      history.push({
        role: "system",
        content: `【时间参考】现在是北京时间${timeStr}。上一次亲密对话是${lastTime}。如果是昨天或更早的对话，不要当成刚刚发生的事。用"昨晚""上次"等词自然拉开时间距离。`,
      });
    }
  }
...
等晚上回家后才可以写肢体接触。`;
  ...
  console.log("[intimate] Calling askZhailian (宅恋 [0.04]报用鹿)...", { promptLen: prompt.length, historyLen: history.length, userContentLen: userContent.length });
  ...
  try {
    reply = await askZhailian({
      model: "[0.04]报用鹿/claude-opus-4.5",
      systemPrompt: prompt,
      userContent,
      history,
maxTokens: 2000,
