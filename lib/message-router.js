...
import { askDeepSeek, askZhizengzeng, askJiushi, askJiushiStream, askXiongmao, askZhailian, ZHAILIAN_KEY_2 } from "./ai.js";
import {
  recordUserMessage,
  recordBotReply,
...
export async function handlePixelHomeMessage(text, opts = {}) {
  ...
  if (onStreamDelta) {
    reply = await askZhailian({
      ...
      model: "[顾里]按量/claude-opus-4.6",
      apiKey: ZHAILIAN_KEY_2,
      maxTokens: 500,
      temperature: 0.70,
      timeoutMs: 120000,
    });
...
等晚上回家后才可以写肢体接触。`;
  ...
  console.log("[intimate] Calling askZhailian ([0.06]报用鹿/claude-opus-4.6)...", { promptLen: prompt.length, historyLen: history.length, userContentLen: userContent.length });
  intimateLog({ stage: "calling_ai", model: "[0.06]报用鹿/claude-opus-4.6", provider: "zhailian", promptLen: prompt.length, historyLen: history.length });
  const t0 = Date.now();
  let reply;
  try {
    reply = await askZhailian({
    ...
    console.log("[intimate] askZhailian done in", Date.now() - t0, "ms, replyLen:", reply?.length);
    intimateLog({ stage: "ai_done", ms: Date.now() - t0, replyLen: reply?.length });
  } catch (e) {
    console.error("[intimate] askZhailian failed:", e.message);
