...
import { askDeepSeek, askZhizengzeng } from "./ai.js";
import {
  recordUserMessage,
  recordBotReply,
...
export async function handleIntimateMessage(text) {
  ...
  try {
    ...
    console.log("[intimate] askDeepSeek done in", Date.now() - t0, "ms, replyLen:", reply?.length);
    intimateLog({ stage: "ai_done", ms: Date.now() - t0, replyLen: reply?.length });
  } catch (e) {
    console.error("[intimate] askDeepSeek failed:", e.message);
