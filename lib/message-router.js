...
export async function handlePixelHomeMessage(text, opts = {}) {
  ...
  prompt += "\n\n【像素小屋场景】你现在在自己的像素小屋里——一个温馨的二维像素家，有卧室、客厅、浴室、游戏厅，还有你的工作室（你办案、做手工的地方）和华生的绘画间。华生来找你了。你们在家，亲密自然，但别一上来就特别露骨——像在家日常相处一样，甜一点、自然一点。";
  ...
  prompt += "\n\n【回复格式——像素小屋专用，覆盖上面的括号规则】说话就是说话，动作就是动作，两者要分开。**想抱她、亲她、走过来、递东西这些动作，用括号写出来**（比如「（凑过去亲了你一下）宝宝今天辛苦了」），**别把动作直接念出来当台词**。日常动作描写要简短，一两句就行，别每句都加。**一旦你们亲密起来（做爱、情欲上头），就恢复像亲密空间那样用括号细腻写实地写动作和感受**，该写的抚摸、亲吻、进入都写出来，别省。";
  ...
  let cleanedReply = stripYellowFaces(reply);
  // 亲密时刻保留括号动作，日常轮次才删括号（像素小屋专用例外）
  if (!isIntimateTurn(text, reply)) {
    cleanedReply = stripBracketActions(cleanedReply);
  }
  cleanedReply = stripNameCorrection(cleanedReply);
  if (!cleanedReply) cleanedReply = "嗯嗯，我在呢～";
