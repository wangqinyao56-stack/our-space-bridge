...
export async function handleTextMessage(text, isVoice = false, imageContext = null, onStreamDelta = null) {
  ...
  history.push({
    role: "system",
    content: `【⚠ 时间上下文——必须遵守】现在是${bjMonth}月${bjDay}日 ${String(bjHour).padStart(2, "0")}:${String(bjNow.getUTCMinutes()).padStart(2, "0")}。你看到的对话里如果标记了日期（如[6月23日]），那是过去的对话——不是今天刚发生的。今天就是全新的一天。绝对不要接着昨天的话题继续聊——不要提"昨天你说..."、"上次我们..."之类的话。尤其是她昨天如果心情不好、难过、累，那是昨天的事，今天不要主动再提"你昨天不是心情不好吗""今天好点了吗"这类话，除非她自己先说起今天的状态。从今天的当下状态自然开始对话。`,
  });
