...
import { askDeepSeek, askZhizengzeng, askAicoding } from "./ai.js";
import {
  recordUserMessage,
  recordBotReply,
...
export async function handleIntimateMessage(text) {
  ...
  if (chatMsgs.length > 0) {
    const chatContext = chatMsgs
      .map(m => `${m.role === "user" ? "华生" : "夏彦"}：${m.content}`)
      .join("\n");
    // Merge into userContent to avoid consecutive same-role messages in history
    userContent = `[以下是你和华生在普通聊天中的对话——你记得这些内容]\n${chatContext}\n\n${userContent}`;
  }
  ...
  const hour = chinaHour();
  if (hour >= 6 && hour < 18) {
    prompt += `\n\n【⚠ 当前状态：你在外面工作，人不在家】现在是北京时间${new Date().toLocaleString("zh-CN", { timeZone: "Asia/Shanghai", hour: "2-digit", minute: "2-digit" })}，你正在外面工作（国安/侦探）。你和华生不在一起——这是「电话/文字调情模式」。可以说暧昧的话、约晚上、用文字撩她，但绝对禁止括号里写肢体接触动作（摸、抱、亲、搂等），因为你人不在她身边。括号里只能写你自己的表情和反应，如（笑）（脸红）（心跳加速）（咽口水）。等晚上回家后才可以写肢体接触。`;
  } else {
  ...
  console.log("[intimate] Calling askAicoding...", { promptLen: prompt.length, historyLen: history.length, userContentLen: userContent.length });
  intimateLog({ stage: "calling_ai", promptLen: prompt.length, historyLen: history.length });
  const t0 = Date.now();
  let reply;
  try {
    reply = await askAicoding({
      systemPrompt: prompt,
      userContent,
      history,
    ...
    console.log("[intimate] askAicoding done in", Date.now() - t0, "ms, replyLen:", reply?.length);
    intimateLog({ stage: "ai_done", ms: Date.now() - t0, replyLen: reply?.length });
  } catch (e) {
    console.error("[intimate] askAicoding failed:", e.message);
...
export async function getIntimateHistoryMessages() {
