...
export async function handleAffectionDateMessage(text, sceneId = null) {
  ...
  if (sceneId) {
    const sceneLabels: Record<string, { label: string; detail: string }> = {
  ...
  // Cross-channel: inject daily chat + home context — for personality continuity only, NOT topic continuation
  // 夏彦 remembers these conversations but shouldn't drag their topics into the date
  ...
  if (chatCtx2.length > 0) {
    ...
    history.unshift({ role: "system", content: `[以下是你们之前在普通聊天中的对话——你记得这些事，但这是背景信息，不是当前话题。你现在在约会，不要接着日常聊天的话题往下聊。]\n${ctx}` });
  }
  const homeCtx = getAffectionHistory("affection_home");
  if (homeCtx.length > 0) {
    const ctx = homeCtx.slice(-5).map(m => `${m.role === "user" ? "华生" : "夏彦"}：${m.content}`).join("\n");
    history.unshift({ role: "system", content: `[你们之前有过居家温存——你记得，但不要搬到约会里提。]\n${ctx}` });
  }
