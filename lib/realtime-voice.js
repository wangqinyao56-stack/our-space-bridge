...
function emotionParams(text) {
  ...
  // 兴奋/开心：更亮更快（只提音调+提速，不降音量，避免气音）
  if (/[！!]|哈哈|太好|开心|喜欢|好耶|太棒|爱死|兴奋|激动|耶|棒呆|嘿嘿|嘻嘻/.test(t)) {
    pitch += 5; speed += 10;
  }
  // 疑问/追问：句尾上扬
  ...
  // 难过/疲惫/安抚：更低（只降音调，不动音量/速度）
  else if (/难过|想哭|对不起|担心|害怕|委屈|难受|哭|疼|伤心|舍不得|累|困/.test(t)) {
    pitch -= 4;
  }
  // 撒娇/软语：略软（只微调音调，不降速）
  else if (/啦|嘛|呀|喔|乖|宝宝|老婆|亲|抱抱|想你/.test(t)) {
    pitch += 2;
  }
