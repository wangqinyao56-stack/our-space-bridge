...
async function generateDailyLifeMoment() {
  ...
  const travelNote = traveling
    ? `\n\n【⚠️ 当前状态：你正在外地出差（${getTravelState().destination || "外地"}），不在未名市】分享的内容必须符合出差状态——你住在酒店，不在家，不在古物店。可以分享：在陌生城市的新发现、酒店窗外的风景、出差途中的偶遇、当地特色的咖啡馆或小吃、异地的天气和环境。禁止：下厨做饭、阳台植物、在家修东西、古物店/邻里/学员相关——这些都不存在。`
    : "";
