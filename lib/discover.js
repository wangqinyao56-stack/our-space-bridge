...
async function searchUnsplashImage(keyword) {
  try {
    // Add "no people" constraint
    const safeKeyword = `${keyword} no people no faces`;
    const searchUrl = `https://source.unsplash.com/featured/800x600/?${encodeURIComponent(safeKeyword)}`;
    console.log(`[discover] Searching Unsplash for: ${keyword}`);
