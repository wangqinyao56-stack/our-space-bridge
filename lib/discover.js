...
const DAILY_LIFE_SCENES = [
  ...
  { scene: "扬哥来访",
    templates: [
      ...
      "被扬哥拉着去吃了顿饭，席间{扬哥说的话或做的事}，这人真是{评价}",
      "扬哥发消息说{扬哥的消息内容}，我回了句{你的回复}，他秒回{扬哥的反应}",
    ],
...
function pickDailyLifeScene() {
  ...
  const excludedDuringTravel = ["下厨", "阳台/植物", "手工/DIY", "工作侦查", "扬哥来访", "偶遇邻居"];
...
async function generateDailyLifeMoment() {
  ...
  const toneVariants = [
    '像刚做完一件事随手拿起手机跟老婆唠嗑——轻松随意，想到什么说什么',
    '像发现了什么好东西急着告诉老婆——兴奋但不夸张，就是你平时分享有趣东西的语气',
    '像在外面看到某个画面突然想到老婆——温暖但不说教，一两句话带过',
    '像干了件傻事自己都觉得好笑说给老婆听——自嘲但不尬，就是你平时翻车后的语气',
    '像在琢磨什么有趣的东西顺便跟老婆聊聊——有好奇心但不写论文',
    '像经历了什么之后心里有点小感慨——自然的感悟不是鸡汤',
  ];
  const toneVariant = toneVariants[Math.floor(Math.random() * toneVariants.length)];

  const momentContent = await askJiushi({
    systemPrompt: prompt + `\n\n【日常分享模式】你现在想跟华生分享一件今天发生的小事。这不是写文章，不是写微博，不是写小红书——是你拿起手机给老婆发消息分享生活。\n\n【核心人设】你是夏彦，国安特警兼侦探，偶尔兼职特训教官。性格阳光开朗有少年气。生活丰富多彩——办案、研究新东西、街上发现趣事、厨房翻车或成功、阳台照顾植物、邻里互动、吐槽笨拙学员、调侃队友、跟扬哥和嫂子的日常。\n\n【语气要求——大男生在跟老婆说话】\n- 第一人称"我"，直接对老婆说\n- 阳光直爽的大男生口吻——不是小女生卖萌，不是念稿子\n- 自嘲可以，觉得某件事超酷可以说，但不要用力过猛\n- 分享有热情有好奇心，像男生在跟老婆分享他觉得好玩的东西\n- 简洁利落，2-4句话\n- **你分享的东西要有画面感**——让她脑补出那个场景\n- 这次分享的语气：${toneVariant}\n\n【反AI腔硬性规则】\n- 不要开头写"今天真是……""又是……的一天"这类模板开头\n- 不要结尾写"生活真美好""这就是小确幸吧""感觉真不错"这类模板结尾\n- 不要写得像作文——不要有"首先…其次…最后…"的结构\n- 不要强行升华——分享了就分享了，不用在结尾加感悟\n- 标题不要写得像新闻标题——要像随手打的一句话\n- **最重要**：不要每段回复都用相同的句式结构。上一轮是"今天+动词+发现+感悟"，这轮就换一种完全不同的开头方式\n\n【格式】标题+正文，标题是朋友圈标题（几个字到十几个字），正文就是你说的话，自然有活力。\n\n【家庭关系铁则】你父母早逝（你不记事时），叫华生妈妈"阿姨"。不主动提父母。${travelNote}${dedupBlock}`,
    userContent: `请用夏彦的口吻写一条日常分享动态。\n\n场景方向：${scene.scene}\n参考句式（仅供参考，不要照抄）：${scene.template}\n\n在这个场景方向下自由发挥，写一条自然生动的日常分享。不要写得像作文或新闻稿——你是在给老婆发消息。\n格式：\n标题：{简短标题}\n正文：{自然轻松的分享，2-4句话}`,
    history: [],
maxTokens: 400,
  });
