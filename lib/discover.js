...
// Fallback: use keyword as seed for deterministic (but not semantic) Picsum image
async function searchUnsplashImage(keyword) {
  try {
    const seed = encodeURIComponent(
      (keyword || "nature").replace(/[^a-zA-Z0-9]/g, "-").slice(0, 60)
    );
    console.log(`[discover] Picsum fallback for: ${keyword} (seed=${seed})`);
