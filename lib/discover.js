...
// ── 冲浪：调 Tavily 搜索，替夏彦去网上看看世界（比抓网页更准、更新鲜）──
function surfTavily(query, maxResults = 5) {
  ...
  return new Promise((resolve) => {
    const req = https.request({
      host: TAVILY_HOST, path: "/search", method: "POST", headers,
      agent: proxyAgent, timeout: 20000,
    }, (res) => {
      const chunks = [];
...
// 冲浪频率限制，避免耗尽 Tavily 免费额度
let surfState = { date: "", count: 0 };
const SURF_MAX_PER_DAY = 15;

async function surfForDiscover(mainCategory) {
  ...
  const results = await surfTavily(`${mainCategory} 新鲜事 趣闻`, 5);
  if (!results || results.length === 0) return "";
  const items = results.slice(0, 5).map((r) => `· ${r.title}：${(r.content || "").slice(0, 100)}`).join("\n");
  return `\n【冲浪见闻——你刚在网上刷到的真实新鲜事（围绕「${mainCategory}」）】\n${items}\n挑一条你觉得好玩的，结合你自己的想法分享，别照着念标题。`;
}
...
export async function generateDiscoverMoment() {
  ...
  // 1. 用 Tavily 按兴趣方向搜真实新鲜事（比抓网页更准、更新鲜）
  try {
    const surf = await surfForDiscover(mainCategory);
    if (surf) {
      sourceText = surf;
      console.log("[discover] Tavily surf OK");
    }
  } catch (err) {
    console.log(`[discover] Tavily surf failed: ${err.message}`);
  }
