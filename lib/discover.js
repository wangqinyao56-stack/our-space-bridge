...
// Search for a topic-relevant image via Picsum (random beautiful images)
async function searchUnsplashImage(keyword) {
  try {
    // Try picsum for a random beautiful photo — always works
    console.log(`[discover] Getting image for: ${keyword}`);
    const imageBuf = await downloadImage("https://picsum.photos/800/600");
    if (imageBuf && imageBuf.length > 1000) {
      console.log(`[discover] Image OK, size=${imageBuf.length}`);
      return { base64: imageBuf.toString("base64"), mime: "image/jpeg" };
    }
    console.log(`[discover] Picsum returned ${imageBuf ? imageBuf.length : 'null'} bytes`);
  } catch (err) {
    console.log(`[discover] Image fetch failed: ${err.message}`);
  }
  return null;
}
...
// Get image for a topic — try Unsplash first, fall back to Flux
export async function getImageForTopic(keyword, fluxPrompt = "") {
  // Try Unsplash first (fast & free)
  const unsplash = await searchUnsplashImage(keyword);
  if (unsplash) return unsplash;

  // Fall back to Flux
  if (fluxPrompt && config.BFL_API_KEY) {
    console.log(`[discover] Unsplash miss, trying Flux for: ${fluxPrompt.slice(0, 60)}...`);
    return await generateFluxImage(fluxPrompt || keyword);
  }
  return null;
}
