...
function extractAnimalKeyword(text) {
  const breeds = [
    { pattern: /橘猫/, keyword: "orange tabby cat" },
    ...
    { pattern: /猫咪|猫猫|小猫/, keyword: "cute cat" },
...
maxTokens: 400,
  ...
  const imageKeyword = animalKeyword || title || scene.imageKeyword;
  const fluxPrompt = animalKeyword
    ? `photorealistic, realistic photography, cute ${animalKeyword}, high quality animal portrait`
    : scene.fluxPrompt;
  const image = Math.random() < 0.45 ? await getImageForTopic(imageKeyword, fluxPrompt) : null;
...
maxTokens: 400,
  ...
  if (title && Math.random() < 0.45) {
    const imgPrompt = animalKw
      ...
      : "";
    const imageResult = await getImageForTopic(animalKw || title, imgPrompt);
    if (imageResult) {
