...
// Get image for a topic — Flux first for variety, free fallback
export async function getImageForTopic(keyword, fluxPrompt = "") {
  const prompt = fluxPrompt || keyword;

  // 1. Flux first — generates unique images every time
  if (config.BFL_API_KEY && prompt) {
    try {
      ...
      console.log(`[discover] Flux failed: ${err.message}, trying free fallback...`);
    }
  }
