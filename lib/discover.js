...
// Keyword-based image search: LoremFlickr (free, no API key) → Picsum seed (last resort)
async function searchUnsplashImage(keyword) {
  const safeKeyword = (keyword || "nature").trim();

  // Try LoremFlickr first — actually respects keywords
  try {
    // Take first 3 words, lowercase, for LoremFlickr URL
    ...
    console.log(`[discover] LoremFlickr returned ${imageBuf ? imageBuf.length : 'null'} bytes, falling back to Picsum`);
  } catch (err) {
    console.log(`[discover] LoremFlickr failed: ${err.message}, falling back to Picsum`);
  }

  // Last resort: Picsum with seed for deterministic random
  try {
    const seed = encodeURIComponent(
      safeKeyword.replace(/[^a-zA-Z0-9]/g, "-").slice(0, 60)
    );
    console.log(`[discover] Picsum seed fallback: seed=${seed}`);
    const imageBuf = await downloadImage(`https://picsum.photos/seed/${seed}/800/600`);
    if (imageBuf && imageBuf.length > 1000) {
      console.log(`[discover] Picsum OK, size=${imageBuf.length}`);
      return { base64: imageBuf.toString("base64"), mime: "image/jpeg" };
    }
  } catch (err) {
    console.log(`[discover] Picsum fetch failed: ${err.message}`);
  }
  return null;
}
...
// Get image for a topic — try Flux first (matches content), fall back to Picsum
export async function getImageForTopic(keyword, fluxPrompt = "") {
  const prompt = fluxPrompt || keyword;
  if (config.BFL_API_KEY && prompt) {
    console.log(`[discover] Trying Flux for: "${prompt.slice(0, 80)}..."`);
    try {
      ...
      console.log(`[discover] Flux failed: ${err.message}, falling back to Picsum`);
    }
  }

  // Fall back to Picsum (keyword as seed for consistent random image)
