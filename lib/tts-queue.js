...
const FISH_API_KEY = config.FISH_AUDIO_API_KEY || "sk-fish-3fRkP-pXlRHUhmGB50-LDlNjbWlBdYXh-vgoRNs8Jwo";
...
// ── Unified TTS (Fish Audio 夏彦声线 primary, 火山 → ElevenLabs fallback) ──
...
async function speak(text) {
  try {
    ...
    console.log(`[tts] FishAudio 失败: ${err.message}, 回退火山`);
    try {
      return await synthesizeBuffer(text);
    } catch (err2) {
      console.log(`[tts] 火山失败: ${err2.message}, 回退 ElevenLabs`);
