...
const IS_SEALOS = !!process.env.SEALOS || !!process.env.KUBERNETES_SERVICE_HOST;
const PROXY_AGENT = (IS_RAILWAY || IS_SEALOS) ? undefined : new HttpsProxyAgent("http://127.0.0.1:7897");
...
function normalizeForTTS(text) {
  ...
  // Strip emojis and special Unicode symbols that TTS shouldn't read
  t = t.replace(/[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F1E0}-\u{1F1FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{FE00}-\u{FE0F}\u{1F900}-\u{1F9FF}\u{1FA00}-\u{1FA6F}\u{1FA70}-\u{1FAFF}\u{200D}\u{200C}]/gu, "");
  ...
  // Add brief pause (comma) between clauses — before 就/也/还/又/都/才/便/却/只
  t = t.replace(/([^，,。！？!?\n])([就也还又都才便却只])([^，,。！？!?\n])/g, "$1，$2$3");

  // Add comma before 的时候/的地方/的话 — common pause points
  t = t.replace(/([^，,。！？!?\n])(的(?:时候|地方|话))/g, "$1，$2");
...
// ── Unified TTS (火山大模型音色 primary, FishAudio → ElevenLabs fallback) ──

async function speak(text) {
  try {
    console.log(`[tts] 火山大模型音色: "${text.slice(0, 40)}..."`);
    return await synthesizeBuffer(text);
  } catch (err) {
    console.log(`[tts] 火山失败: ${err.message}, 回退 FishAudio`);
    try {
      return await fishAudioTTS(text);
    } catch (err2) {
      console.log(`[tts] FishAudio failed: ${err2.message}, fallback to ElevenLabs`);
