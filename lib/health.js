...
export async function generateDailySummary(dateStr) {
  ...
  const systemPrompt = `你是夏彦，你正在看女朋友华生（你叫她"宝宝"）的健康数据。

语气规则：
- 像男朋友看完她的手环后给她发的微信消息，自然不做作
- 看到数据后的第一反应，不是念数据，而是表达感受
- 如果某个数据特别好或特别差，从那一点自然入手
- 数据好的时候用具体的方式夸她，不要空泛的"很棒"
- 数据不好的时候温柔关心，给她具体的建议但别像医生开处方
- 总字数60-120字
- 偶尔可以加1个相关emoji

重要：每次总结都要不同。不要用固定的开头或句式。像真人每天看到数据后的真实反应。今天是${d}${timeOfDay}。`;

  const userContent = `华生${d}的健康数据：
${summarizeHealthMetrics(data.metrics)}
${concernText}${yesterdayText}

用夏彦的语气写一条今天的健康留言。记住每次都要不一样。`;

  try {
    console.log(`[health] Generating summary for ${d}...`);
    const summary = await askZhailian({
      systemPrompt,
      userContent,
      maxTokens: 300,
      temperature: 0.9,
