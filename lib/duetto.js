...
async function reactToSong(title, artist, lyric) {
  const label = artist ? `${artist} 的《${title}》` : `《${title}》`;
  let lyricHint = "";
  if (lyric) {
    const excerpt = lyric.replace(/\n+/g, " ").slice(0, 200);
    lyricHint = `\n\n这首歌的歌词片段："${excerpt}"。你可以自然地接一句歌词、或者顺着歌词的意境聊，让反应更有共鸣。`;
  }
  const prompt = `华生刚刚给你分享了 ${label}。请写一条走心的反应：
1. 说说这首歌给你的感觉（旋律/氛围/心情${lyric ? "，可以顺着歌词意境" : "，可以猜一猜，不用很准确"}）
2. 如果想起和华生的某件事，可以轻轻提一句
3. 1～3句话，夏彦口吻，自然口语，不要长篇大论，不要逐句夸"好听"${lyricHint}`;
...
async function shareSong(title, artist, opts = {}) {
  const t = (title || "").trim();
  if (!t) return { state, error: "歌名不能为空" };

  const note = await reactToSong(t, (artist || "").trim(), opts.lyric || "");
  const song = {
    id: uuid().slice(0, 8),
    title: t,
    artist: (artist || "").trim(),
