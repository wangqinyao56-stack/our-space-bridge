...
function load() {
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      console.log("[period] Loaded from", DATA_FILE, "currentStart:", state.currentStart);
    } else if (fs.existsSync(LEGACY_FILE)) {
      const saved = JSON.parse(fs.readFileSync(LEGACY_FILE, "utf-8"));
      state = { ...state, ...saved };
      if (!state.dailyTipsUsed) state.dailyTipsUsed = [];
      save();
      console.log("[period] Migrated legacy data from", LEGACY_FILE, "->", DATA_FILE, "currentStart:", state.currentStart);
    } else {
      console.log("[period] No data file at", DATA_FILE, "- starting fresh");
    }
  } catch (e) {
    console.error("[period] Failed to load period data:", e.message);
  }
}
load();
...
function save() {
  try {
    ...
    console.error("[period] Failed to save period data:", e.message);
  }
}
