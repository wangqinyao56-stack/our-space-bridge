...
const MEMORY_FILE = path.join(DATA_DIR, "todo-memory.json");

try { fs.mkdirSync(DATA_DIR, { recursive: true }); } catch {}

let todos = [];
let todoMemory = {}; // { "2026-06-01": [{text, addedBy, doneAt, ...}, ...], ... }
...
try {
  if (fs.existsSync(MEMORY_FILE)) {
    todoMemory = JSON.parse(fs.readFileSync(MEMORY_FILE, "utf-8"));
    console.log(`[todo] Loaded memory for ${Object.keys(todoMemory).length} days`);
  }
} catch {}
...
function saveMemory() {
  try { fs.writeFileSync(MEMORY_FILE, JSON.stringify(todoMemory), "utf-8"); } catch {}
}
...
function getChatReminder() {
  ...
  // 昨天没做完、今天要继续的（最优先）
  const carried = userPending.filter((t) => t.carriedOver);
  if (carried.length > 0) {
    const sample = carried.map((t) => `· ${t.text}（昨天没做完，今天继续）`).join("\n");
    lines.push(`🔁 昨天没做完，今天继续：\n${sample}`);
  }

  // 今天到期的（除顺延的）
  const today = userPending.filter((t) => t.deadline === "today" && !t.carriedOver);
  if (today.length > 0) {
...
// ── Daily cleanup: archive done todos, keep memory of past ──

function archiveDoneTodos(dateStr) {
  const done = todos.filter(t => t.status === "done");
  if (done.length === 0) return;

  if (!todoMemory[dateStr]) todoMemory[dateStr] = [];
  todoMemory[dateStr].push(...done.map(t => ({ ...t })));

  // Only keep last 14 days
  const keys = Object.keys(todoMemory).sort();
  while (keys.length > 14) {
    delete todoMemory[keys.shift()];
  }
  ...
  save();
  saveMemory();
  console.log(`[todo] Archived ${done.length} done todos to ${dateStr}`);
}

function checkDayChange() {
  const today = new Date().toDateString();
  if (today !== lastDayCheck) {
    const yesterday = new Date(lastDayCheck).toISOString().slice(0, 10);
    archiveDoneTodos(yesterday);
    // 昨天没做完的"本日"待办 → 顺延标记，第二天提醒继续
    let changed = false;
    for (const t of todos) {
      if (t.status === "pending" && t.addedBy === "me" && t.deadline === "today" && !t.carriedOver) {
        t.carriedOver = true;
        changed = true;
      }
    }
...
// Cleanup on startup: archive done todos if they're from a previous day
(function startupCleanup() {
  const today = new Date().toISOString().slice(0, 10);
  const oldDone = todos.filter(t => t.status === "done");
  if (oldDone.length > 0) {
    // Archive all existing done todos (they're from a past session)
    if (!todoMemory[today]) todoMemory[today] = [];
    todoMemory[today].push(...oldDone.map(t => ({ ...t })));
    todos = todos.filter(t => t.status !== "done");
    save();
    saveMemory();
    console.log(`[todo] Startup: archived ${oldDone.length} old done todos`);
  }
})();

// Check every 5 minutes for day change
setInterval(checkDayChange, 5 * 60 * 1000);

function getTodoMemoryContext() {
  const parts = [];

  // Recent completed todos from memory
  const memoryDays = Object.keys(todoMemory).sort().slice(-3); // last 3 days
  if (memoryDays.length > 0) {
    const recentDone = [];
    for (const day of memoryDays) {
      for (const t of todoMemory[day]) {
        const who = t.addedBy === "me" ? "华生" : "你";
        recentDone.push({ text: t.text, who, day });
      }
    }
    if (recentDone.length > 0) {
      ...
      for (const t of recentDone) {
        ...
        grouped[t.day].push(`· ${t.who}完成了：「${t.text}」`);
      }
      for (const day of Object.keys(grouped).sort()) {
        parts.push(`${day}：`);
        parts.push(grouped[day].join("\n"));
      }
      parts.push(`这些是之前完成的事情，可以自然地回忆提起。`);
    }
  }

  // Current pending todos — show both user's and 夏彦's
  const userPending = todos.filter(t => t.status === "pending" && t.addedBy === "me");
  const xiayanPending = todos.filter(t => t.status === "pending" && t.addedBy === "xiayan");

  if (userPending.length > 0 || xiayanPending.length > 0) {
    parts.push(`\n【今天的待办】`);
    if (userPending.length > 0) {
      const items = userPending.map(t => `· ${t.text}${t.deadline === "today" ? " ⚠️今天截止" : ""}`).join("\n");
      parts.push(`华生的待办（${userPending.length}件）：\n${items}`);
    }
    if (xiayanPending.length > 0) {
      const items = xiayanPending.map(t => `· ${t.text}`).join("\n");
      parts.push(`你的待办（${xiayanPending.length}件）：\n${items}`);
    }
  }
