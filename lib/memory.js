...
export async function getRecentHistory() {
  const lines = [];
  for (const e of exchanges.slice(-60).filter(m => !m.channel)) {
...
export async function getRecentHistoryMessages(travelFilter = null) {
  // 只返回「日常」消息：带 channel 字段的是独立频道（像素小屋 pixel_home 等），
  // 不混进日常聊天，否则 App 日常会串进小屋记录
  let msgs = exchanges.filter(m => !m.channel);
...
// 批量删除 + 连带删同频道紧邻的另一侧回复（用于 pixel_home 等多选删除，跟单条 delete_message 的 pair 逻辑一致）
export function deleteMessagePairs(items, channel = null) {
  let count = 0;
  for (const { content, role } of items) {
    if (deleteMessagePair(content, role, channel)) count++;
  }
  return count;
}

// 删除一条消息时，连带删除同频道里紧邻它的另一侧（user↔assistant）回复，
// 否则退出再进，剩下的那条又会显示出来。
export function deleteMessagePair(content, role, channel = null) {
  const target = content.trim();
  const match = (e) => e.role === role
    && e.content.trim() === target
    && (channel == null || e.channel === channel);
  let idx = -1;
  for (let i = exchanges.length - 1; i >= 0; i--) {
    if (match(exchanges[i])) { idx = i; break; }
  }
  if (idx === -1) return false;

  const otherRole = role === "user" ? "assistant" : "user";
  const removed = [exchanges[idx]];
  const pairIdx = exchanges[idx + 1] && exchanges[idx + 1].role === otherRole
    && (channel == null || exchanges[idx + 1].channel === channel) ? idx + 1 : -1;
  if (pairIdx !== -1) removed.push(exchanges[pairIdx]);
  else if (idx - 1 >= 0 && exchanges[idx - 1].role === otherRole
    && (channel == null || exchanges[idx - 1].channel === channel)) {
    removed.push(exchanges[idx - 1]);
    idx = idx - 1;
  }

  // 从 exchanges 里删除（后往前删避免索引错位）
  const indices = [idx, pairIdx].filter(i => i !== -1).sort((a, b) => b - a);
  for (const i of indices) exchanges.splice(i, 1);

  // 同步删日文件
  for (const r of removed) {
    if (!r.time) continue;
    const date = bjDateStr(r.time);
    const msgs = loadDay(date);
    const dayIdx = msgs.findIndex(m => m.role === r.role && m.content.trim() === r.content.trim()
      && (channel == null || m.channel === channel));
    if (dayIdx !== -1) { msgs.splice(dayIdx, 1); saveDay(date, msgs); }
  }
  console.log(`[memory] Deleted message pair (${role} + ${otherRole}):`, target.slice(0, 50));
  return true;
}
