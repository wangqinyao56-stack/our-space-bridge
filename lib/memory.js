...
export function recordUserMessage(text) {
  ...
  exchanges.push({ role: "user", content: text.slice(0, 2000), time: now, travel });
  const date = bjDateStr(now);
  const msgs = loadDay(date);
  msgs.push({ role: "user", content: text.slice(0, 2000), time: now, travel });
  saveDay(date, msgs);
}
...
export function recordBotReply(text) {
  const now = Date.now();
  const travel = isTraveling();
  exchanges.push({ role: "assistant", content: text.slice(0, 2000), time: now, travel });
  const date = bjDateStr(now);
  const msgs = loadDay(date);
  msgs.push({ role: "assistant", content: text.slice(0, 2000), time: now, travel });
  saveDay(date, msgs);
}
...
export function deleteMessage(content, role) {
  ...
  // Find the newest matching exchange (search backwards)
  let idx = -1;
