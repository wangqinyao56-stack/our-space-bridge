...
export async function generateTaskScene(task, history = []) {
  let ctx = "";
  if (history.length > 0) {
    const recent = history.slice(-3).map((h) => `[${h.who === "player" ? "华生" : "夏彦"}·${h.label}] ${h.scene}`).join("\n");
    ctx = `\n\n（刚才的场景：\n${recent}\n请让这段场景和之前自然连贯、有延续感，接着上一段的状态往下写，不要像第一次发生一样生硬。如果这一格是"事后温存"或"中场休息"，就接着上一段的状态往下走。）`;
  }
...
4. 直接开始，不要加"好的""我们来"这类开头`;
  ...
  try {
    const reply = await askJiushi({ systemPrompt: getIntimatePrompt(), userContent: prompt, history: [], maxTokens: 500, temperature: 0.9, timeoutMs: 20000 });
...
/** 生成开场：夏彦看到华生打开大富翁，知道她想干嘛，笑着过来一起玩（随机角度，每次不一样） */
const OPENING_ANGLES = [
  "你刚打开棋盘，眼睛就亮起来，笑着凑过来，已经伸手把你揽进怀里，手指漫不经心地在你腰上摩挲",
  "从背后环住你，下巴抵在你肩上，在你耳边低笑，说今晚想和你玩点好玩的",
  "把你拉到腿上坐着，手指已经滑进你衣服下摆，一边问你今天想怎么玩",
  "捏了捏你的脸，笑着说你今天兴致不错，已经迫不及待想看你落在哪一格了",
  "把你面前的棋盘转过来，单手撑着桌沿低头看你，问你是自己乖乖坐过来还是要他抱",
];

export async function generateOpening() {
  const angle = OPENING_ANGLES[Math.floor(Math.random() * OPENING_ANGLES.length)];
  const prompt = `华生打开了"亲密大富翁"。请以夏彦的身份写一段开场——${angle}。要求：3～5句，动作+情话，自然有画面感，宠溺温柔，可以已经在爱抚她，直接开始，不要"好的""我们来"这类开头。`;
  try {
    const reply = await askJiushi({ systemPrompt: getIntimatePrompt(), userContent: prompt, history: [], maxTokens: 400, temperature: 1.0, timeoutMs: 20000 });
    if (reply?.trim()) return reply.trim();
  } catch (e) {
    console.error("[monopoly] opening failed:", e.message);
  }
  return "（夏彦看到你打开大富翁，笑着凑过来）宝宝，想和老公玩点好玩的？";
}
