...
let data = {
  // date(YYYY-MM-DD) -> 夏彦射精次数（保留 intimacy 名称兼容旧日历）
  intimacy: {},
  lastEjaculationTs: 0,
  ejaculationThresholdNotified: {},
  ejaculationEvents: {},
  // date -> [{ id, text, ts }]
  notes: {},
...
try {
  if (fs.existsSync(CALENDAR_FILE)) {
    ...
    data = { intimacy: {}, notes: {}, alarms: [], lastEjaculationTs: 0, ejaculationThresholdNotified: {}, ...raw };
  }
} catch (e) {
...
/** 记录一次夏彦射精。返回 { date, count, crossed, skipped } */
export function recordEjaculationEvent(eventId = "") {
  const now = Date.now();
  const date = bjDateStr(now);
  const key = eventId ? `${date}:${eventId}` : "";
  if (key && data.ejaculationEvents?.[key]) {
    return { date, count: data.intimacy[date] || 0, crossed: false, skipped: true };
  }
  data.ejaculationEvents ||= {};
  if (key) data.ejaculationEvents[key] = now;
  data.lastEjaculationTs = now;
  data.intimacy[date] = (data.intimacy[date] || 0) + 1;
  const count = data.intimacy[date];
  const crossed = count === 4 && !data.ejaculationThresholdNotified[date];
  if (crossed) data.ejaculationThresholdNotified[date] = true;
  save();
  return { date, count, crossed, skipped: false };
}
