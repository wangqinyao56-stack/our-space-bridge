...
const DEEPSEEK_MODEL = "[企业按量]claude-opus-4-6"; // 玖时已无 gpt-5.4，提取改用活的按量 opus
...
const ZHAILIAN_KEY_2 = "sk-UKGuEqZZEdSc9EZgJUgxE6hADi2WquEFMGP9y99ZoQwAYCY5";
const ZHAILIAN_HOST = "az.zlapi.vip";

export { ZHAILIAN_KEY, ZHAILIAN_KEY_2 };

export async function askZhailian(opts = {}) {
  const apiKey = opts.apiKey || ZHAILIAN_KEY;
  try {
    return await _openaiCompatCall({
      ...opts,
      model: opts.model || "[君离-按量]k/claude-opus-4-6",
      host: ZHAILIAN_HOST,
      apiKey,
    });
  } catch (e) {
...
// ── 熊猫API (api520.pro) for 阿鹿亲密空间 + 交换日记 ──

export const XIONGMAO_KEY = "sk-J9tf0Ypjjp3vrBjiY0HYgUKcYju8WefQ2771laqIKYAh8hUP";
const XIONGMAO_HOST = "api520.pro";
const XIONGMAO_MODEL = "熊猫-A-29-claude-opus-4.6";

export async function askXiongmao(opts = {}) {
  return _openaiCompatCall({
    ...opts,
    model: opts.model || XIONGMAO_MODEL,
    host: XIONGMAO_HOST,
    apiKey: opts.apiKey || XIONGMAO_KEY,
  });
}

// ── 聚梦API (api.jumengai.net) for 亲密空间（渠道对海外IP路由差异，启动时探测可用模型） ──

const JUMENGAI_KEY = "sk-raBs7UUNc4truapDgGIGcKZ5dwrsCbMrUrLrbL1CEiIojSSF";
const JUMENGAI_HOST = "api.jumengai.net";
const JUMENGAI_PROXY_URL = process.env.JUMENGAI_PROXY || "http://127.0.0.1:7890";
const JUMENGAI_AGENT = new HttpsProxyAgent(JUMENGAI_PROXY_URL);
const JUMENGAI_CANDIDATES = [
  "[网逆]claude-opus-4-7",
  "[网逆]claude-opus-4-8",
  "[网逆]claude-opus-4-6",
  "[K2-个人]claude-opus-4-6",
  "【J2缓存按量】claude-opus-4-6",
  "[J3按量]claude-opus-4-6",
  "【k-个人】claude-opus-4-6",
];

let _jumengaiModel = null;
let _jumengaiProbe = null;

const JUMENGAI_PROBE_PROMPT = "请用至少200字详细描写一个温馨的居家场景，包含人物、动作、声音、氛围，连贯成段，不要列表。";

function probeJumengai(model) {
  return new Promise((resolve) => {
    const body = JSON.stringify({
      model, max_tokens: 800, temperature: 0.7,
      messages: [{ role: "user", content: JUMENGAI_PROBE_PROMPT }],
    });
    const req = https.request({
      host: JUMENGAI_HOST, path: "/v1/chat/completions", method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${JUMENGAI_KEY}` },
      timeout: 20000,
      agent: JUMENGAI_AGENT,
    }, (res) => {
      let d = "";
      res.on("data", c => d += c);
      res.on("end", () => {
...
async function detectJumengaiModel() {
  let best = null;
  for (const m of JUMENGAI_CANDIDATES) {
    const len = await probeJumengai(m);
    console.log(`[jumengai] probe ${m} => ${len > 0 ? len + "字" : "不可用"}`);
    if (len > 0 && (!best || len > best.len)) best = { model: m, len };
  }
  if (!best) throw new Error("聚梦所有候选模型均不可用");
  console.log(`[jumengai] 选用 ${best.model}（${best.len}字）`);
  return best.model;
}

export async function askJumengai(opts = {}) {
  if (!_jumengaiModel) {
    _jumengaiProbe = _jumengaiProbe || detectJumengaiModel();
    _jumengaiModel = await _jumengaiProbe;
  }
  return _openaiCompatCall({
    ...opts,
    model: _jumengaiModel,
    host: JUMENGAI_HOST,
    apiKey: JUMENGAI_KEY,
    agent: JUMENGAI_AGENT,
  });
...
export async function askJiushi(opts = {}) {
  try {
    ...
    const rest = { ...opts };
    delete rest.model; // 去掉玖时专用 model，宅恋用自己默认 [君离-按量]k/claude-opus-4-6
...
async function _openaiCompatCall(opts = {}) {
  ...
  // 429 限流 + 瞬时网络错误（ECONNRESET/timeout/5xx）退避重试：
  // 中转站源服务器抖动（520/522）是概率性的，指数退避多试几次往往就过
  const MAX_RETRIES = 4;
  let lastErr;
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const retriable = msg.includes("429")
      ...
      console.error(`[ai] 可重试错误 (${msg.slice(0, 60)})，${waitMs}ms 后重试 (${attempt + 1}/${MAX_RETRIES})`);
