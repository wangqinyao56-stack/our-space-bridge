...
// Only use proxy locally — Sealos has no proxy
// Proxy auto-detect: only use when locally available (never on Sealos)
...
// ── api2d (Claude proxy, supports system role, Chinese users OK) ──

const API2D_KEY = "fk243680-T6aAs9kjlPLEy9sMfkDFwRdA6ob1Xwov";
const API2D_HOST = "openai.api2d.net";

export async function askApi2d(opts = {}) {
  const {
    systemPrompt,
    userContent,
    history = [],
  ...
  // Strip extra fields (time, travel, etc.) — api2d rejects them
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      res.on("end", () => {
        if (res.statusCode !== 200) {
          console.error("[api2d] HTTP", res.statusCode, Buffer.concat(chunks).toString().slice(0, 300));
          reject(new Error(`api2d ${res.statusCode}`));
          return;
        }
        try {
          const data = JSON.parse(Buffer.concat(chunks).toString());
          ...
          if (data.choices?.[0]?.message?.content) {
            ...
            // Anthropic format (content array of blocks)
            reply = data.content.filter(b => b.type === "text").map(b => b.text || "").join("").trim();
          } else if (typeof data.content === "string") {
            reply = data.content.trim();
          }
          if (!reply) {
            console.error("[api2d] Empty response, body:", JSON.stringify(data).slice(0, 300));
            throw new Error("Empty response");
          }
          resolve(reply);
        } catch (e) {
    ...
    req.on("error", (e) => { console.error("[api2d] Request error:", e.message); reject(e); });
    req.on("timeout", () => { req.destroy(); reject(new Error("Timeout")); });
    req.write(body);
    req.end();
  });
...
// ── 玖时API (jiushi.xin) for daily chat / discover / intimate ──
...
export async function askJiushi(opts = {}) {
  return _openaiCompatCall({
    ...opts,
    model: opts.model || "[按量]claude-opus-4-6-thinking",
    host: JIUSHI_HOST,
    apiKey: JIUSHI_KEY,
  });
}
