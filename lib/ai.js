...
function probeJumengai(model) {
  return new Promise((resolve) => {
    const body = JSON.stringify({ model, max_tokens: 10, messages: [{ role: "user", content: "你好" }] });
    const req = https.request({
...
async function detectJumengaiModel() {
  for (const m of JUMENGAI_CANDIDATES) {
    const ok = await probeJumengai(m);
    console.log(`[jumengai] probe ${m} => ${ok ? "OK" : "不可用"}`);
    if (ok) return m;
  }
  throw new Error("聚梦所有候选模型均不可用");
}
