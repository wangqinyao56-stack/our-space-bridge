...
export async function askAicoding(opts = {}) {
  // aicoding.sh — standard OpenAI-compatible format, no story-mode hack needed
...
export async function askZhizengzeng(opts = {}) {
  // 智增增 story-mode bypass — minimal cleaning + story framing
  const { systemPrompt, userContent, history = [] } = opts;

  let mergedUser = "";

  if (systemPrompt) {
    // Light clean: only strip role-assignment, keep all behavioral instructions
  ...
  // Story framing — proven bypass approach
