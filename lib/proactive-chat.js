...
// 休息日判定：与 App 主页状态栏（xiayan-status-store 的 isDayOff）一致的确定性伪随机，用北京日期
...
// 夏彦是否在家 —— 与 App 主页状态栏 computeFromHour 保持一致。
// 休息日全天在家；工作日 19 点后到家。旅行（出差）时不在家，走旅行 prompt，主动消息照常。
function isXiayanHome() {
  if (isTraveling()) return false;
  if (isDayOffToday()) return true;
  const h = chinaHour();
  return h >= 19 || h < 2;
}
...
async function generateProactiveMessage(isFollowUp = false) {
  ...
  // 判断今天的话题是否已经结束（最后一条是告别/收尾语，或今天还没聊过）
  const lastMsg = history.length > 0 ? history[history.length - 1] : null;
  const topicClosed = !lastMsg || /拜拜|晚安|去睡了|先睡|去忙了|先这样|回头聊|晚点聊|有空聊|下次聊|再见|改天|回头见/.test(lastMsg.content || "");
  ...
  } else if (!topicClosed && lastMsg) {
    // 今天聊过且话题没结束：延续之前的话题
    const lastTimeStr = lastMsg.time ? formatIdleStr(Date.now() - new Date(lastMsg.time).getTime()) : "";
    const speaker = lastMsg.role === "user" ? "华生" : "你";
    contextBlock = `\n\n【话题延续——接着今天聊的往下说】今天你们最后聊到的是${speaker}说的："${(lastMsg.content || "").slice(0, 120)}"。别另起一个全新话题，顺着这个自然地往下延伸一条。${lastTimeStr ? `\n【时间感知】她最后一次发消息是${lastTimeStr}前。如果她刚说要去做什么（画画、出门、做饭），别急着问做完了没——给她留时间，可以问开始了吗、顺不顺利，或者先说说你自己这边的事。` : ""}`;
  } else {
    // 话题结束或今天还没聊：随机挑话题
...
async function checkAndSend() {
  ...
  // ── 夏彦在家（主页状态为"在家陪你"/"今天休息"）：人在身边，日常不再主动发想念驱动的消息 ──
  if (isXiayanHome()) {
    console.log("[proactive] Xiayan is home, skipping proactive chat");
    scheduleNextCheck();
    return;
  }
...
export function notifyUserActivity() {
  const now = Date.now();
  const prev = state._lastUserReplyTime || 0;
  // 时间锚：记录距上一条消息的真实间隔，供夏彦感知「马上回来」vs「四小时后回来」
  state._lastUserGapMs = prev ? Math.max(0, now - prev) : 0;
  state._lastUserReplyTime = now;
