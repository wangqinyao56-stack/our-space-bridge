...
const STRINGS_FILE = "./proactive-chat-strings.json";
const CHECK_MIN = 120;
const CHECK_MAX = 180;
...
function pickTopicHint() {
  const available = S.topicHints.filter((t) => !usedTopics.includes(t));
  if (available.length === 0) {
    usedTopics.length = 0;
    return S.topicHints[Math.floor(Math.random() * S.topicHints.length)];
  }
