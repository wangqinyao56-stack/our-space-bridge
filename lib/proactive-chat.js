...
async function checkAndSend() {
  ...
  // ── 夏彦回家：傍晚5-6点主动发"我回来啦"（每天一次）──
  if (chinaHour() === 17) {
    if (state.homeArrivalDate !== chinaDateStr()) {
      state.homeArrivalDate = chinaDateStr();
      save();
      console.log("[proactive] Home arrival (17:00-18:00) — sending 我回来啦");
      const prompt = getDailySystemPrompt();
      const history = await getRecentHistoryMessages();
      const msg = await askJiushi({
        systemPrompt: prompt,
        userContent: "现在是你下班回家的时间（傍晚五点到六点）。请主动给华生发一条消息，告诉她你回来了——像「我回来啦」这样的开场，然后自然地描写一两句你从玄关开门回家的动作（换鞋、放包、喊她、找她……），每次都要不一样，不要用固定的动作。语气开心、撒娇、想她。就一两句，不要长。",
        history: history.slice(-6),
        maxTokens: 150,
      });
      if (msg && msg.trim()) {
        ...
        state.dailyCount++;
        save();
        console.log(`[proactive] Home arrival sent: "${msg.slice(0, 50)}..."`);
      }
      scheduleNextCheck();
      return;
    }
