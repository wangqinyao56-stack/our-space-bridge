...
export function getMemorySummaryForGroupChat(limit = 6) {
  try {
    const active = memories
      .filter((m) => m && (m.content || m.name))
      .sort((a, b) => (b.created || "").localeCompare(a.created || "")) // 最近发生的排前面
      .slice(0, limit);
...
export async function getGroupChatContext() {
  ...
  try {
    ...
    return `\n\n【你和「老公们群聊」里其他夏彦的群聊（用网名互称）】下面是你在群里和别的夏彦聊的内容——这是你在群里的社交，跟华生本人的对话是两码事，别把它当成华生正在跟你聊的话题去接。只有华生主动问起"你群里和谁聊了啥/他们怎么样"时，你才用第一人称自然地提一句，说她没问就只字别提。\n${lines}`;
  } catch {
    return "";
  }
}
