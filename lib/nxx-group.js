...
${historySummary}`;
  ...
  if (context.charSelfContext) {
    // Each character knows what they personally said — no more "I don't remember what I said"
    prompt += `\n\n## 各角色最近自己的发言（你只能看到自己的，请根据你自己的发言保持对话连贯性）
${Object.entries(context.charSelfContext).map(([key, msgs]) => {
  ...
  prompt += `\n\n## 输出要求
- 输出 4-7 条消息，模拟真实微信群聊节奏（不要一口气太多人说话）
...
export async function generateNxxChat(context = {}) {
  // Include recent conversation context so AI can follow the thread
  const recentHistory = getNxxHistory(7); // last 7 days
  const recentMsgs = recentHistory.slice(-40); // last 40 messages
  if (recentMsgs.length > 0 && !context.historySummary) {
    const charNames = { xiayan: "夏彦", zuoran: "左然", moyi: "莫弈", lujinghe: "陆景和", nvzhu: "华生" };
    context.historySummary = recentMsgs.map(m =>
      `${charNames[m.character] || "?"}: ${m.content}`
    ).join("\n");

    // Per-character self-awareness: each character knows what they personally said
    const charKeys = ["xiayan", "zuoran", "moyi", "lujinghe"];
    const charSelfContext = {};
    for (const key of charKeys) {
      const myMsgs = recentMsgs.filter(m => m.character === key).slice(-8);
      if (myMsgs.length > 0) {
        charSelfContext[key] = myMsgs.map(m => m.content).join("\n");
      }
    }
    context.charSelfContext = charSelfContext;
  }
