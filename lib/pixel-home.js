...
function scheduleVisit(s) {
  ...
  s.timers.visit = setTimeout(() => {
    ...
    if (!huashengAway) emit({ type: "text_reply", reply_to: "", content: visit.line, proactive: true });
...
function scheduleDone(s) {
  s.timers.done = setTimeout(() => {
    ...
    emit({ type: "pixel_home_xiayan_move", room: s.huashengRoomIdx, event: currentState.event });
    if (!huashengAway) emit({ type: "text_reply", reply_to: "", content: done.line, proactive: true });
    saveState();
    scheduleNextMove(s);
