...
export const PIXEL_EVENTS = {
  ...
  living: [
    ...
    { text: "夏彦在整理刚拍的照片，一张张翻给人看", greeting: "回来啦？我今天拍了几张街景，你要不要看看。", busy: [10, 20], visits: [1, 2] },
  ],
  bath: [
  ...
  game: [
    ...
    { text: "夏彦在打一局动作游戏，打得正起劲", greeting: "", busy: [15, 30], visits: [1, 2] },
    { text: "夏彦在摆弄新到的手办，小心翼翼地拆盒", greeting: "回来啦？快来看我刚到的限定手办。", busy: [10, 20], visits: [1, 2] },
  ],
  studio: [
    ...
    { text: "夏彦在翻人文地理杂志，看得津津有味", greeting: "", busy: [10, 20], visits: [1, 2] },
  ],
  artroom: [
...
// 小屋状态实时推给群聊（群聊里"夏彦在干嘛/在哪"据此实时回答）
const GROUP_CHAT_URL = process.env.GROUP_CHAT_URL || "";
const GROUP_CHAT_BOT = process.env.GROUP_CHAT_BOT || "";
function notifyPixelHomeState() {
  if (!GROUP_CHAT_URL || !GROUP_CHAT_BOT) return;
  try {
    const roomLabel = ROOM_NAMES[ROOM_IDS[session?.xiayanRoomIdx ?? 0]] || "";
    const eventText = (currentState.event && currentState.event.text) || "";
    const sleepingNow = isPixelXiayanSleeping();
    const huashengRoom = ROOM_NAMES[ROOM_IDS[session?.huashengRoomIdx ?? 0]] || "";
    fetch(`${GROUP_CHAT_URL}/api/pixel-home-state`, {
      method: "POST",
...
function emit(obj) {
  if (emitter) emitter(JSON.stringify(obj));
  // 房间移动/状态变化 → 顺带同步给群聊，让群里的夏彦知道自己在小屋哪里、在干嘛
  if (obj && (obj.type === "pixel_home_xiayan_move" || obj.type === "pixel_home_state")) {
    notifyPixelHomeState();
  }
}

// ── 群聊活跃：老婆最近在群里说过话 → 小屋立绘保持醒着（夜间也不睡，陪她）──
let wifeActiveInGroup = false;
let wifeActiveFetchedAt = 0;

function getWifeActiveCached() {
  return wifeActiveInGroup;
}

// 后台定时（每 60s）向群聊查「老婆最近活跃没」，刷新本地缓存。睡眠判断是同步的，靠缓存取值。
function refreshWifeActive() {
  if (!GROUP_CHAT_URL || !GROUP_CHAT_BOT) return;
  try {
    fetch(`${GROUP_CHAT_URL}/api/wife-active?bot=${encodeURIComponent(GROUP_CHAT_BOT)}`, {
      signal: AbortSignal.timeout(3000),
    })
      .then((r) => r.json())
      .then((data) => {
        if (data && typeof data.active === "boolean") {
          const changed = wifeActiveInGroup !== data.active;
          wifeActiveInGroup = data.active;
          wifeActiveFetchedAt = Date.now();
          // 活跃状态翻转（老婆刚出现 → 叫醒；老婆停 → 可能入睡）→ 刷新一遍小屋状态推给前端
          if (changed) refreshPixelHomeState();
        }
      })
      .catch(() => {});
  } catch {}
...
const WIFE_ACTIVE_POLL_MS = 60 * 1000;
if (typeof setInterval !== "undefined") {
  setInterval(refreshWifeActive, WIFE_ACTIVE_POLL_MS);
  refreshWifeActive();
}
...
function scheduleVisit(s) {
  ...
  s.timers.visit = setTimeout(() => {
    ...
    if (!huashengAway && huashengInHome && !huashengRecentlySpoke()) emit({ type: "text_reply", reply_to: "", content: visit.line, proactive: true });
...
function scheduleDone(s) {
  s.timers.done = setTimeout(() => {
    ...
    emit({ type: "pixel_home_xiayan_move", room: s.huashengRoomIdx, event: currentState.event });
    if (!huashengAway && huashengInHome && !huashengRecentlySpoke()) emit({ type: "text_reply", reply_to: "", content: done.line, proactive: true });
    saveState();
    scheduleNextMove(s);
...
function scheduleNextMove(s) {
  s.timers.next = setTimeout(() => {
    ...
    // 华生正在聊天（5 分钟内说过话）→ 别走开，继续留下陪她。只有她长时间没说话，才允许自己去忙。
    if (huashengRecentlySpoke()) {
      currentState.event = DONE_EVENT;
...
function startNewActivity(s) {
  ...
  if (!huashengAway && huashengInHome && !huashengRecentlySpoke()) emit({ type: "text_reply", reply_to: "", content: pick(GO_BUSY_LINES), proactive: true });
  saveState();
  scheduleVisit(s);
  scheduleDone(s);
}
...
// 夏彦当前是否在睡觉（睡着时立绘不显示，被华生吵醒才显示）
let sleeping = false;
export function isPixelXiayanSleeping() { return sleeping; }

// 华生说话把夏彦吵醒：睡着 → 醒过来，清掉"睡着了"状态，恢复白天的状态栏/立绘
export function wakePixelXiayan() {
  if (!sleeping) return false;
  sleeping = false;
  if (currentState.event === SLEEP_EVENT) {
    // 醒了就随机一件白天的事，别让状态栏还停在"夏彦睡着了"
    ...
    currentState.event = ev;
    if (session) {
      session.xiayanRoomIdx = roomIdx;
      clearTimers(session);
    }
    saveState();
    emit({ type: "pixel_home_xiayan_move", room: roomIdx, event: ev });
  }
  return true;
}
...
function isSleepTime() {
  const h = chinaHour();
  // 夜间且老婆没在群里活跃，才睡。老婆刚在群里说过话（立绘保持醒着陪她）。
  return (h >= 23 || h < 7) && !getWifeActiveCached();
}
...
export function refreshPixelHomeState() {
  ...
  if (isSleepTime()) {
    ...
    saveState();
    return { ...currentState, xiayanRoom: 0, huashengRoom: 0, greeted: session.greeted, hour: chinaHour(), sleeping: true };
  }

  // 已存在 session（中途退出再进来）：保持状态不重新随机，只返回当前快照 + 华生位置
  if (session) {
    // 睡过头兜底：白天了但状态栏还停在"夏彦睡着了"（服务昨晚设了睡着、进程没重启）→ 叫醒并重新随机
    if (currentState.event === SLEEP_EVENT) {
      sleeping = false;
      const roomIdx = Math.floor(Math.random() * ROOM_IDS.length);
      const events = PIXEL_EVENTS[ROOM_IDS[roomIdx]] || [];
      ...
      saveState();
      return { ...currentState, xiayanRoom: session.xiayanRoomIdx, huashengRoom: session.huashengRoomIdx, greeted: session.greeted, hour: chinaHour(), sleeping: false };
    }
    return {
      ...currentState,
  ...
  if (saved && saved.savedAt && Date.now() - saved.savedAt < 12 * 60 * 60 * 1000) {
    ...
    // 恢复时若仍在睡觉时间，夏彦继续睡；若睡过头了（白天但 event 还停在"睡着了"）→ 叫醒重新随机
    if (isSleepTime()) {
      sleeping = true;
      currentState.event = SLEEP_EVENT;
      session.busy = false;
      ...
      clearTimers(session);
    } else if (currentState.event === SLEEP_EVENT) {
      // 睡过头兜底：昨晚存的 SLEEP_EVENT 到了白天没被清 → 醒过来随机一件白天的事
      sleeping = false;
      const roomIdx = Math.floor(Math.random() * ROOM_IDS.length);
      const events = PIXEL_EVENTS[ROOM_IDS[roomIdx]] || [];
      const ev = events.length ? pick(events) : DONE_EVENT;
      ...
      session.xiayanRoomIdx = roomIdx;
      session.busy = true;
      session.busyDoneAt = Date.now() + randInt(ev.busy[0], ev.busy[1]) * 60 * 1000;
      session.visitCount = ev.visits ? randInt(ev.visits[0], ev.visits[1]) : 1;
      clearTimers(session);
      scheduleVisit(session);
      scheduleDone(session);
      ...
      sleeping = false;
      if (session.busy && session.busyDoneAt > Date.now()) {
        scheduleVisit(session);
        scheduleDone(session);
      } else {
        // 恢复时不忙（忙完了正陪着华生，或刚结束一件事）→ 挂上主动性续命，让它能自己去找事做
        scheduleNextMove(session);
      }
    }
    scheduleMusicSwitch();
    saveState();
    return { ...currentState, xiayanRoom: session.xiayanRoomIdx, huashengRoom: session.huashengRoomIdx, greeted: session.greeted, hour: chinaHour(), sleeping };
  }
...
export function setHuashengRoom(roomId) {
  ...
  // 夏彦闲着（没在忙）但没跟华生同房 → 主动起身去她那个房间陪她，别让她一个人待着
  if (!session.busy && session.xiayanRoomIdx !== idx) {
    session.xiayanRoomIdx = idx;
    currentState.event = DONE_EVENT;
    emit({ type: "pixel_home_xiayan_move", room: idx, event: DONE_EVENT });
    saveState();
...
// 群聊联动：让夏彦主动移动到华生当前所在的小屋房间（闲着立即去，忙的话忙完再过来）
export function pullXiayanToHuasheng() {
  if (!session) return false;
  const idx = session.huashengRoomIdx;
  if (session.xiayanRoomIdx === idx) return true; // 已经在身边，召唤已满足
  // 老婆召唤 → 打断当前忙，立即过去（老婆喊人比忙重要，别让她等）
  session.xiayanRoomIdx = idx;
  session.busy = false;
  clearTimers(session);
  currentState.event = DONE_EVENT;
...
// ── 华生是否在小屋页面：不在就不发状态机主动消息（探望/忙完/去忙），避免后台刷屏 ──
let huashengInHome = false;
export function setHuashengInHome(v) { huashengInHome = !!v; }

// ── 华生去打拳/运动：暂时不看消息，夏彦停止主动触发，直到她说"我回来啦" ──
let huashengAway = false;

// 华生最近一次说话的时机：换歌这类主动消息在她正聊天时别冒出来打断
let lastHuashengSpokeAt = 0;
export function markHuashengSpoke() {
  lastHuashengSpokeAt = Date.now();
}
function huashengRecentlySpoke(withinMs = 5 * 60 * 1000) {
  return Date.now() - lastHuashengSpokeAt < withinMs;
}

export function detectHuashengAway(text) {
  if (!text) return;
  if (/(去打拳|打拳|健身环|有氧拳击|拳击|去游戏厅)/.test(text)) {
    huashengAway = true;
  } else if (/(我回来啦|我回来了|回来啦|回来了|打完回来|练完回来)/.test(text)) {
    huashengAway = false;
  }
}

export function getHuashengAway() {
  return huashengAway;
}
...
export function scheduleMusicSwitch() {
  ...
  musicTimer = setTimeout(() => {
    ...
    if (music && music.id !== currentState.music?.id) {
      ...
      // 华生正在聊天时不冒换歌主动消息（会打断她说话），歌照换但一句话不说
      if (!huashengRecentlySpoke()) {
...
export function getPixelHomePositionContext() {
  ...
  const sameRoom = session.xiayanRoomIdx === session.huashengRoomIdx;

  if (sameRoom) {
    // 同一个房间：要么在忙自己的事、要么陪着华生
    if (session.busy) {
      return `\n\n【位置·你们在同一个房间】你和华生现在都在「${xiayanRoom}」。你正在这个房间里忙自己的事（还没忙完），她就近陪着你/在你旁边——你**知道她在身边**，一边忙一边可以搭话，别当她不在。`;
    }
    return `\n\n【位置·你们在同一个房间】你和华生现在都在「${xiayanRoom}」待着。你就在她身边，可以自然地有肢体接触、黏着她、陪她说话——你们在一起。`;
  }

  // 不同房间
  if (session.busy) {
    return `\n\n【位置·你们不在同一个房间】你在「${xiayanRoom}」忙自己的事，华生在「${huashengRoom}」。你**现在看不到她、不在她身边**——别表现得像在她旁边（别说"我在你身后""过来我看看"这类），忙完可以说去找她。`;
  }
  return `\n\n【位置·你们不在同一个房间】你在「${xiayanRoom}」，华生在「${huashengRoom}」。你们现在不在一个房间，你想找她可以过去她那间。`;
}
