...
function scheduleVisit(s) {
  ...
  s.timers.visit = setTimeout(() => {
    ...
    if (!huashengAway && huashengInHome) emit({ type: "text_reply", reply_to: "", content: visit.line, proactive: true });
...
function scheduleDone(s) {
  s.timers.done = setTimeout(() => {
    ...
    emit({ type: "pixel_home_xiayan_move", room: s.huashengRoomIdx, event: currentState.event });
    if (!huashengAway && huashengInHome) emit({ type: "text_reply", reply_to: "", content: done.line, proactive: true });
    saveState();
    scheduleNextMove(s);
...
function startNewActivity(s) {
  ...
  if (huashengInHome) emit({ type: "text_reply", reply_to: "", content: pick(GO_BUSY_LINES), proactive: true });
  saveState();
  scheduleVisit(s);
  scheduleDone(s);
}
