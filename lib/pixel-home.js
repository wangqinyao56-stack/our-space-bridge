...
// 忙完后状态栏显示（不再提刚才那件事——擦完就是擦完，别重复）
const DONE_EVENT = { text: "夏彦忙完了，陪在你身边", greeting: "", busy: [0, 0], visits: [0, 0] };
...
function scheduleDone(s) {
  s.timers.done = setTimeout(() => {
    if (session !== s) return;
    s.busy = false;
    // 忙完了：清掉刚才那件事，状态栏换成"陪在你身边"，不再提擦滑板之类的话
...
export function setHuashengRoom(roomId) {
  ...
  if (wasTogether && session.xiayanRoomIdx !== idx) {
    ...
    emit({ type: "pixel_home_xiayan_move", room: idx, event: currentState.event || DONE_EVENT });
  }
}
