...
// 忙碌途中主动探望华生时说的话（倒茶/切水果/亲一口/蹭一蹭/看一眼）
const VISIT_LINES = [
  "渴不渴？给你倒了杯茶，放这儿啦。",
  "切了点水果放你手边，记得吃。",
  "过来看看你……亲一口，继续忙啦。",
...
function clearTimers(s) {
  if (!s) return;
  for (const k of ["done", "visit", "leave"]) {
    if (s.timers[k]) clearTimeout(s.timers[k]);
  }
  s.timers = { done: null, visit: null, leave: null };
}

// 中途探望：夏彦过来看华生 → 说句话/小动作 → 停留片刻 → 若还在忙则回去继续
function scheduleVisit(s) {
  if (!s.busy || s.visitCount <= 0) return;
  const totalMs = Math.max(1, s.busyDoneAt - Date.now());
  const intervalMs = Math.min(8 * 60 * 1000, Math.max(90 * 1000, totalMs / (s.visitCount + 1)));
  s.timers.visit = setTimeout(() => {
    if (session !== s) return;
    const wasSameRoom = s.xiayanRoomIdx === s.huashengRoomIdx;
    if (!wasSameRoom) emit({ type: "pixel_home_xiayan_move", room: s.huashengRoomIdx });
    emit({ type: "text_reply", reply_to: "", content: pick(VISIT_LINES), proactive: true });
    s.timers.leave = setTimeout(() => {
      if (session !== s) return;
      s.visitCount -= 1;
      if (!wasSameRoom && s.busy) emit({ type: "pixel_home_xiayan_move", room: s.xiayanRoomIdx });
      scheduleVisit(s);
    }, 25000);
  }, intervalMs);
}
...
function scheduleDone(s) {
  s.timers.done = setTimeout(() => {
    if (session !== s) return;
    s.busy = false;
    if (s.xiayanRoomIdx !== s.huashengRoomIdx) {
      emit({ type: "pixel_home_xiayan_move", room: s.huashengRoomIdx });
    }
    s.xiayanRoomIdx = s.huashengRoomIdx;
    emit({ type: "text_reply", reply_to: "", content: pick(DONE_LINES), proactive: true });
...
export function refreshPixelHomeState() {
  ...
  session = {
    ...
    timers: { done: null, visit: null, leave: null },
  };
  scheduleVisit(session);
  scheduleDone(session);
  return { ...currentState, xiayanRoom: roomIdx, hour: chinaHour() };
}

export function setHuashengRoom(roomId) {
  if (!session) return;
  const idx = ROOM_IDS.indexOf(roomId);
  if (idx >= 0) session.huashengRoomIdx = idx;
}
