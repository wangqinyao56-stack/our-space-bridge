...
const VISIT_LINES = [
  { line: "渴不渴？给你倒了杯茶，放这儿啦。", status: "夏彦在你桌边放了杯热茶" },
  { line: "切了点水果放你手边，记得吃。", status: "夏彦在你手边放了碗切好的水果" },
  { line: "过来看看你……亲一口，继续忙啦。", status: "夏彦走过来，亲了你一下" },
  { line: "就想过来蹭蹭你，抱一下再走。", status: "夏彦走过来，抱了抱你" },
  { line: "路过看看你，别太累，我忙完就来陪你。", status: "夏彦过来看了看你" },
];

// 忙完过来找华生时说的话
const DONE_LINES = [
  { line: "忙完啦，过来陪你了。", status: "夏彦忙完了，走到你身边抱住你" },
  { line: "事儿弄完了，坐你旁边了。", status: "夏彦忙完了，坐到你身边陪你" },
  { line: "搞定，我回你身边了。", status: "夏彦走过来，亲了你一下" },
];
...
export function refreshPixelHomeState() {
  ...
  if (isSleepTime()) {
    ...
    saveState();
    return { ...currentState, xiayanRoom: 0, huashengRoom: 0, hour: chinaHour() };
  }
  ...
  if (saved && saved.savedAt && Date.now() - saved.savedAt < 12 * 60 * 60 * 1000) {
    ...
    scheduleMusicSwitch();
    saveState();
    return { ...currentState, xiayanRoom: session.xiayanRoomIdx, huashengRoom: session.huashengRoomIdx, hour: chinaHour() };
  }
  ...
  scheduleDone(session);
  scheduleMusicSwitch();
  return { ...currentState, xiayanRoom: roomIdx, huashengRoom: 0, hour: chinaHour() };
}
