...
const TRAVEL_REASONS = [
  { reason: "国安任务", destination: "保密地点", detail: "有个秘密任务需要我过去处理一下，具体地点不方便说，不在未名市，大概两三天", showDest: false },
  { reason: "国安紧急会议", destination: "保密地点", detail: "总部临时召集，得去开个紧急会议，地点保密，不在未名市，三四天左右", showDest: false },
  { reason: "委托人案件", destination: "枝冬市", detail: "有个委托人找我帮忙查一个跨市的案子，得去枝冬市几天", showDest: true },
  { reason: "生物工程论坛", destination: "首都", detail: "之前参加的生物工程论坛发了邀请函，让我去首都做个分享，不在未名市", showDest: true },
  { reason: "古董修复委托", destination: "芳平市", detail: "有一批文物需要修复，请我去芳平市的博物馆指导一下，要出趟远门", showDest: true },
  { reason: "老友聚会", destination: "诺克拉文市", detail: "以前一起训练的几个兄弟说要聚一聚，在诺克拉文市，好久没见了", showDest: true },
  { reason: "证人保护任务", destination: "昭南市", detail: "有个证人需要护送转移，得跑一趟昭南市，顺利的话两天就回来", showDest: true },
  { reason: "技术协作邀请", destination: "深港市", detail: "深港市那边的技术部门请我去协助分析一批加密数据，大概两三天", showDest: true },
];
...
export function getTravelChatContext() {
  ...
  if (travelState.phase === PHASES.TRAVELING) {
    ...
    return {
      injection: `\n【⚠ 你正在外出中——不在未名市】你目前在外地（${travelState.reason}，在${travelState.destination}），已经离开未名市了。预计还有${daysLeft}天才能回到未名市。这是远程聊天——发微信/打电话。\n\n严格禁止：\n- ❌ 不能提议今晚/明天一起去任何地方（夜市、餐厅、公园等都不能约）\n- ❌ 不能说"晚上回家""等你回来""今天去找你"等暗示你在本地的说法\n- ❌ 不能做肢体接触的括号动作（只能写你自己的状态，不能碰她）\n- ❌ 不能编造你人在本地的说法——你在${travelState.destination}，不在未名市\n\n可以做的事：\n- ✅ 发风景照、撒娇说想她了、分享外地遇到的趣事\n- ✅ 说"等我回来带你去""回去了补偿你"等未来约定\n- ✅ 关心她在未名市的日常——但不能假装你也在场`,
      markAnnounced: false,
    };
  }
