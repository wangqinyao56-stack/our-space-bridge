...
const TRAVEL_REASONS = [
  { reason: "国安任务", destination: "保密地点", detail: "有个任务需要我过去处理一下，具体地点不方便说" },
  { reason: "委托人案件", destination: "外地", detail: "有个委托人找我帮忙查一个案子，得去外地几天" },
  { reason: "赛车队邀请", destination: "赛道", detail: "赛车队那边说有个活动，约了我一起去骑一圈" },
  { reason: "学术会议", destination: "外地会场", detail: "之前参加的生物工程论坛发了邀请函，让我去做个分享" },
  { reason: "古董修复委托", destination: "外地博物馆", detail: "有一批文物需要修复，请我过去指导一下" },
  { reason: "老友聚会", destination: "外地", detail: "以前一起训练的几个兄弟说要聚一聚，好久没见了" },
];
...
export function getTravelChatContext() {
  if (travelState.phase === PHASES.ANNOUNCING) {
    if (!travelState.announcedAt) {
      ...
      return {
        injection: `\n【出行通知——必须执行】你需要在这轮对话中自然地告诉华生你要出远门。原因：${travelState.reason}。具体说法：${travelState.detail}。要自然——不是宣布任务，是跟老婆说一声你要出门几天。可以说你会想她、会给她带礼物、提醒她照顾好自己。说完后对话照常继续即可。`,
        markAnnounced: true,
      };
    }
  ...
  if (travelState.phase === PHASES.TRAVELING) {
    ...
    return {
      injection: `\n【你正在外出中】你目前在外地（${travelState.reason}，${travelState.destination}）。预计还有${daysLeft}天回来。你现在是在跟华生远程聊天——发微信、打电话。不能做肢体接触的动作（括号动作只能写你自己的状态，不能碰她）。可以发风景照、可以撒娇说想她了、可以分享你在外地遇到的事。`,
      markAnnounced: false,
    };
  }
