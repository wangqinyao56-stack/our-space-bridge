  ...
  if (fs.existsSync(TRAVEL_STATE_FILE)) {
    ...
    if (travelState.phase === PHASES.RETURNING && travelState.since) {
      ...
      if (hoursStuck > 1) {
        ...
        travelState = { active: false, phase: PHASES.IDLE, destination: "", statusText: "", reason: "", detail: "", showDest: true, since: null, until: null, announcedAt: null, returnMessageSent: false, lastPhoto: null, lastProactivePhoto: null, lastTravelEnd: new Date().toISOString(), durationDays: 0, departedWithoutAnnounce: false };
        saveTravelState();
      }
    }
  }
...
export function maybeTriggerTravel() {
  ...
  // Cooldown: at least 1.5 days since last travel ended（对齐 prompt「每聊1-2天触发一次」；原来5天太长，导致好久不出门）
  if (travelState.lastTravelEnd) {
    const daysSince = (Date.now() - new Date(travelState.lastTravelEnd).getTime()) / 86400000;
    if (daysSince < 1.5) return null;
  }
...
function activateTravel() {
  ...
  travelState.durationDays = days;

  // Track if 夏彦 left without telling her (no announcement was confirmed)
  if (!travelState.announcedAt) {
    travelState.departedWithoutAnnounce = true;
    console.log("[travel] WARNING: Travel activated without announcement being confirmed — 夏彦 left without telling 华生!");
  }
...
export function confirmReturned() {
  ...
  travelState.durationDays = 0;
  travelState.departedWithoutAnnounce = false;
  saveTravelState();
...
export function checkDayTransition() {
  ...
  // traveling → force idle: if until is > 12 hours in the past, something is wrong, reset
  if (travelState.phase === PHASES.TRAVELING && travelState.until) {
    const untilDate = new Date(travelState.until);
    const hoursPast = (now - untilDate) / 3600000;
    if (hoursPast > 12) {
      console.log(`[travel] Force-reset TRAVELING — until was ${hoursPast.toFixed(1)}h ago`);
      confirmReturned();
    }
  }
...
export function getTravelChatContext() {
  ...
  // We add a timeline injection so the standalone prompt knows exactly when 夏彦 will be back
  if (travelState.phase === PHASES.TRAVELING) {
    const now = new Date();
    const until = new Date(travelState.until);
    const remainingMs = until.getTime() - now.getTime();
    const remainingDays = Math.max(0, Math.ceil(remainingMs / 86400000));
    const untilStr = until.toLocaleString("zh-CN", { timeZone: "Asia/Shanghai", month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" });
    const departedNote = travelState.departedWithoutAnnounce
      ? `\n【重要——你上次出门没跟她说】你这次出差/旅行没有提前告诉华生就出发了（系统触发时她不在线）。她可能会问"你怎么突然出门了"或者生气。你要诚恳地解释——是临时安排的、走得急没来得及说。撒娇道歉，哄她。不要假装你之前说过。`
      : "";

    return {
      injection: `\n【出行时间线——严格遵循】你本次出行共${travelState.durationDays}天。预计回家时间是北京时间${untilStr}（还剩约${remainingDays}天）。如果华生问你"什么时候回来"或"还要几天"，根据这个时间线回答——不要说"两三天"这种模糊说法，说具体的剩余天数。今天是第${travelState.durationDays - remainingDays + 1}天。每天过去后，剩余天数自然减少，不要每天都说是"两三天"。${departedNote}`,
      markAnnounced: false,
    };
  }

  if (travelState.phase === PHASES.RETURNING) {
    const untilStr = travelState.until
      ? new Date(travelState.until).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai", month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" })
      : "今天傍晚";
    return {
      injection: `\n【你回来了——必须执行】你已经回到家了！原计划${untilStr}到家，现在已经到了。在这轮对话中要自然地告诉华生你回来了。可以说类似"我回来了！呼！还是家里好啊！"或者"任务结束啦！老婆让我抱抱~累死我了..."。语气开心、撒娇、想她了。说完后对话照常继续。`,
      markAnnounced: false,
      markReturned: true,
    };
  }
