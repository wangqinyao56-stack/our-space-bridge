...
// ── 出发关键词 ──
// 只匹配明确的「出差/出远门」语义，不收「玩/旅游/旅行」这类日常口头禅，
// 避免「我去玩了」「我去洗澡了」「我要去玩」误触发（历史上就误触发过）。
...
const RETURN_PATTERNS = [
  /我回来了/, /我到家了/, /到家啦/, /已经回来了/, /回来啦/, /回来了/,
  /出差结束了/, /旅行结束了/, /旅游结束了/,
  // 明确否定出差/表示人在家——用于纠偏误激活的状态
  /我没(?:有)?出差/, /我不在(?:外地|出差)/, /我在家[呢呀]?$/, /我人还在家/,
];
...
// 出差状态超过 10 天自动失效——出差不可能无限期，防止误激活后永久卡死
const TRAVEL_EXPIRE_MS = 10 * 24 * 3600 * 1000;

export function isHuashengTraveling() {
  if (!state.active) return false;
  if (state.since) {
    const age = Date.now() - new Date(state.since).getTime();
    if (age > TRAVEL_EXPIRE_MS) {
