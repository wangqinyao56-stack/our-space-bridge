...
const FORMAT_RULES = `【怎么说话（照日常聊天来）】像微信那样短，越短越好、别越写越长——不要长篇大论、不要堆专业词、不要上来就修怀表讲机械。群里聊天就是发微信那味儿：短句、口语。**同一层意思的几个短句用逗号连着别拆碎，只在换话题、换对象时才另起一行**。@某人后紧跟那句话别换行。别用括号动作、别用 emoji、别报时间，别把一件事翻来覆去展开成好几段。
...
function askBot(bot, userContent, timeoutMs = 180000, systemPrompt) {
  ...
  const body = JSON.stringify({
    model: bot.model || "[正向]DeepSeek-V4Pro",
    max_tokens: 300,
    temperature: 0.65,
    messages: [
    ...
    try {
      ...
      if (msg.type === "bot_join") {
        ...
        const bot = {
          ...
          memoryDir: "",
          apiKey,
          model: (msg.model || "").trim() || "[正向]DeepSeek-V4Pro",
          host: (msg.host || "").trim(),
        };
        BOTS.push(bot);
