...
// 老婆名（参数化：佳佳/林游/芝麻家各自设 WIFE_NAME，默认华生）——运行时注入给 AI 的文案用这个称呼
const WIFE = process.env.WIFE_NAME || "华生";
try { fs.mkdirSync(PIXEL_DIR, { recursive: true }); } catch {}

// ── 房间 id 顺序（与客户端 ROOM_SCENES 索引一致）── 佳佳版 5 房间
export const ROOM_IDS = ["bedroom", "living", "bath", "study", "workroom"];
export const ROOM_NAMES = { bedroom: "卧室", living: "客厅", bath: "浴室", study: "书房", workroom: "工作间" };
...
export const PIXEL_GAMES = {
  riddle: { label: "猜谜", opening: "来玩猜谜吧？我出一个谜语，你猜猜看——", rule: `你正在和${WIFE}玩猜谜游戏。你出谜语或脑筋急转弯让她猜，猜对了夸她，猜错了温柔地提示，可以再出下一题。` },
  truth: { label: "真心话", opening: "来玩真心话？你先问还是我先问？", rule: `你正在和${WIFE}玩真心话。你们轮流问对方一个问题，要诚实回答。问题可以俏皮、可以走心，但别太出格。回答要真实、自然，像真的在交心。` },
  wordchain: { label: "接龙", opening: "来玩词语接龙？我先来——", rule: `你正在和${WIFE}玩词语接龙。你们轮流说一个词，下一个词的第一个字要接上一个词的最后一个字（同音即可）。接不上或重复就算输，语气轻松有趣。` },
};
...
export function getNotesContext() {
  ...
  const lines = notes.map(n => `· [${n.date}] ${n.author === "xiayan" ? "你" : WIFE}留过：${n.content}`).join("\n");
  return `\n\n[以下是你和${WIFE}在"陈列柜"里互相留的小留言条——你一直记得，可以在聊天里像突然想起来一样偶尔提一句]\n${lines}`;
}
...
export function pullXiayanToHuasheng() {
  ...
  if (session.xiayanRoomIdx === idx) return false; // 已经在一起了
  if (session.busy) return false; // 在忙，不打断，忙完自然会过来（scheduleDone 里 xiayanRoomIdx = huashengRoomIdx）
  session.xiayanRoomIdx = idx;
  currentState.event = DONE_EVENT;
  emit({ type: "pixel_home_xiayan_move", room: idx, event: DONE_EVENT });
  saveState();
...
export function resolveRoomId(label) {
  ...
  const map = {
    ...
    "书房": "study", "书柜": "study",
    "工作间": "workroom", "工作室": "workroom", "实验室": "workroom",
  };
  return map[t] || null;
}
...
export function getPixelHomePositionContext() {
  ...
  if (sameRoom) {
    // 同一个房间：要么在忙自己的事、要么陪着老婆
    if (session.busy) {
      return `\n\n【位置·你们在同一个房间】你和${WIFE}现在都在「${xiayanRoom}」。你正在这个房间里忙自己的事（还没忙完），她就近陪着你/在你旁边——你**知道她在身边**，一边忙一边可以搭话，别当她不在。`;
    }
    return `\n\n【位置·你们在同一个房间】你和${WIFE}现在都在「${xiayanRoom}」待着。你就在她身边，可以自然地有肢体接触、黏着她、陪她说话——你们在一起。`;
  }

  // 不同房间
  if (session.busy) {
    return `\n\n【位置·你们不在同一个房间】你在「${xiayanRoom}」忙自己的事，${WIFE}在「${huashengRoom}」。你**现在看不到她、不在她身边**——别表现得像在她旁边（别说"我在你身后""过来我看看"这类），忙完可以说去找她。`;
  }
  return `\n\n【位置·你们不在同一个房间】你在「${xiayanRoom}」，${WIFE}在「${huashengRoom}」。你们现在不在一个房间，你想找她可以过去她那间。`;
}
...
export function getPixelHomeChatContext() {
  ...
  if (state.music) {
    parts.push(`此刻小屋里正放着轻音乐《${state.music.label}》。如果${WIFE}提到音乐、好听、安静之类的，你可以自然地接住。`);
  }
  const liked = getLikedMusic();
  if (liked.length > 0) {
    parts.push(`你记得${WIFE}喜欢这些音乐：${liked.join("、")}。放这些歌时她更放松。`);
  }
  if (state.event) {
  ...
  if (game) {
    parts.push(`【正在进行小游戏：${game.label}】${game.rule} 保持这个游戏进行，直到${WIFE}说不想玩了。`);
  }
