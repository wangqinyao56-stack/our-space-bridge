...
const ZHAILIAN_MODEL = "[0.3]不明鹿/claude-opus-4-6";
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      headers: {
        ...
        "Authorization": `Bearer ${ZHAILIAN_KEY}`,
      },
      timeout: timeoutMs,
    }, (res) => {
      const chunks = [];
      ...
      res.on("end", () => {
        if (res.statusCode !== 200) {
          ...
          reject(new Error(`zhailian ${res.statusCode}: ${errBody}`));
          return;
        }
        try {
...
function proxyRequest({ body, timeoutMs }) {
  return new Promise((resolve, reject) => {
    const req = http.request({
      ...
      path: `${ZHAILIAN_HOST}:443`,
      headers: { Host: `${ZHAILIAN_HOST}:443` },
    });
      ...
      const r = https.request({
        ...
        resp.on("end", () => {
          if (resp.statusCode !== 200) {
            const errBody = Buffer.concat(chunks).toString().slice(0, 300);
            reject(new Error(`zhailian ${resp.statusCode}: ${errBody}`));
            return;
          }
          try {
            const data = JSON.parse(Buffer.concat(chunks).toString());
 ...
 * @param {string} [opts.model]        - Default: [0.3]不明鹿/claude-opus-4-6
