...
async function handlePixelChat(text, quote) {
  ...
  // 记忆标记（跟日常一致）
  systemPrompt += "\n\n**【记忆标记】如果聊到值得长期记住的事，在回复单独一行用 [记]标题|正文[/记] 写下来。不要滥用。这个标记不会显示给林游。**";
...
httpServer.listen(PORT, "0.0.0.0", () => console.log(`[linyou-chat] HTTP+WS 已监听 :${PORT}`));

wss.on("connection", (ws) => {
  console.log("[linyou-chat] 林游已连接");
  ws.isAlive = true;
  ws.on("pong", () => { ws.isAlive = true; });
  // 小屋状态机的主动消息（探望/忙完/去忙/换歌/休息提醒）推给当前连接的林游
    ...
    try {
      ...
      // ── 像素小屋 ──
        ...
        try {
          ...
          console.error("[linyou-chat] pixel home error:", err.message);
